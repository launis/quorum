...
* **Backend Joustavuus vs Frontend Strict-Nirvana:** Vaikka Frontend noudattaa ehdotonta Fail-Fast -mallia (`disallow_unrecognized_keys: true`), Backendin Pydantic-malleille (esim. V2_Core) sallitaan käytännön joustavuus (`strict=False` tietyissä Enumeissa). Tämä mahdollistaa mm. TinyDB:n palauttamien merkkijonojen turvallisen konversio-operaation. Kuitenkin siirrossa API:n ylitse uuteen Flutter-kappaleeseen Frontend vaatii priimaa (100% tiukkuus).
...
* **O(1) Lists & Zero-Touch Riippuvuudet:** Riverpodin suorituskyvyn varmistamiseksi massiivisilla listoilla (esim. RAG-lokit) **EI ASENNETA** erillisiä infrastruktuuripaketteja (kuten `fast_immutable_collections`), jotta `pubspec.yaml` riippuvuudet pysyvät koskemattomina (Zero-Touch). Sen sijaan käytetään natiivisti Dartin `List<T>`-rakennetta, mutta syvävertailu ohitetaan täysin annotaatiolla `@Freezed(equal: false)`.
...
* **Ei ID-kenttien kysymistä käyttäjältä (Opaque ID Ban):** Luonti- ja muokkauslomakkeissa (Forms) ei saa IKINÄ olla näkyvissä "ID" tekstikenttää, jota käyttäjä tai pääkäyttäjä täyttäisi käsin. Kaikki Stripe Pattern ID:t (esim. `blk_abc123`) generoidaan järjestelmän toimesta aina taustalla The Cascading Clone tai luontiprosessien aikana. Käyttäjä syöttää ainoastaan inhimillisen nimikkeistön (`label`, `description`). Slugit ovat poistettu (Epic 10).
