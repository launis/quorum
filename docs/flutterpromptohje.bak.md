...
> **1. Backend Supplies Data, Frontend Supplies Presentation**
> The Backend MUST NEVER send translated UI texts over the API. The server sends ONLY immutable keys and Enum codes (e.g., `{"status": "AUTH_ORGANIC"}`). The ONLY exception is dynamic free-text natively generated by the AI (LLM).
> 
> **2. Split Responsibility**
> * **Frontend (Flutter):** Solely responsible for all UI translations (`.arb` files). The code key is translated to the user's language only at the UI rendering phase.
> * **Backend (Python):** Uses the localization service *strictly* for internal Server-Side Rendering (such as generating PDF reports).
> 
> **3. Absolute Respect for Grammar (No Hacks)**
> String concatenation in code (e.g., `"Welcome " + name`) and manual if/else plurals (e.g., `if count == 1`) are **STRICTLY FORBIDDEN**. All variable interpolations and pluralization rules MUST be handled using ICU formatting directly inside the translation file (e.g., `"welcomeUser": "Welcome {name}!"`).
> 
> **4. Keys are Sacred**
> In dynamic tools (e.g., Cognitive Studio), Administrators may ONLY edit *values*. Altering or translating System Keys (e.g., `history_text`) inside the database permanently breaks the system's global localization.

THE CENTRALIZED LOCALIZATION MANDATE (The "No-String" Policy)
...
* **Execution**: Context Language is auto-detected via ContextVar. Do NOT pass `lang` explicitly.
...
### **10.6. Studio & Builder Localization Safety**

The **Cognitive Studio** (Workflow Builder) operates in **Raw Mode**. It interacts directly with the `seed_data.json` / Registry structure key-values. This makes it highly susceptible to localization corruption.
...
* **The Hazard**: 
  ...
  * **The Risk**: If an Administrator renames *History Text* to *Historiateksti* in the Studio UI, the **Key** in the database becomes *Historiateksti*. The Backend translation lookup breaks globally, and English users will see "Historiateksti".

* **The Rule ("Edit Values, Never Keys")**:
  * **Allowed**: Changing numerical weights, thresholds, system prompts, and AI instruction text.  
  * **Forbidden**: Renaming generic UI labels or system keys in the Builder Config. The Studio is for **Assembly**, not **Copywriting**.
