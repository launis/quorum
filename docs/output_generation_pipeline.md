...
**Matriisien Monikielisyys (The Translation Schema Doctrine) & Deep Objects:**
Matriisit (PromptBlock) on normalisoitu sisältämään **Deep Object -rakenteet** alielementeilleen (`MatrixRow` ja `MatrixClaim`). Rakenteessa on noudatettava tiukkaa arkkitehtonista kahtiajakoa (Bifurcation):
*   **LLM Prosessointi (The Brain):** Konepellin alla, kun DAG-moottori rakentaa kehotetta (promptia), se käyttää `prompt_compiler.py`:ssä yksinomaan komponenttien `ai_label` ja `ai_description` -kenttiä **aina englanniksi** (Language of instructions). Tämä maksimoi LLM:n ymmärryksen erottelukyvyn ja tekee pisteytyksestä armottoman tarkan. Käännöksiä ei anneta AI:lle koskaan.
*   **Renderöinti (The UI/PDF):** Kun `render_blueprint` käskee moottoria piirtämään matriisin X/Y-akselit ja tulostamaan rivin (Row) tai väitteen (Claim) näytölle, renderöintimoottori (`/render` tai `BlueprintTransformer`) lukee pyydetyn kielen (esim. `?lang=fi`). Moottori hakee tietokannan alkuperäisestä `PromptBlock` oliosta kyseisen `MatrixRow` tai `MatrixClaim` -solmun ja poimii nimenomaan sen sisäisestä asiointiobjektista `label.translations.fi` mukaisen arvon näytölle (Language of presentation). Renderöijän koodi ei saa koskea AI-kenttiin. **Tämä on arkkitehtuurin erikseen hyväksytty poikkeus No-String -sääntöön. Staattiset UI-tekstit tulevat Frontendin `.arb`-tiedostosta, mutta dynaamiset SDUI-matriisisisällöt resolvoidaan poikkeuksellisesti Backendin BlueprintTransformerissa asiointikielelle.**

Näin yhdestä ja samasta englanninkielisestä "Aivojen" ajosta (Execution) voidaan napin painalluksella renderöidä täydellinen suomen- tai englanninkielinen raportti pelkkää asetteluohjetta (Blueprint) tulkitsemalla.
