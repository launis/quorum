...
* **Fail-Fast Protocol (RFC 7807 & Zero-Fallback)**: The system never uses `try-except pass` to silence errors or guess default values. If an entity is missing, data is malformed, or relations are violated, the Service layer immediately raises an `AppException` (Fail-Fast).
  * *Recovery Protocol (Data Integrity):* Automated failures must never trigger a blind override or revert of manual configurations (like `seed_data.json`). Root causes must be analyzed and patched sequentially to preserve all uncommitted data state.
* **Context-Aware Citations & Graceful Degradation**: Enforces a strict Pydantic parsing of citations. Hallucinated sources are gracefully nullified by the API and intercepted via Dual-Reporting (logging the failure without crashing backend processing), empowering the frontend to collapse UI elements (`SizedBox.shrink()`) per the "No-String Mandate".
