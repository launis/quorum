...
- [x] **Semantic Document Model (SDUI Refactoring)**: Transitioned from a visual UI model to an agnostic content model. The backend now returns semantic blocks (`type`, `label`, `value`, `intent`) instead of visual components, guaranteeing WYSIWYG parity between Flutter (Screen) and Jinja2 (PDF). Generative generic SDUI usage is restricted in favor of strict semantic domain modeling.
    - *Action 1*: Stripped UI/CSS references from Backend DTOs (`models/view/sdui.py` -> `semantic_models.py`).
    - *Action 2*: Refactored `bff_transformer.py` to assemble semantic blocks instead of defining explicit UI views.
    - *Action 3*: Refactored Frontend router (`schema_mapper.dart`) to act as a semantic interpreter mapping types to existing native widgets.
    - *Action 4*: Unified PDF generation on the backend driven by the identical semantic block list, removing the duplicated Flutter-side PDF generation.
