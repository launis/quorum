...
- **Micro-Level Concurrency Mandate**: You MUST rely on the global `LLMClient`'s internal semaphore for concurrency limits. DO NOT instantiate a localized `asyncio.Semaphore` inside the batch function. A local semaphore created per batch call does not throttle global concurrency and provides a false sense of security. The macro-level orchestrator and the `LLMClient` globally manage `SystemConcurrency.MAX_CONCURRENT_LLM_STEPS` per the `system_concurrency_ssot` rule.
- **Ensemble LLM Client (DI Fix)**: The Bo3 MUST use a separate `ensemble_client: LLMClient` initialized via `LLMClient.from_strategy(get_settings().ensemble_strategy_name, repo)`. 
  - **Dependency Injection Rule**: You MUST NOT pass `repo` into the static `evaluate_atom_boolean_batch()` method. Instead, instantiate the `ensemble_client` at the macro orchestrator level (`dag_executor.py`), pass it into `EnrichedDagExecutor.__init__`, and forward it to `evaluate_atom_boolean_batch()`.
...
- **Insufficient Consensus & DLQ Routing (CRITICAL)**: If no single status achieves the minimum consensus for a `tda_id`:
  1. **Transient API Failure Split (< 2 valid results total)**: If the lack of consensus is due to transient API timeouts (e.g., 2 of 3 calls returned `None`), `resolve_majority_vote` MUST NOT raise an exception. Raising an exception would violently crash the macro-level `asyncio.TaskGroup` in the EnrichedDagExecutor. Instead, mark the atom as `ExecutionStatus.SYSTEM_ERROR` with reasoning `"UPSTREAM_TRANSIENT_TIMEOUT"`. This routes the atom gracefully to the DLQ per `dlq_arq_fallback_routing`.
