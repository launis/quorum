...
- **SSOT Compliance**: (RESOLVED) The BO3 Orchestrator must not rely on a globally hardcoded strategy in `settings.py`. To prevent a Split-Brain architecture and strictly enforce the Single Source of Truth, the orchestrator MUST read the execution strategy dynamically from the database-driven `step.model_strategy` (`seed_data.json`).
