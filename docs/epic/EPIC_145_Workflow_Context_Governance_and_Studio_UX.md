...
2. **What Is Preserved & Curated for Rich Synthesis**:
   - **Exact Direct Quotes**: `MatrixExplanationService` curates the sharpest user voice and verbatim evidence (`settings.max_synthesis_quote_length`, 3-5 quotes per matrix).
...
| Monolithic input checkboxes container | `@[client_app_v2/lib/features/studio/views/widgets/workflow/workflow_step_card.dart]` | Refactored | Split into Categorized Source Material and Prior Step Reports |
| Jargon header `studioWorkflowInputMappings` | `@[client_app_v2/lib/l10n/app_fi.arb]`, `@[client_app_v2/lib/l10n/app_en.arb]` | Replaced | Replaced by `studioWorkflowSourceDocsTitle` and `studioWorkflowPriorStepsTitle` |
...
2. **Fail-Fast vs. Truncation Boundary**:
...
3. **Cross-Domain DTO Parity**: Python Pydantic V2 `StepRule` and Dart 3 Freezed `StepRule` synchronously updated with `is_synthesis_source` / `isSynthesisSource`.
4. **Dual-Axis Localization**: Zero hardcoded strings in Flutter widgets. All labels, subtitles, and tooltips defined in `app_fi.arb` and `app_en.arb`.
...
- **Target Files**:
  - `@[backend_v2/models/v2_core.py#L801-L833]` (`StepRule` & `Step` models)
  - `@[backend_v2/models/v2_core.py#L1070-L1087]` (`SynthesisConfigDTO`)
  ...
  - `@[backend_v2/api/routers/studio/steps.py]` (Step CRUD & Protection API)
...
- **Actions**:
  ...
  6. In backend Step deletion API / service:
...
- **Target Files**:
  - `@[client_app_v2/lib/l10n/app_fi.arb#L1075-L1100]`
  - `@[client_app_v2/lib/l10n/app_en.arb#L1620-L1645]`
...
- **Actions**:
  1. Implement comprehensive unit and negative test suite in `test_synthesis_payload_compression.py`:
