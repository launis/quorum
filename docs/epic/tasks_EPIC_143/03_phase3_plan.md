...
**Overview:** Harden `matrix_explanation_service.py` to eliminate 12 legacy anti-patterns, implement Status-Aware Dual Reporting (`SUPPORTING EVIDENCE` + `UNMET CRITERIA / DEFICITS`) with Ranked Round-Robin curation, define `MatrixExplanationContextList = TypeAdapter(list[MatrixExplanationContextDTO])` to eliminate Double-Serialization in `worker.py`, integrate SSOT quote limits in `SynthesisPayloadCompressor`, update Knowledge Item documentation, and expand unit/contract test coverage.
**Target Files:**
- `[MODIFY]` @[backend_v2/models/dtos/synthesis.py]
- `[MODIFY]` @[backend_v2/worker.py#L910-L980]
- `[MODIFY]` @[backend_v2/services/orchestrator/matrix_explanation_service.py#L15-L165]
- `[MODIFY]` @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L17-L148]
...
- `[MODIFY]` @[backend_v2/tests/unit/test_epic93_contract_verification.py#L158-L313]
...
<execution_protocol>
  <step id="0" name="STRATEGIC ALIGNMENT CHECK">
    ...
    <action>Look forward: Verify if the current plan's assumptions still hold true in @[backend_v2/services/orchestrator/matrix_explanation_service.py#L15-L165], @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L17-L148], @[backend_v2/models/dtos/synthesis.py], @[backend_v2/worker.py#L910-L980], and test files.</action>
  ...
  <dod_checklist>
    - [ ] `MatrixExplanationService.assemble_matrices_to_explain` updated to mandatory `target_locale: str` without defaults.
    - [ ] Global settings hoisted at method start (`max_synthesis_quote_length`, `max_synthesis_quotes_per_matrix`, `max_synthesis_unmet_criteria_per_matrix`).
    - [ ] Legacy anti-patterns eliminated: duck-typing (`isinstance`, `hasattr`, `getattr`, `.get()`), raw `+` concatenation, and unhandled `LightweightMatrixOutput` / `LevelStatsDTO` validations.
    - [ ] Status-Aware Dual Reporting implemented (`SUPPORTING EVIDENCE` for `PASSED` atoms and `UNMET CRITERIA / DEFICITS` for `FAILED` atoms).
    - [ ] Ranked Round-Robin claim diversity implemented with per-matrix pre-deduplication hash set (`seen_matrix_quotes`), eliminating Single-Claim Starvation and Deduplication Starvation.
    - [ ] Unmet criteria curated deterministically by ascending scale score order (Level 1 critical deficits first, alphabetical tie-break), eliminating Priority Inversion.
    - [ ] Low-substance quote fragments (&lt; 15 characters) filtered from `SUPPORTING EVIDENCE`.
    - [ ] `MatrixExplanationContextList = TypeAdapter(list[MatrixExplanationContextDTO])` defined in @[backend_v2/models/dtos/synthesis.py].
    - [ ] Double-Serialization in @[backend_v2/worker.py] eliminated by replacing `json.dumps([m.model_dump(...) for m in ...])` with direct `MatrixExplanationContextList.dump_json(matrices_to_explain, indent=2, exclude_none=True).decode("utf-8")`.
    - [ ] Hardcoded `[:300]` replaced with `settings.max_synthesis_quote_length` in @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L17-L148].
    - [ ] @[ki_synthesis_payload_compression.md] updated with SSOT settings and round-robin curation.
    - [ ] All 5 existing test cases in @[backend_v2/tests/unit/services/orchestrator/test_matrix_explanation_service.py] updated with `target_locale="en"` and 6 new unit tests added.
    - [ ] Call-site in @[backend_v2/tests/unit/test_epic93_contract_verification.py#L257-L313] updated with mandatory `target_locale="en"`.
  </dod_checklist>
  ...
  <step id="1" name="Matrix Explanation Service Hardening &amp; Dual Reporting">
    <action>Modify @[backend_v2/services/orchestrator/matrix_explanation_service.py#L15-L165]:
1. Import `logging`, `logger = logging.getLogger(__name__)`, `ErrorCodes`, `settings` (global import), `ranked_round_robin_select`, `LevelStatsDTO`, `LightweightMatrixOutput`, `PromptBlockCategory`, and `ValidationError`.
...
3. Hoist settings at start: `max_quote_len = settings.max_synthesis_quote_length`, `max_quotes_per_matrix = settings.max_synthesis_quotes_per_matrix`, `max_unmet_criteria = settings.max_synthesis_unmet_criteria_per_matrix`.
...
5. Probe `LightweightMatrixOutput`: wrap `LightweightMatrixOutput.model_validate(payload_to_validate, strict=False)` in `try...except (ValidationError, ValueError)` (PROBE BOUNDARY with inline comment), log warning on failure and `continue`.
...
11. Guard `level_breakdown`: `if lw_matrix.level_breakdown:`, validate `LevelStatsDTO.model_validate(stats_raw, strict=False)` with try/except PROBE BOUNDARY, format via f-strings.
12. Assemble `justification_text` using deterministic sections (`[DISTRIBUTION CONTEXT: ...]`, `SUPPORTING EVIDENCE:\n...`, `UNMET CRITERIA / DEFICITS:\n...`, or fallback `No direct evidence quotes or specific deficits recorded for this matrix.`).
    </action>
    <contract_freeze>
  ...
  <step id="2" name="Synthesis Payload Compressor SSOT Alignment">
    <action>In @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L17-L148], replace hardcoded `[:300]` quote slice with `settings_obj.max_synthesis_quote_length`.</action>
...
3. At line 964 (Matrix Row Explanations prompt assembly), replace:
  ...
  <step id="5" name="Matrix Explanation Service Unit &amp; Contract Tests (Atomic Test Migration)">
    <action>Modify @[backend_v2/tests/unit/services/orchestrator/test_matrix_explanation_service.py]:
1. Update 5 existing test cases to pass `target_locale="en"` and assert `SUPPORTING EVIDENCE:` and `UNMET CRITERIA / DEFICITS:` headers.
2. Add 6 new unit tests:
   ...
   - `test_assemble_matrices_to_explain_multilingual_resolution`: verify `target_locale="fi"` resolves Finnish claim translations.
   - `test_assemble_matrices_to_explain_corrupt_level_stats_graceful_handling`: verify malformed level stats are logged and skipped without crashing.
    ...
    <action>Modify @[backend_v2/tests/unit/test_epic93_contract_verification.py#L257-L313] to pass mandatory `target_locale="en"` in `test_matrices_to_explain_assembly`.</action>
    <test_contracts>
      ...
      <test name="test_assemble_matrices_to_explain_short_quote_filtering" category="boundary">
        <input>atom with quote="yes"</input>
