...
- **Schema Convergence Mandate Compliance:** Do NOT create TheoryGroundingDTO — the existing `TheoryGrounding` model at `@[c:\src\quorum\backend_v2\models\v2_core.py#L187-L201]` already provides `source_url` and `citation_reference` with `ConfigDict(strict=True, extra="forbid")`. Reuse it directly.
...
<execution_block level="0_create_plan_validated">
  ...
  <step id="3" name="Phase 3: DTO Strictness &amp; Engine Metadata Wiring">
    <action>Start session via `/tier5-resume`.</action>
    <constraint invariant="schema_convergence_mandate">Do NOT create TheoryGroundingDTO — reuse the existing `TheoryGrounding` model from `@[c:\src\quorum\backend_v2\models\v2_core.py#L187-L201]`. Do NOT create MatrixClaimRuleDTO — per-claim metadata already flows through the existing `FlattenedAtom` model from `@[c:\src\quorum\backend_v2\models\dtos\engine.py#L21-L38]`. One Concept = One Schema.</constraint>
