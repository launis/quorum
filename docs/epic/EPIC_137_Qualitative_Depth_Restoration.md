...
- **Schema Convergence Mandate Compliance:** Do NOT create TheoryGroundingDTO — the existing `TheoryGrounding` model at `@[c:\src\quorum\backend_v2\models\v2_core.py#L187-L198]` already provides `source_url` and `citation_reference` with `ConfigDict(strict=True, extra="forbid")`. Reuse it directly.
...
#### [MODIFY] @[backend_v2/tests/integration/test_prompt_compiler.py]
