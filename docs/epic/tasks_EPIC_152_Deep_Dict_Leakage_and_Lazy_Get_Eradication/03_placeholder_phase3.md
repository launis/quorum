...
| `@[backend_v2/models/dtos/base.py#L47-L58]` &amp; `@[backend_v2/models/dtos/hook_state.py#L21-L44]` | Loose dictionary event passing, speculative `events/` folder creation, and missing `StepOutputDTO` in `DomainInputValue` | Lock `DataStarvationEvent` in `base.py` under `ConfigDict(strict=True, frozen=True, extra="forbid")`; encapsulate `StepOutputDTO | list[StepOutputDTO]` in `DomainInputValue` and `ExecutionInputsDTO.dynamic_inputs` | Prune speculative `backend_v2/events/` directory creation and backward compatibility re-export shims | Unit test verifying Pydantic V2 validation rejection of unauthorized keys with `extra="forbid"` and acceptance of `StepOutputDTO` |
