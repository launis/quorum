...
## Targets (Modify)
- `c:\src\quorum\scripts\migrate_seed_data.py` [NEW]
...
- **Rule 28**: Do not use direct SDK calls. The ETL script MUST use `LLMTaskExecutor.execute_structured_task()` to parse old string data into new schemas.
- **Rule 25 & 26 (Opaque Stripe ID Mandate)**: No md5 hashing. All missing IDs must be generated as `tda_[a-f0-9]{32}` matching the `v2_core.py` pattern.
- **Rule 65 (PEP 750 t-strings)**: The ETL prompt template MUST use t-strings, not f-strings.

## Implementation Steps
1. Write a standalone Python script in `scripts\` that loads `seed_data.json` and navigates to `prompt_blocks -> scales -> claims -> tda_assertions`.
2. Convert the 186 `ai_rule_description` strings into the new structured JSON representation:
   - `concept_description`
   - `acceptance_criteria`: For complex rules, explicitly inject permission to use `contextual_override=true` if concepts cross sentence boundaries.
   - `anti_patterns`: Convert strict "immediate failures" into conditional failures (allow pass if `contextual_override` is justified).
   - `contrastive_example`
   - `syntactic_anchors`
...
3. The script must use `LLMTaskExecutor` for the conversion, applying the Opaque Stripe ID mandate for missing `tda_id` fields.
   - Validate every generated block via `PromptBlock.model_validate()` and `I18nText.validate_i18n()` to ensure both `en` and `fi` translations exist before saving.
...
`/tier2-execute docs/epic/tasks_bilingual_schema_refactor/phase3_offline_migration.md`
