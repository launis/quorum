...
<execution_protocol>
  ...
  <step id="2_1" name="Update Target OutputProfile in Seed Data">
    <action>Locate the primary OutputProfile (e.g., 'holistic_audit') in `seed_data.json`.</action>
  ...
  <step id="2_2" name="Configure layouts Array">
    ...
    <action>Ensure the `3d_matrix` preset block correctly defines `matrix_visible_columns`, `extension_labels`, and `matrix_column_labels`.</action>
