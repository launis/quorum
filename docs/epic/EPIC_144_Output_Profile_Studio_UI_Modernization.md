...
During codebase verification, the following 12 existing Fail-Fast violations were identified in the files being refactored. These MUST be eradicated as part of the decomposition phases (Phase 0, Phase 1, and Phase 2), NOT deferred:
...
**Block Type Enum SSOT:** `TargetBlockType` in `@[backend_v2/models/enums.py]`
**Block Order SSOT:** `OutputProfile.target_block_order` in `@[backend_v2/models/v2_core.py]`
...
| Layer | Location | Detail |
| :--- | :--- | :--- |
| **DB Config** | `OutputLayoutBlock.preset_view: Literal["1d_metrics", "2d_compare", "3d_matrix", "default", "text_only", "matrix_summary"]`, `OutputLayoutBlock.steps: list[str]` | `@[backend_v2/models/v2_core.py]` — Defines the visual preset and which workflow step IDs provide the axes. |
...
1. **SSOT Enum Parity & Centralized Constraints Creation:**
   ...
   - Update `@[client_app_v2/lib/features/studio/models/output_profile.dart]` to use `DisplayScale displayScale` and eradicate all `unknownEnumValue` parameters (`PresetView.defaultView`, `TextDeliveryMode.full`, `HistoricalContextMode.disabled`, `PresetView.metrics1d`).
2. **Pre-Flight Database Migration & Reseed Gate:**
   - Implement an idempotent migration script `backend_v2/database/migrations/purge_legacy_output_profile_keys.py` that iterates persistent storage (`output_profiles` collection in TinyDB / Firestore) and strips any residual `"include_diagnostic_scorecard"` keys before Python schema removal.
   ...
   - Execute local environment database re-seeding via `uv run python backend_v2/seed/run_seed.py local`.
...
1. **Universal Baseline (The Block Visibility Toggle):**
   ...
   - **SSOT Mechanism:** The toggle MUST map exclusively to adding/removing the block's `TargetBlockType` string from `OutputProfile.target_block_order: list[str]` (`@[backend_v2/models/v2_core.py]`). No separate `include_X: bool` fields are permitted. This ensures a single, universal visibility mechanism for ALL blocks.
...
5. **AI Extensions Block Editor (`client_app_v2/lib/features/studio/views/widgets/profile/blocks/xai_extensions_block_card.dart`):**
   - Uses interactive `FilterChip` pills. Slider for `max_extension_items` MUST be clamped to `SystemUiConstraints` to prevent framework assertion crashes.
   - **Client-Side Boundary Validation:** The numerical text input override MUST include a `TextFormField` validator ensuring the value is `>= 1` and `<= 100`. This guarantees local Fail-Fast before the API network call.
...
1. **Backend Fail-Fast & Negative Unit Tests (`@[ki_ai_testing_standards.md]`):**
   ...
   - Execute negative tests in `backend_v2/tests/unit/models/dtos/test_output_profile_dto.py` and `@[backend_v2/tests/unit/test_output_profile_models.py]` verifying that `pydantic.ValidationError` is raised on out-of-bounds `max_extension_items` (`ge=1, le=100`, specifically testing values `0`, `-1`, and `101`), invalid `display_scale` strings, and legacy `include_diagnostic_scorecard` keys under `ConfigDict(strict=True, extra="forbid")`.
...
- [ ] Flutter `client_app_v2/lib/features/studio/views/widgets/profile/blocks/xai_extensions_block_card.dart` safely clamps slider display values to prevent framework assertion crashes on out-of-bounds database values.
...
- [ ] Idempotent pre-flight migration script `backend_v2/database/migrations/purge_legacy_output_profile_keys.py` is implemented and verified to purge legacy `include_diagnostic_scorecard` keys from persistent TinyDB/Firestore storage before schema removal.
...
| `idempotent_pre_flight_migration` | An idempotent migration script `backend_v2/database/migrations/purge_legacy_output_profile_keys.py` MUST be executed to purge `"include_diagnostic_scorecard"` from persistent TinyDB / Firestore documents prior to removing the field from Python models. |
