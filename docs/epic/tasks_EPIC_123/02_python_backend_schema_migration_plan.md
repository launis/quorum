...
<execution_protocol>
  <step id="1" name="Update Enums and Literals">
    ...
    <action>In `@[c:\src\quorum\backend_v2\models\enums.py]`, create a new `UiVariant(StrEnum)` enum with values (`default`, `success`, `warning`, `error`, `neutral`), and create a `LaxUiVariant = Annotated[UiVariant, Field(strict=False)]` alias.</action>
  ...
  <step id="4" name="Delete GlobalSynthesisDTO and Migrate Views">
    ...
    <action>Refactor `ReportView.status_theme` from `StrictStr` to `VisualIntent`.</action>
    <action>Refactor `AssessmentView.uiVariant` to use `LaxUiVariant`.</action>
    <demolish>REMOVE: `GlobalSynthesisDTO` class and `global_synthesis` field.</demolish>
  </step>

  <step id="5" name="Fix SDUI List[Any] Leaks">
    <action>In `@[c:\src\quorum\backend_v2\models\view\sdui.py]`, update `SduiGridBlock.items` to strictly `list[str]`.</action>
  ...
  <step id="6" name="Update Test Fixtures and Parity Checks">
    <action>In `@[c:\src\quorum\backend_v2\tests\test_data\report_data_dto_fixture.json]` and `@[c:\src\quorum\backend_v2\tests\test_data\exe_c0bc_inputs.json]`, remove legacy flat fields and update to match the new `inner_sdui_blocks` schema.</action>
    <action>In `@[c:\src\quorum\backend_v2\tests\unit\models\test_enum_parity.py]`, add a new `test_parity_ui_variant()` check to assert exact parity between Python `UiVariant` and Dart `UiVariant`.</action>
  </step>
