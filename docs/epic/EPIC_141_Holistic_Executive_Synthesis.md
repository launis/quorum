...
- **Target File:** @[backend_v2/services/orchestrator/dag_executor.py#L558-L750]
...
- **Target File:** @[backend_v2/services/orchestrator/synthesis_distiller.py#L30-L113]
- **Action:** Modify `_compress_synthesis_payload()` (@[backend_v2/services/orchestrator/synthesis_distiller.py#L30-L113]).
- **Remove:** The blind `[:20]` cutoff at line 98.
- **Replace With:** A configurable limit sourced from the application configuration SSOT (@[backend_v2/settings.py], specifically a [NEW] field `max_synthesis_evaluations: Annotated[int, Field(description="Maximum evaluations forwarded to synthesis LLM")]` with a default value of 60, respecting the `strict_configuration_segregation` mandate. **Architectural Rationale:** This limit protects the LLM token budget from exploding. Because token budgets are environment-dependent (specifically: Local models vs. Cloud Vertex AI limits), this value MUST NOT be stored in the database. Storing infrastructure limits in the database violates the database ban for environment variables). This preserves token budget control while removing the arbitrary blindness.
- **Enrich `lite_ev` -> DistilledEvaluationDTO:** The current `lite_ev` dict (lines 89-96) contains ONLY `atom_id`, `exact_quotes`, `semantic_reasoning` (truncated), and `extensions`. This violates the `no_naked_dicts_in_state` mandate. This Epic mandates replacing the naked dict with a new DistilledEvaluationDTO (@[backend_v2/models/domain/synthesis.py], with `ConfigDict(strict=True, extra='forbid')`) and adding/fixing the following keys to restore Epic 92 functionality:
...
- **Cross-Reference Mechanism:** The `_compress_synthesis_payload()` function operates on serialized `StepOutputDTO.payload` dicts. These evaluation payloads contain `atom_id` (or `tda_id`) but NOT `resolved_claim` or `depends_on`, which live in the extraction-phase `LinkedAtomGraph` nodes. To bridge this gap, the caller (`synthesis_distiller_hook`) MUST pre-build a global lookup map (`tda_id_to_atom_context: dict[str, LinkedAtomGraph]`) by iterating over all `available_dtos` with extraction-type payloads (containing `results` arrays with `ExtractedAtom`-shaped items) BEFORE calling `_compress_synthesis_payload()`. This map must then be passed as a parameter to `_compress_synthesis_payload()`, which uses it to enrich each DistilledEvaluationDTO via the `atom_id` join key. **CRITICAL RULE:** You MUST perform this join using strict dictionary access (specifically: `tda_id_to_atom_context[atom_id]`) to enforce deterministic Fail-Fast KeyError crashes if an ID is missing. The use of `.get()` or `try/except` fallback blocks is strictly prohibited per the Zero Compromise Pledge.
...
- **Target File:** @[backend_v2/services/orchestrator/prompts/matrix_sensor_prompt_builder.py#L89-L157]
...
- **ATOMIC DATA FIXTURE MIGRATION:** Because Phase 2 alters the structure of `lite_ev`, you MUST simultaneously update the mocked test fixtures in @[backend_v2/tests/unit/test_bug_synthesis_hook.py] and @[backend_v2/tests/unit/test_epic93_contract_verification.py] in the exact same phase, otherwise the test suite will crash continuously in a failure loop.
...
- Pydantic V2 Strict DTO Schemas
