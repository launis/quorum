...
* **Architecture**:
  ...
  * **Guard Domain Model Sunset Plan**: The following artifacts MUST be explicitly refactored or deleted:
    * `backend_v2/models/domain/guard.py` — DELETE. `GuardInput`, `TaintedDataContent`, `GuardDTO`, `GuardOutput`, and `SanitizationResult` are intentionally dropped (`SanitizationResultDTO` already exists). `SecurityCheck` is migrated to `backend_v2/models/domain/security.py` and nested in `InputProcessingOutputDTO`.
    * `backend_v2/models/domain/scoring.py` (`StepGuardDTO`) — REFACTOR to read security data from the unified Input Processing output state instead of a separate `step_guard` key. Remove any `TaintedDataContent` references.
    ...
    * `backend_v2/tests/unit/models/domain/test_guard.py` and `backend_v2/tests/unit/test_guard_models.py` — DELETE.
    * `backend_v2/models/domain/__init__.py` — PURGE all `Guard` and `SanitizationResult` exports.
...
* **Enum Definition Required** (Tripartite Architecture — `backend_v2/models/enums.py`):
  ...
  * `LaxSearchStatus = Annotated[SearchStatus, Field(strict=False)]` MUST be added to the lax type aliases block for Pydantic boundary parsing.
* **Architecture**:
  * **Step 3.1**: The LLM evaluates flattened atoms and returns a `BatchSearchQueryDTO` containing a list of required Tavily queries. In strict alignment with LLM Architecture constraints, this MUST be invoked via `LLMTaskExecutor.execute_structured_task()` to guarantee Fail-Fast Native Structured Outputs validation without regex/fallback parsing. The `BatchSearchQueryDTO` and `TavilySearchResultDTO` MUST enforce `model_config = ConfigDict(strict=True, extra='forbid', frozen=True)`. If the document is massive, atoms must be processed in **Chunks** to prevent LLM context explosion.
  ...
  * **Step 3.2**: The Backend natively executes all unique Tavily queries concurrently using `asyncio.TaskGroup`.
    ...
    * **Transient Error & DLQ Routing Mandate (RFC7807 & KI)**: To prevent a single Tavily network timeout or 502 error from crashing the entire `TaskGroup` (which would instantly orphan all parallel searches), individual workers MUST trap transient errors using Tenacity with exception whitelisting. You MUST classify the error type first. Only retry on transient network anomalies. Structural validation failures (`ValidationError`) must instantly break the retry loop. If retries are exhausted, the worker MUST append a strictly typed Pydantic Dead Letter Queue (DLQ) state (e.g., a `TavilySearchResultDTO` with `status: LaxSearchStatus = SearchStatus.DLQ_TIMEOUT`) to the shared result list, accompanied by an RFC7807 structured `logger.error`, rather than bubbling up the exception to the `TaskGroup`. The use of raw untyped dictionaries (e.g., `{"_dlq_status": "FAILED"}`) for state transit is strictly prohibited.
  * **Step 3.3**: The aggregated search results are injected into a final synthesis prompt to generate the Fact Checker's definitive Output Matrix.
    * **DLQ Anti-Hallucination Directive (De-Generator Mandate)**: The synthesis `PromptBlock` MUST explicitly instruct the LLM on how to handle DLQ states. Following the De-Generator Mandate, you MUST NOT use XML tags inside the JSON string for this instruction. Add a Markdown header (e.g., `DLQ TIMEOUT PROTOCOL:`) and instruct explicitly: "If SearchStatus is DLQ_TIMEOUT, you MUST NOT invent or hallucinate the fact. You must explicitly state that the validation timed out." Without this, the LLM will fall back on latent knowledge to fill the void.
...
* **Adapter Delegation Mandate**: Rather than adding caching methods directly to `LLMTaskExecutor`, all caching lifecycle manipulation MUST occur exclusively via `LLMCachingService`. Creating a parallel caching system or bypassing this service boundary is strictly prohibited per the `provider_abstraction_mandate` and the Global Document Cache KI.
* **Architecture**:
  * **Composite Cache Segmentation (Model + Inputs)**: Because the UI allows individual DAG roles to override their LLM `Model Strategy` AND dynamically select which input documents they receive, a single monolithic cache is structurally impossible and logically dangerous (passing a 2MB document to a role that only requested a 10KB chat log violates UI mapping isolation and increases hallucination risk).
    * Before the concurrent `asyncio.TaskGroup` (Floodgate) opens, the DAG Orchestrator (`EnrichedDagExecutor.execute_graph` / `tda_engine.py`) MUST calculate a composite cache signature for each active role: `(physical_model_name, sorted_tuple_of_input_keys)`.
    * The Orchestrator MUST pre-warm the cache by grouping roles by this signature and uploading the static source text exactly ONCE **per unique signature** via `LLMCachingService.pre_cache_document()`. 
  ...
  * **Dynamic Injection & Prefix Survival Mandate**: `LLMCachingService` must transparently inject the active `cache_id` (keyed by `workflow_run_id` or `execution_id`) into `LLMClient` payloads dynamically. Do NOT manually pass `cache_id` arguments down through `LLMTaskExecutor` and `LLMClient`. **CRITICAL RULE**: To guarantee Context Cache Prefix Hash survival, the individual Role instructions MUST be appended AFTER the cached payload (e.g., explicitly injected as `user` messages at the absolute end of the sequence).
  * **FinOps Explicit Cleanup Mandate (CRITICAL BUG FIX)**: `LLMTaskExecutor.execute_structured_task` currently holds a `finally: LLMCachingService.teardown_workflow_caches()` block. This is a catastrophic bug. Because `LLMTaskExecutor` is invoked inside parallel `asyncio.TaskGroup` loops for atom evaluation, the *very first* atom to finish immediately deletes the global cache, instantly crashing the remaining parallel atoms with 404 Cache Expired errors.
  * **Race Condition Guard (Orchestrator-Level Lifecycle)**: The `finally` teardown block MUST be removed from `LLMTaskExecutor`. The `try...finally` block that guarantees cache execution teardown MUST be placed strictly at the Orchestrator level (`EnrichedDagExecutor` or `tda_engine.py`), entirely wrapping the `asyncio.TaskGroup`. The global context cache must outlive all parallel executions.
  * **Cache Expiration Resiliency (TTL Guard)**: Context Caches physically expire on the LLM provider side (e.g., after 15-60 minutes). Because the Floodgate contains complex external network tasks paired with Tenacity backoffs, it is highly probable that some parallel workers will outlive the Context Cache's TTL.
    * The Exception Whitelist used by the LiteLLM Router / Tenacity MUST explicitly classify cache-related errors (e.g., 404 Context Not Found, Cache Expired) as transient exceptions.
    * If a cache expires mid-flight, the specific failed worker MUST dynamically fall back to sending the full physical payload (cache-miss fallback) for that specific retry to ensure the entire execution run is not catastrophically aborted.
