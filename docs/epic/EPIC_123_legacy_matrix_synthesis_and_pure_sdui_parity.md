...
### Business Objective
Restore the rich synthesis styling and XAI output extensions (such as "Arjen Vinkki", "Vasta-argumentti", Jargon Ratio) that existed prior to Epic 110/122, ensuring complete visual parity between the interactive Flutter UI and the generated PDF report.
...
- **Enum Sync**: Add `error` to Dart `VisualIntent`. Create a NEW `@JsonEnum() enum AlertSeverity` with values `info`, `warning`, `criticalOverride`, `success`, `error` and sync `SduiAlertBoxBlock.severity` to it. Create a NEW `@JsonEnum() enum UiVariant` for `AssessmentView.uiVariant` to match Python. Refactor `ReportView.statusTheme` to `VisualIntent`. Sync `SduiGridBlock.items` and `SduiQuoteCard.citations` types.
- **Legacy Comment Cleanup**: Remove `// Epic 6:` and `// Epic 88` comments from `matrix_scorecard_dto.dart`.
- **Test Sync (Dart)**: Update Dart mock test fixtures (specifically all golden fixture files within `client_app_v2/test/features/execution/goldens/`) to remove legacy flat fields and match the new `innerSduiBlocks` schema. This MUST be done here so the tests don't crash when running the audit loop.
...
- **Database Source (`extension_labels`)**: Because `AlertBlock.text` expects a `str`, `blueprint.py` MUST resolve the `I18nText` object from `extension_labels` using the current Execution's `target_language` before injecting it into the SDUI block (for example: `label = ext_labels[type].get_translation(execution.target_language)`). The backend MUST ALWAYS provide the fully resolved string — the frontend NEVER performs translation lookups on SDUI block content.
- **Prompt Directory Features (`models/prompts`)**: If any textual prefixes (such as "Jargon Ratio:") are injected via static prompt configurations, the backend MUST utilize the localized properties matching the `target_language` rather than hardcoding.
