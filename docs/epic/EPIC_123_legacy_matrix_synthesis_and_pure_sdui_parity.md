...
- **Backend Duck-Typing Risk in SDUI Mapper (Juurisyy 8):** When extracting legacy flat fields (e.g., `coaching`) to generate `AlertBlock` models, using defensive programming like `getattr(row, "coaching", None)` or `.get("coaching")` circumvents static typing. If the underlying schema changes or a field is removed, duck-typing causes a silent failure (returning `None` without crashing), leading to missing SDUI blocks in production.
...
- **`GlobalSynthesisDTO` Migration**: Refactor `blueprint.py` to stop generating `GlobalSynthesisDTO`. Instead, map the `executive_summary` and `user_role` directly into standard SDUI blocks (e.g., `MarkdownBlock`) and inject them into a `UiSection` in the `ReportView.sections` array.
