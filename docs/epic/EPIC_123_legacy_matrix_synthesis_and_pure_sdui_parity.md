...
- **Epic 120 Leftovers (`list[Any]` Leaks) (Juurisyy 12):** Epic 120 Phase 3 mandated the eradication of `list[Any]` from all SDUI models. However, `SduiGridBlock.items` and `SduiQuoteCard.citations` were skipped and remain loosely typed. These must be explicitly typed to enforce full SDUI strictness. To adhere to the Dumb Painter architecture and avoid Dart's `List<dynamic>` vulnerabilities, `SduiGridBlock.items` MUST be strictly typed as `list[str]` (Python) and `List<String>` (Dart). The backend must coerce all values (numbers, etc.) to formatted strings before serialization. `SduiQuoteCard.citations` MUST be typed as `list[int]`.
...
- **EPIC 120 Leak Cleanup**: Fix the remaining `list[Any]` type leaks in `sdui.py`. Update `SduiGridBlock.items` to strictly `list[str]` (enforcing Dumb Painter string coercion prior to SDUI serialization) and `SduiQuoteCard.citations` to `list[int]`.
