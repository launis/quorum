...
- Execute `uv run python scripts/backend_audit_loop.py` to ensure schema integrity and routing.
- Execute `uv run python scripts/flutter_audit_loop.py --build` to synchronize Freezed DTOs and verify widget compilation.
- **COMPLETE `grouped_extensions` Eradication Verification**: Confirm that the `grouped_extensions` field has been DELETED from `ReportDataDTO` (Python) and its Flutter equivalent. Verify with `grep_search` that ZERO references to `grouped_extensions` or `groupedExtensions` remain in `@[c:\src\quorum\backend_v2\services\blueprint.py]`, `@[c:\src\quorum\backend_v2\services\sdui_mapper_service.py]`, `@[c:\src\quorum\backend_v2\templates\report_template.jinja2]`, and the entire `@[c:\src\quorum\client_app_v2\lib]` directory. Any remaining reference is a BLOCKING failure.
- **BREAKING CHANGE - Database Wipe & Seed**: Since the removal of `grouped_extensions` and `global_synthesis` is a breaking schema change (due to strict `extra='forbid'` parsing), all historical executions in the local database will crash on load. You MUST wipe the local database and re-seed before performing E2E tests:
  ...
  ```
- **Testing & Quality Gate**: Execute E2E Live REST API verification gate (`$env:RUN_LIVE_E2E="true"; uv run pytest backend_v2/tests/integration/test_integration_real_llm.py`).
- **SDUI Semantic Parity**: Verify SDUI Semantic Parity (Epic 123 Core Mandate) by running the E2E Golden Fixture pipeline as per KI guidelines (generating random Polyfactory fixtures and passing to Flutter via `--dart-define=GOLDEN_PATH=...`). Generate a holistic audit report to verify visual parity between Flutter and PDF.
...
- **Database Source (`extension_labels`)**: Because `AlertBlock.text` expects a `str`, `blueprint.py` MUST resolve the `I18nText` object from `extension_labels` using the current Execution's `target_language` before injecting it into the SDUI block (for illustrative purposes only: `label_obj = ext_labels.get(XaiExtensionType(type)); label_str = label_obj.resolve(execution.target_language)`). The backend MUST ALWAYS provide the fully resolved string — the frontend NEVER performs translation lookups on SDUI block content. Furthermore, you MUST explicitly identify and remove any string manipulation fallback (specifically `ext_key.replace("_", " ").title()`) which violates the `strict_enum_l10n_mapping` rule.
