...
- **Action 1**: Remove the restrictive passing-only check inside `_assemble_matrices_to_explain`. The extraction must iterate over ALL atoms inside `atoms.items()` (both PASSED and FAILED). Additionally, per `the_no_legacy_mandate`, you MUST completely delete the legacy `elif isinstance(atoms, list):` fallback branch (which handled deprecated `MatrixScorecardRowDTO` arrays), enforcing strict dictionary payloads. You MUST programmatically bypass N/A items using exactly: `if atom_status == ExecutionStatus.N_A: continue`.
- **Action 2**: Enforce structured status tagging via Strict Pydantic DTOs. Building LLM prompts via raw string concatenation is strictly forbidden (to protect Context Caching parity). However, you MUST NOT dynamically instantiate `PromptBlock` objects in the service layer, as this violates the `<prompt_asset_ssot_mandate>` and causes a "Lost in the Middle" token flood during the holistic XAI Synthesis phase (which cannot be chunked). Instead, you MUST extract the failed claims and quotes, and rigorously deduplicate them to compress the token budget. **God Code Prevention**: Because `synthesis_distiller.py` is already 458 lines (violating the 200-line hard architectural smell threshold), you MUST NOT write this complex extraction and deduplication logic inside `synthesis_distiller.py`. You MUST create a new, dedicated file (specifically: `backend_v2/services/orchestrator/matrix_context_extractor.py`) to hold this pure, testable extraction logic. Note: The `global_quotes_map` building logic must ALSO be moved into this new extractor module to prevent orphaned extraction logic. Inject the result cleanly into a Strict Pydantic DTO (specifically: [NEW] MatrixExplanationContextDTO located in @[backend_v2/models/dtos/synthesis.py]) rather than a mutable state dictionary, adhering to the Tripartite Pipeline Architecture event-driven data envelopes mandate. The `PromptCompiler` will automatically handle the strict XML serialization dynamically at the end of the payload.
- **Action 3 (Downstream Integration)**: Update @[backend_v2/worker.py] (specifically lines extracting `matrices_to_explain`). It currently expects raw dictionaries. You MUST update it to cleanly deserialize using `MatrixExplanationContextDTO.model_validate(obj, strict=True)` to ensure the XAI prompt continues to receive the expected context structure without crashing on Pydantic objects, strictly forbidding raw dict state passing.
- **Action 4 (Tests)**: Update @[backend_v2/tests/unit/services/orchestrator/test_synthesis_distiller.py] and downstream SDUI Parity / E2E assertions to expect the new realistic matrix scores and verify that failed context is correctly forwarded.
