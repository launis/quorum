...
- **Action 1a**: Modify `LightweightMatrixOutput.evaluated_atoms` (in `lightweight_matrix.py`, line 58) from `dict[str, bool | str]` to `dict[str, LaxExecutionStatus]`. *(Architectural Note: Pydantic V2 will automatically coerce valid string inputs, specifically the string `"FAILED"`, into the `ExecutionStatus.FAILED` enum via the `LaxExecutionStatus` alias, guaranteeing structural purity without breaking strict Enum hydration rules.)*
...
- **Action 2 (Compliance Enforcement)**: Ensure that no `@field_validator(mode="before")` or fallback logic is added to handle legacy raw `True`/`False` values. According to the `zero_legacy_fallback_hacks` mandate, models must remain mathematically pure. If legacy data crashes the validation, it must be fixed at the source (specifically: in Phase 1.5 for the producer, and in Phase 4 for test fixtures) rather than polluting the domain layer with duct-tape parsing logic.

### Phase 1.5: Producer Contract Fix (scoring.py & execution.py)
- **Target Files**:
  - @[backend_v2/hooks/scoring.py]
  - @[backend_v2/services/execution.py]
- **Justification**: Phase 1 locks the DTO boundary to `LaxExecutionStatus`, but the upstream PRODUCERS currently emit raw `True`/`False` booleans and `.value` strings. If the DTO boundary is enforced without fixing the producers, ALL real executions will crash at the Pydantic validation boundary. This phase MUST be completed BEFORE Phase 2.
...
- **Action 3**: Enforce structured status tagging via Strict Pydantic DTOs. Extract both passed and failed claims/quotes, and deduplicate them to compress the token budget. Tag them appropriately so the downstream LLM understands which claims were validated and which failed. Inject the result cleanly into a Strict Pydantic DTO (specifically: [NEW] MatrixExplanationContextDTO located in @[backend_v2/models/dtos/synthesis.py]) rather than a mutable state dictionary, adhering to the Tripartite Pipeline Architecture event-driven data envelopes mandate.
- **Action 4 (Downstream Integration)**: Update @[backend_v2/worker.py] (specifically line 789 extracting `matrices_to_explain`). It currently expects raw dictionaries and directly serializes them with `json.dumps()`. Because Action 3 now enforces that the service returns native MatrixExplanationContextDTO objects, `worker.py` MUST NOT call `model_validate` (which would constitute Naked Dict leakage/Double-Serialization). Instead, `worker.py` must assume they are already native objects and simply call `[obj.model_dump(exclude_none=True) for obj in matrices_to_explain]` before feeding the list to `json.dumps()` for the LLM prompt.
