...
20. **System-Wide Emoji Contamination & Presentation Pollution in Execution Pipeline:** In `@[backend_v2/hooks/scoring/matrix_hook.py]` (Lines 432, 444, 446, 448), `@[backend_v2/services/llm_task_executor.py]` (Line 258), `@[backend_v2/templates/report_template.jinja2]` (Lines 131, 134, 303), `@[backend_v2/templates/dashboard_pdf.html]` (Lines 183, 192), and Flutter localization files `@[client_app_v2/lib/l10n/app_fi.arb]` (Lines 803–818) and `@[client_app_v2/lib/l10n/app_en.arb]` (Lines 1127–1170), emojis (specifically and exhaustively: `📍`, `💡`, `⚠️`, `🛠️`, `💬`, `⚖️`, `🔍`, `🎭`, `📚` and their Unicode escape sequences `\U0001f4cd`, `\U0001f4a1`, `\u26a0\ufe0f`, `\U0001f6e0\ufe0f`, `\U0001f50d`, `\U0001f3ad`, `\U0001f4da`) are hardcoded into analytical payloads, log statements, PDF templates, and all 10 telemetry title keys (`reportQuoteTitle`, `reportSemanticExplanationTitle`, `reportFrameworkReference`, `reportCoachingTitle`, `reportFalsificationTitle`, `reportMissingContextTitle`, `reportRiskFlagTitle`, `reportRemediationStepsTitle`, `reportEmotionalSentimentTitle`, `reportTheoryLinkTitle`). Emojis cause PDF rendering glyph failures (tofu boxes), render inconsistently across OS platforms, break external B2B JSON parsing, and violate enterprise design standards. Rather than merely relocating emojis to SDUI adapters, emojis MUST be completely eradicated across all backend, execution, hook, SDUI adapter, logging, template, and client localization layers, binding UI headers to native Flutter Material icons.
...
| `@[backend_v2/utils/math_utils.py]` | `curr.__dict__[part]` on BaseModel (Line 210) | REMOVE | Direct attribute traversal via `object.__getattribute__(curr, part)` to comply with AST guardrail QGR001 with zero `__dict__` reflection |
| `@[client_app_v2/lib/core/api/execution_client.dart]` | `(Epic 91 Phase 4)` comment on Line 64 | REMOVE | Objective English docstring complying with `internal_language_and_epic_ban` |
...
| `@[client_app_v2/lib/l10n/app_en.arb]` & `@[client_app_v2/lib/l10n/app_fi.arb]` | Hardcoded emojis (`💬`, `💡`, `⚖️`, `🔍`, `⚠️`, `🛠️`, `🎭`, `📚`) in 10 telemetry localization keys (`reportQuoteTitle`, `reportSemanticExplanationTitle`, `reportFrameworkReference`, `reportCoachingTitle`, `reportFalsificationTitle`, `reportMissingContextTitle`, `reportRiskFlagTitle`, `reportRemediationStepsTitle`, `reportEmotionalSentimentTitle`, `reportTheoryLinkTitle`) | REMOVE | Clean semantic text strings; bind `@[client_app_v2/lib/features/execution/views/widgets/xai_axis_telemetry_grid.dart]` to native Flutter Material icons (`Icons.format_quote`, `Icons.lightbulb_outline`, `Icons.gavel`, `Icons.build`, `Icons.warning`, `Icons.search`, `Icons.psychology`, `Icons.menu_book`) |
...
| `@[client_app_v2/lib/core/api/execution_client.dart]` | `Map<String, dynamic>` API returns and `(Epic 91 Phase 4)` comment | `Future<ReportDataDto>` / `Future<GenericStatusResponseDto>` typed returns and compliant English docstring | Eliminate untyped Map and banned "Epic" nomenclature | `flutter_audit_loop.py` passing clean |
...
2. **Pre-Implementation Technical Debt Cleanups (Scoped Boy Scout Rule):**
   ...
   - Eradicate `curr.__dict__[part]` on BaseModel in `@[backend_v2/utils/math_utils.py]` (Line 210) in favor of `object.__getattribute__(curr, part)` to satisfy AST rule QGR001 with zero `__dict__` access.
   - Eradicate `(Epic 91 Phase 4)` comment in `@[client_app_v2/lib/core/api/execution_client.dart]` (Line 64) in compliance with `internal_language_and_epic_ban`.
...
8. **Universal Emoji Eradication in SDUI Adapters, Jinja2, PDF Templates & Flutter ARB Telemetry:**
   ...
   - Modify `@[client_app_v2/lib/l10n/app_fi.arb]` (Lines 803–818) & `@[client_app_v2/lib/l10n/app_en.arb]` (Lines 1127–1170):
     - Eradicate emojis (`💬`, `💡`, `⚖️`, `🔍`, `⚠️`, `🛠️`, `🎭`, `📚`) from all 10 telemetry and evidence localization keys:
       ...
       - `reportSemanticExplanationTitle`: replace `"💡 Tekoälyn semanttinen perustelu (Kontekstuaalinen ohitus):\n{reasoning}"` / `"💡 AI Semantic Explanation (Contextual Override):\n{reasoning}"` with `"Tekoälyn semanttinen perustelu (Kontekstuaalinen ohitus):\n{reasoning}"` / `"AI Semantic Explanation (Contextual Override):\n{reasoning}"`.
       - `reportFrameworkReference`: replace `"⚖️ Viitekehys: {source}"` / `"⚖️ Reference framework: {source}"` with `"Viitekehys: {source}"` / `"Reference framework: {source}"`.
       - `reportCoachingTitle`: replace `"💡 Arjen Vinkki"` / `"💡 Coaching Tip"` with `"Arjen Vinkki"` / `"Coaching Tip"`.
       - `reportFalsificationTitle`: replace `"⚖️ Vasta-argumentti"` / `"⚖️ Devil's Advocate"` with `"Vasta-argumentti"` / `"Devil's Advocate"`.
       - `reportMissingContextTitle`: replace `"🔍 Puuttuva Konteksti"` / `"🔍 Missing Context"` with `"Puuttuva Konteksti"` / `"Missing Context"`.
       - `reportRiskFlagTitle`: replace `"⚠️ Vakava Riski Havaittu"` / `"⚠️ High Risk Identified"` with `"Vakava Riski Havaittu"` / `"High Risk Identified"`.
       - `reportRemediationStepsTitle`: replace `"🛠️ Korjaavat Toimenpiteet"` / `"🛠️ Remediation Steps"` with `"Korjaavat Toimenpiteet"` / `"Remediation Steps"`.
       - `reportEmotionalSentimentTitle`: replace `"🎭 Emotionaalinen Sävy"` / `"🎭 Sentiment Analysis"` with `"Emotionaalinen Sävy"` / `"Sentiment Analysis"`.
       - `reportTheoryLinkTitle`: replace `"📚 Teoreettinen Yhteys"` / `"📚 Theoretical Connection"` with `"Teoreettinen Yhteys"` / `"Theoretical Connection"`.
   - Modify `@[client_app_v2/lib/features/execution/views/widgets/xai_axis_telemetry_grid.dart]`:
     - Bind telemetry section headers directly to native Flutter Material icons (`Icons.format_quote`, `Icons.lightbulb_outline`, `Icons.gavel`, `Icons.build`, `Icons.warning`, `Icons.search`, `Icons.psychology`, `Icons.menu_book`) instead of relying on emoji characters embedded in localization strings.
...
- [ ] Exactly 0 emoji characters or emoji Unicode escapes (`\U0001f...`, `\u26a0...`) exist across backend source code, execution hooks, result projectors, SDUI adapters, loggers, Jinja2/HTML templates, and all 10 Flutter ARB telemetry title keys; verified via automated regex gate in audit scripts.
- [ ] `@[backend_v2/utils/math_utils.py]` satisfies AST rule QGR001 with zero `__dict__` reflection, traversing BaseModel attributes via `object.__getattribute__`.
