...
### Phase 1: Pydantic-skeemojen Ankkurointi (Schema Level Hardening)
...
### Phase 3: Synteesin Päätoimittajan (Chief Editor) Suvereniteetti
Päivitetään synteesimoottorin (`backend_v2/hooks/synthesis.py`) Master Prompt varmistamaan, että fragmentoituneiden laajennusten (esim. useiden matriisien jättämien *coaching*-viestien) sulauttaminen tapahtuu ilman, että ohjeiden terävyys katoaa tekstien yhdistelyssä.
* Varmistetaan, että `TARGET EXTENSIONS TO HARVEST` -logiikka karsii pois kaikki löysät tai toistuvat itsestäänselvyydet ja nostaa näytölle vain 3 kovinta, liiketoiminnan kannalta riskialtteinta/arvokkainta huomiota.
...
3. **Fail-Fast Compliance:** Arkkitehtuuria ei rikota. Yksikään näistä muutoksista ei muuta litistettyjen avainten mallia (esim. `blk_abc_coaching`), vaan parantaa ainoastaan sisällön laatua arvoketjun alkupäässä.
