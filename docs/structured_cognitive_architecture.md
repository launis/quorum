...
*   **XAI Reporter**: Generates the human-readable explanation (PDF/JSON).
...
1.  **Strict Pydantic Validation**: Every step output is validated against a strict schema.
2.  **Fail Fast**: The system crashes (raises `AppException` with RFC 7807 code) rather than retaining invalid state.
3.  **Zero-Fallback**: Configuration must be explicit. No hardcoding of models or rules in Python code.