...
Järjestelmä on suunniteltu toimimaan kahdessa eri toimintatilassa (*workflows*), jotka optimoivat prosessin joko syvällisyyden tai tehokkuuden mukaan:

1. **Sequential Audit (Courtroom 2.0):** Perinteinen, syväluotaava tila, jossa agentit toimivat peräkkäin auditointiketjussa. Tämä on oletustila maksimaalista tarkkuutta vaativissa tilanteissa.

2. **Fused Audit (Courtroom 3.0):** Uudempi, tehokkaampi tila, joka hyödyntää **PanelAgent**\-komponenttia. Tämä komponentti yhdistää logiikan, falsifioinnin ja kausaalisuuden arvioinnin yhdeksi "Paneeliksi". Tämä vähentää järjestelmän kognitiivista kuormaa ja nopeuttaa prosessia säilyttäen kuitenkin kriittiset arviointinäkökulmat. Kyseessä on pragmaattinen optimointi (PanelAgent), jossa useampi teoreettinen rooli suoritetaan yhden API-kutsun sisällä ("Single-Pass Inference"). Tämä vähentää merkittävästi latenssia ja token-kustannuksia verrattuna täysin eriytettyyn agenttimalliin.

Kvoorum koostuu erikoistuneista agentista, jotka jakautuvat toiminnallisiin ryhmiin (huomioiden valitun toimintatilan):

* **Turvaportti:** Vartija-agentti.  
* **Tiedonhallinta ja Vuorovaikutus:** Arkistonhoitaja-agentti (Archivist), joka vastaa datan esikäsittelystä ja eheyden ylläpidosta, sekä Vuorovaikutus-agentti (Interaction), joka ohjaa dynaamista dialogia.  
* **Analyysi:** Analyytikko-agentti ja Loogikko-agentti.  
* **Kriitikot (tai Paneeli):** Falsifioija, Valvoja, Kausaalinen ja Tunnistaja (koodiarkkitehtuurissa Profiloija-agentti eli Profiler). Fused Audit \-tilassa nämä roolit yhdistyvät PanelAgent-komponentin alle.  
* **Synteesi ja Raportointi:** Tuomari-agentti ja XAI-Raportoija-agentti.  
* **Ohjaus ja Palaute:** Valmentaja-agentti. Teknisesti kunkin agentin kognitiivinen rooli on operationalisoitu laajalla "System Prompt" \-määrittelyllä. Nämä kehotteet sisältävät roolikohtaiset toimintaohjeet sekä "Few-Shot" \-menetelmän mukaisia esimerkkejä (oikeat ja väärät suoritukset), jotka toimivat agentin implisiittisenä tietämyksenä ja ankkuroivat sen toiminnan haluttuun laatutasoon.

Tämän moniagenttijärjestelmän täydellinen operationaalinen komentorakenne, agenttien roolit ja vaiheittainen työnkulku muodostavat sen operatiivisen mallin.
...
Wooldridgen (2009) määritelmän mukaisesti järjestelmä toteuttaa moniagenttiarkkitehtuurin (MAS), joka tässä viitekehyksessä konkretisoituu ”sekventiaalisena auditointiketjuna”, jossa erikoistuneet agentit prosessoivat informaatiota peräkkäin kumuloituvassa prosessissa. Tämä arkkitehtuuri on itsessään verrattavissa tieteellisen menetelmän soveltamiseen (Cheng 2001):

* **Vaihe 1: Empiirinen havainnointi.** Prosessi alkaa todistusaineiston keräämisestä ja ankkuroinnista (Analyytikko-agentti, ks. Luku 2.3.3).  
* **Vaihe 2: Hypoteesin luominen.** Jäsennellyn argumentin muodostaminen (Loogikko-agentti, ks. Luku 2.3.4).  
* **Vaihe 3: Falsifiointi.** Argumentti altistetaan systemaattiselle kumoamisyritykselle (Kriitikkoryhmä, ks. Luku 2.3.5).  
* **Vaihe 4: Synteesi ja Johtopäätökset.** Tulokset kootaan yhteen (Tuomari-agentti), ja analyysiin liittyvä epävarmuus tuodaan esiin selitettävän tekoälyn (Explainable AI, XAI; vrt. Adadi & Berrada 2018\) avulla (XAI-Raportoija-agentti, ks. Luku 2.3.6).

Tämä vaiheittainen malli on tietoinen arkkitehtuurivalinta, joka priorisoi maksimaalista auditoitavuutta ja jäljitettävyyttä tehokkuuden kustannuksella. Vaikka dynaamisemmat debatti-arkkitehtuurit voivat tuottaa syvällisempiä oivalluksia (Du ym. 2023), tämä tiukasti sekventiaalinen malli valittiin perusarkkitehtuuriksi, jotta arviointiprosessi pysyy vakioituna ja mitattavana. Se pakottaa analyysin noudattamaan tiukasti falsifioinnin ja Toulminin mallin kaltaisia, ennalta määriteltyjä ja vakiintuneita loogisia rakenteita.

peratiivisesti tämä auditoitavuus toteutuu siten, että jokainen agentti tuottaa validoidun, tiukasti tyypitetyn DTO-välitulosteen (Pydantic-mallin) raa'an JSON-datan sijaan. Tämä 'Strict DTO' \-arkkitehtuuri ja modulaarinen rakenne edellyttää kaikkien välitulosten välittämistä prosessin loppuun yhteisen tilaobjektin (WorkflowState) kautta. Vaikka tämä lisää datan määrää, se on strateginen valinta: se varmistaa, että lopullinen päätös perustuu koko päättelyketjuun ja mahdollistaa Tuomari-agentin suorittaman hierarkkisen konfliktinratkaisun. Operationaalisella tasolla tämä sekventiaalinen ketju ja sen deterministinen valvonta on toteutettu modulaarisella, tapahtumapohjaisella (event-driven) Hook-arkkitehtuurilla. Agenttien välistä tiedonsiirtoa ja rangaistusmekanismeja ei ole kovakoodattu itse agentteihin, vaan ne on eriytetty deterministisiksi middleware-komponenteiksi (esim. scoring-, integrity- ja validation-hookit), jotka suorittavat pakolliset tarkistukset ja laskutoimitukset agenttien suoritusvaiheiden välissä (vrt. OWASP Foundation 2025d). Nämä välitulosteet muodostavat yhdessä "kognitiivisen jäljen" koko päättelyprosessista, joka voidaan tallentaa ja tarkastaa jälkikäteen (vrt. Luku 4.2).

#### **2.3.3 Vartija-agentti: Esikäsittely ja Turvaportti (Vaihe 1\)**

Operatiivinen prosessi alkaa syötteen esikäsittelyllä, jonka suorittaa Vartija-agentti, vastaten OWASP Foundationin (2025a) suosituksia syötteen validoinnista ja puhdistuksesta kriittisenä tietoturvakontrollina. Tämä agentti toteuttaa Teknisen Kontrollikerroksen (ks. Luku 2.4.2) toiminnot: Rakenteellinen Puhdistus, Datan Normalisointi, Datan Anonymisointi ja Aktiivinen Uhkien Luokittelu. Sen tehtävänä on torjua ulkoisia uhkia, kuten kehotemurtoja (LLM01:2025). Torjunta kohdistuu erityisesti epäsuoriin kehotemurtoihin (engl. Indirect Prompt Injection), jotka ovat kasvava uhka kielimalleille (Yi ym. 2025; Greshake ym. 2023; Liu, Yi ym. 2023; Liu, X. ym. 2024\) sekä autonomisten agenttien erityisuhkiin (AIMultiple 2025).

Viimeisenä valmisteluvaiheena ja ainoastaan jos turvatarkistukset on läpäisty, Vartija-agentti **leimaa** alkuperäisen datan turvalliseksi (engl. Security Stamping). Vartija ei luo uutta datakopiota (token-talouden ja injektiosuojan vuoksi), vaan palauttaa **turvallisuusstatuksen** (DATA\_CHECKED\_AND\_SECURED). Tämä status toimii portinvartijana ja ilman sitä prosessi keskeytyy välittömästi (Circuit Breaker).
...
##### **2.3.3.1 Tekninen Arkkitehtuuripäätös: Sivuvaunu-malli (Sidecar Auditor)**

Vartija-agentti on toteutettu rinnakkaisena tarkastajana (Parallel Audit), ei sarjaankytkettynä suodattimena.

* Mekanismi: Vartija analysoi syötteen ja palauttaa turvallisuusluokituksen (DATA\_CHECKED\_AND\_SECURED), mutta ei toista alkuperäistä tekstiä.  
* Perustelu: Tämä ehkäisee "Prompt Injection Mirroring" \-hyökkäykset (jossa tekoäly suorittaisi hyökkäyksen toistaessaan sitä) ja puolittaa token-kustannukset massiivisilla syötteillä.  
* Kill Switch: Jos Vartija havaitsee uhkan (threat\_detected: true), järjestelmä laukaisee välittömän keskeytyksen (Circuit Breaker), jolloin saastunut data ei koskaan etene Analyytikolle.
...
* **RFI-Protokolla (Tiedonhankinta):** Agentti suorittaa kohdennetun uusintahaun (Request for Information Protocol) (ks. Luku 2.4.3.1, Protokolla 3). Nykyisessä prototyypissä, jossa ei ole integroitua verkkohakutyökalua, tämä vaihe toteutetaan hyödyntämällä mallin sisäistä tietämystä ja kontekstin ristiintarkistusta (engl. *Simulated Retrieval*). Tämä iteratiivinen lähestymistapa on välttämätön monivaiheisessa päättelyssä (vrt. Trivedi ym. 2024). Se käyttää edistyneitä tekniikoita löytääkseen tietoa, joka jäi alkuperäiseltä haulta piiloon:  
...
Tämän agentin tehtävä on auditoida prosessin ajallista johdonmukaisuutta ja kausaalista uskottavuutta soveltamalla seuraavia heuristiikkoja (ks. Luku 2.4.3.4, Heuristiikat 1–3):
...
Tämä agentti keskittyy tunnistamaan käyttäytymismalleja ja pelistrategioita, jotka viittaavat järjestelmän manipulointiin (Goodhartin laki) (Strathern 1997; Stumborg ym. 2022). Tätä ohjaa mandaatti (ks. Luku 2.4.3.2, Mandaatti 4). Agentti etsii useita indikaattoreita:
...
* **Koskee:** Vartija-agentti, Analyytikko-agentti, Loogikko-agentti, Looginen Falsifioija-agentti, Faktuaalinen ja Eettinen Valvoja-agentti, Kausaalinen Analyytikko-agentti, Performatiivisuuden Tunnistaja-agentti, Tuomari-agentti, XAI-Raportoija-agentti.
...
* **Koskee:** Analyytikko-agentti, Loogikko-agentti, Looginen Falsifioija-agentti, Faktuaalinen ja Eettinen Valvoja-agentti, Kausaalinen Analyytikko-agentti, Performatiivisuuden Tunnistaja-agentti, Tuomari-agentti, XAI-Raportoija-agentti.
