...
2. ARCHITECTURAL BANS & MANDATORY VERIFICATION (Non-Negotiable - Enforced by Strict Mandates):
   ...
   - Frontend: Use Riverpod 3.0 code generation (`@riverpod`) and `Freezed` models for API/Domain data due to development speed and stability. Data management is kept small and concise. All asynchronous data must be rendered in the UI following the formal model. Routing MUST use `GoRouteData`. NO manual `if(isLoading)` checks (Use `.when()`). NO `Future.wait` monoliths for State.
