...
* **Staattiset Säännöt & Blind Extraction Engine:** Varsinainen kognitiivinen Blueprint (`<objective>` ja `<rules>`) pidetään aina 100% staattisena. Tämän ansiosta jopa 95% syötteestä pysyy muuttumattomana eri asiakkaiden ja suoritusten välillä, mahdollistaen maksimaalisen Token Caching -säästön. LLM pakotetaan "Extract-and-Justify" -arkkitehtuuriin (`BaseTDAExtraction`: `localized_anchors_found`, `semantic_reasoning`, `exact_quote`, `contextual_override`), jättäen varsinaisen matemaattisen Pass/Fail/DLQ-päätöksen täysin deterministiselle Python-funktiolle (`evaluate_extraction`). LLM on "riisuttu" arviointivallastaan ja toimii puhtaasti sokeana tekstinlukijana (Blind Extraction Engine).
