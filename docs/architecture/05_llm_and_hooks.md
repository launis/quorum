...
* **`client.py` & `provider.py`:** Huolehtivat rajapintatason (HTTP) kommunikaatiosta, asynkronisista aikatasauksista (Retry/Rate Limit) sekä erilaisten mallien `Parsing Mode`ista (esim. JSON Structured Output -pakotukset `GEMINI_JSON` modessa).
  ...
  * **Third-Party Health Check Bypass (Epic 81):** LiteLLM:n sisäiset pre-call health checkit on tarkoituksella kytketty pois päältä (`enable_pre_call_checks=False` Router-alustuksessa). Kolmannen osapuolen vialliset ja kaatuvat "EUR to USD" health-check mallikutsut aiheuttivat järjestelmän kaatumisia malformed-viestien (kuten "role: tool") takia. Arkkitehtuuri nojaa yksinomaan Quorumin omiin transienttien virheiden sietoketjuihin (Retry Loop) ilman kolmannen osapuolen ylimääräistä verkkoliikennettä.
