...
* **Staattiset Säännöt & Blind Semantic Extractor (Epic 56):** Varsinainen kognitiivinen Blueprint (`<objective>` ja `<rules>`) pidetään aina 100% staattisena. Tämän ansiosta jopa 95% syötteestä pysyy muuttumattomana eri asiakkaiden ja suoritusten välillä, mahdollistaen maksimaalisen Token Caching -säästön. LLM pakotetaan semanttisen tiedonkerääjän (Semantic Extractor) rooliin, joka palauttaa dynaamisen Pydantic-mallien tehtaan (`DynamicExtractionResponse` & `ExtractedFactsDTO`) tuottaman JSON-vastausrakenteen. Varsinainen matemaattinen Pass/Fail/DLQ-päätös on siirretty 100 % deterministiselle Python-funktiolle (`evaluate_extraction`) ja AST-evaluaattorille (`ast_evaluator.py`). LLM on "riisuttu" kaikesta arviointivallasta (ei `[5. VALIDATION DECISION]` -lokia promptissa) ja toimii puhtaasti semanttisena poimijana.
* **Prompt Topology & Tail-End Injection (Epic 48, Phase 5):** Roolit on eristetty tiukasti API-tasolla. Staattiset TDA-säännöt ja järjestelmäohjeet (`[System Prompt]`) lähetetään eksklusiivisesti `{"role": "system"}` -blokissa, kun taas dynaaminen data ja lähteet sijoitetaan `{"role": "user"}` -blokkiin. This säilyttää Prefix Tree -välimuistin eheyden massiivisissa Map-Reduce ajoissa. Mikäli Pydantic V2 -skeema kaatuu ja järjestelmä pakottaa "Self-Healing" korjauksen, LLM:n edellistä virheellistä vastausta EI lisätä keskusteluhistoriaan (mikä myrkyttäisi kontekstin ja saattaisi evätä välimuistin tehon). Sen sijaan uusi korjauskehote injektoidaan lennosta suoraan olemassa olevan User-viestin loppuun (`Tail-End Injection`). Tämä varmistaa, että alkuperäinen iso data pysyy täsmälleen identtisenä ja säilyttää OpenAI/Anthropic Prefix Cache -osumat myös Retry-luupeissa.
...
*   **Native English Generation Mandate & Bilingual Decoupling (Epic 56 Poikkeus):** LLM ei pääsääntöisesti koskaan tuota kognitiivista päättelyään (kuten yleisiä synteesejä tai asiantuntijahypoteeseja) suoraan muulla kuin englannin kielellä ("Intelligence Dropping" -esto). Kuitenkin Epic 56 TDA-poimintavaiheessa sovelletaan **Bilingual Decoupling** -poikkeusta: vaikka JSON Schema -rakenteen avaimet pidetään englanniksi, `context_scan_trace` (Micro-CoT) ja etsittävät semanttiset faktat (`facts_to_find`) käsitellään ja poimitaan suoraan kohdeasiakirjan alkuperäisellä kielellä. Tämä poistaa Cross-Lingual Attention Taxin, estää kääntämisestä johtuvat hallusinaatiot ja takaa leksikaalisen ankkuroinnin osumatarkkuuden. Yleiset synteesit käännetään Post-Hook -kerroksessa `translation_hook.py`:n kautta.
...
* **Kielto Työnkulkujen Kaatamiselle:** LLM:n epäonnistuessa loogisessa validoinnissa (esim. max_logical_retries ylittyy tai taustalla havaitaan jumittunut korjaussilmukka), järjestelmä ei saa kaataa koko työnkulkua `AgentExecutionError`-poikkeuksella.
...
* **Polymorfinen ja rekursiivinen Null Object -fallback (V2.6 / Phase 9):**
  * *Ongelma:* Aiempi suoraviivainen Null Object -malli oletti DTO-rakenteiden olevan aina litteitä ja tasaisia (sisältäen suoria kenttiä kuten `exact_quote`, `score`, `justification`). Kun suoritettiin monimutkaisia matriisievaluointeja, joissa arvioinnit on ryhmitelty dynaamisesti sisäkkäisten listojen ja alimallien alle (esim. `evaluations: list[AtomResponse]`), litteä tyhjennys loi vain tyhjän juuri-objektin ilman vaadittua listaa. Tämä johti jatkokäsittelyn pisteytyskoukuissa kriittisiin `VALIDATION_FAILED`-kaatumisiin, koska downstream-logiikka vaati tyypitettyjen tietorakenteiden ja listojen fyysistä olemassaoloa.
  * *Ratkaisu:* `LLMTaskExecutor` hyödyntää dynaamista, rekursiivista `build_fallback(model_cls, existing)`-generaattoria osana virheenkäsittelyä. Tämä apufunktio tutkii kohdemallin Pydantic-kentät ohjelmallisesti ajonaikana:
    * Se tunnistaa dynaamisesti alimallit ja listat (kuten `list[BaseModel]`) ja käsittelee ne rekursiivisesti alaspäin.
    * Se säilyttää osittain validoidusta mallista (`validated_model`) kriittiset rakenteelliset tunnisteet ja metatiedot (kuten `atom_id` evaluointilistojen alkioissa), jotta jatkovaiheet voivat yhä kohdistaa tulokset oikeisiin lähteisiin.
    * Se nollaa epäonnistuneen tai jumittuneen evaluoinnin pisteytyskentät (esim. `exact_quote` ja `score` arvoon `None`, `contextual_override` arvoon `False`) ja asettaa laadullisille perustelukentille (kuten `justification`, `reasoning_trace`) järjestelmävirhemerkinnän `[SYSTEM ERROR: LLM Unable to verify.]`.
* **Vaikutus (Impact):** Tämä "Graceful Degradation" varmistaa sataprosenttisen tyyppiturvallisuuden ja virhesietoisuuden monimutkaisimmissakin matriiseissa. Vaikka LLM:n itsekorjaussilmukat ylittyisivät tai jumittuisivat, suoritus ei kaadu vaan putoaa hallitusti skeeman mukaiseen rekursiiviseen Null-tilaan. Jatkovaiheen pisteytyskoukut ohittavat `None`-arvoiset pisteet ja mittarit turvallisesti ilman Pydantic-tyyppirikkoja tai kaatumisia, eikä epäonnistuminen korruptoi aggregoitua matematiikkaa.
