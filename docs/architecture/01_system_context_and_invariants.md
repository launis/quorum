...
1. **Epic-Derived Multi-Phase Tracking (`docs/epic/EPIC_XXX_tracker.md`):** Synchronizes phase progression, phase gates, and cross-phase handover context generated by `/tier1-tracker-generator`.
2. **Standalone Single-Phase Tracking (`docs/implementationplans/TRACKER_[slug].md`):** Synchronizes step execution status, target file-level hardening checklists, requirements traceability matrix (RTM) step parity, and session handover context generated by `/tier1-plan-tracker-generator`.
3. **Automated Verification Pipeline (`scripts/audit_tracker_output.py`):** Strictly verifies both Epic and Standalone Plan tracker schemas in a single deterministic pipeline (`TrackerMode.EPIC` vs `TrackerMode.PLAN`), validating mandatory sections, step checkbox formatting, and 1:1 bidirectional step traceability against physical plan files before execution or quality gate completion.
