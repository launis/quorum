...
3. **NodeExecutor (Yksittäisen tason äly - Strategy Pattern):** 
   ...
   * **Dead Letter Queue (DLQ) Arq Fallback Logic:** Rinnakkaiset `ChunkWorker`-prosessit on suojattu "Fail-Fast DLQ Routing" -logiikalla, joka estää massiiviset Arq-tason retry-myrskyt Pydantic `ValidationError` tai API-virhetilanteissa. Worker palauttaa orkestraattorille `{"_dlq_status": "FAILED/DLQ", "reason": "<error>"}`. Tämä mahdollistaa chunkin tyylikkään siirtämisen Dead Letter Queueen auditoitavaksi ("Duct Tape Ban" -säännön puitteissa) kaatamatta koko asynkronista puuta.
