...
3. **Data Visualization Component**: Dedicated chart or table (`SduiRadarChartBlock`, `SduiScatterPlotBlock`, `SduiQuadrantMatrixBlock`, `SduiMetrics1DBlock`, `SduiMatrixTableBlock`) with its internal `title` attribute explicitly set to `None` to prevent duplicate header rendering. Complex visualization components decompose into focused, private Dumb Painter cell widgets (header cells, matrix identity cells, distribution cells, and diagnostic narrative cells). In-build sorting and mutable list allocations within render cycles are completely eliminated; the renderer maps backend DTO sequences directly to decomposed cells.
4. **Citations / Sources**: Localized footer blocks (`MarkdownBlock(text="#### " + localized_sources)`, `PrintableSourcesBlock`).
For text-only views, the data visualization component is omitted entirely, producing strictly `MarkdownBlock` -> `ParagraphBlock`. All scaling math, score calculations, date formatting, and static headers are pre-computed and pre-localized on the backend, ensuring client rendering remains mathematically decoupled from presentation. Within the presentation pipeline, `BlueprintTransformer` operates as a 100% read-only Dumb Painter: it executes zero database mutation calls (`update_execution` is strictly prohibited in presentation transformers), directly projects the finalized evaluation score from `scoring_result.total_score`, and passes active penalty tokens directly to `AdapterContext.penalties_applied` for `PenaltiesAdapter` rendering, executing zero scoring recalculations or profile-based score mutations.
...
- **Backend (Python Pydantic V2)**: `AnySduiBlock = Annotated[..., Field(discriminator="block_type")]`. Every concrete child model inherits from `SduiBlockBase`, configuring `model_config = ConfigDict(title="<block_type>", strict=True, extra="forbid")` and binding a literal discriminator field (`block_type: Literal["<block_type>"]`). Forward-reference hydration for complex nested SDUI blocks (`MatrixScorecardRowDTO`, `ReportDataDTO`) is deterministically resolved through co-located Pydantic `model_rebuild()` calls at the module level in view and report models, injecting the exact types namespace (`AnySduiBlock`, `MCPAuditTrace`) and eliminating runtime circular imports.
- **Frontend (Flutter Dart 3)**: Sealed class `@Freezed(unionKey: 'block_type') sealed class SduiBlockDTO`. Every union variant enforces `@JsonSerializable(disallowUnrecognizedKeys: true)`.
- **Exhaustive Renderer Matching**: In the client block renderer, rendering uses Dart 3 native `switch` expressions over `SduiBlockDTO` exhaustively matching all 18 concrete block types without wildcard defaults (`_ =>`):
...
Presentation logic is modularized into isolated presentation adapters. Every adapter strictly adheres to a standardized Two-Section Architecture:
- **Section 1 (Strongly Typed Aesthetic Rules)**: Module-level styling configurations bind to dedicated, immutable Pydantic V2 aesthetic DTOs (`GlobalScoreAestheticsDTO`, `MatrixGraphsAestheticsDTO`, `MatrixSummaryAestheticsDTO`, `McpAuditAestheticsDTO`, `MetadataAestheticsDTO`, `SynthesisTextAestheticsDTO`, `WarningCardAestheticsDTO` in `sdui_rules`). This centralizes all visual styling decisions, mapping domain keys to visual properties (`severity`, `icon_name`, label keys) via static dot-notation attributes and typed mappings, completely eradicating Primitive Obsession and untyped dictionary subscripting. Missing configurations trigger an immediate RFC 7807 error and an `AppException`, preventing unconfigured attributes from silently defaulting.
...
### 2.4. Database-Driven Target Block Dispatching & Presentation Services
...
- **Multi-Tab Excel & Flat-File CSV**: Compiles matrix evaluations, scores, citations, risk flags, and summary metrics into structured spreadsheets.
- **B2B Tabular Row Delivery**: Emits strongly typed `ReportRowItemDTO` items via `/rows` for programmatic ingestion into enterprise BI platforms.
- **Dual-Axis Localization Anchoring**: Export column headers and section titles resolve strictly from backend `I18nText` database records and static dictionaries, guaranteeing that backend export services never access client `.arb` resource files.

#### Desktop-Class Pro Tool UI for Report Artifacts
Client-side report browsing and authoring adhere strictly to the 16-pillar Desktop Pro Tool UX standard:
- **`ExecutionReportsView`**: Built with the Adaptive Master Selector standard (`LayoutBuilder` wide >= 900px lateral 280px sidebar list with `ListView.builder` virtualization vs compact < 900px horizontal `ChoiceChip` top-bar), Single Cognitive Unit isolation (rendering exactly ONE report at a time), 4-tab Progressive Disclosure (SDUI visual tree, PDF document preview, tabular rows table, execution telemetry and FinOps metadata), and full `AppErrorBoundary` containment.
- **`CreateReportDialog`**: Implements the Dual-Shield FormField Architecture with focus-loss auto-flush, modal exit interception via `PopScope(canPop: false)` with post-frame blur flush, serialization-based model dirty checking (`jsonEncode(_model.toJson()) != _initialModelJson`), Modal-Internal Error Surface (absolute SnackBar ban), auto-scroll to invalid fields, and atomic save debouncing.

### 2.5. Asynchronous SDUI Matrix Synthesis & Arq Worker Orchestration
Qualitative report synthesis is decoupled from synchronous API endpoints and executes asynchronously within Arq background workers (`generate_report_artifact_job`):
- **Centralized Prompt Directives SSOT**: Foundational prompt templates, Layer 1 system identities, and structural constants are imported from the static synthesis prompt assets SSOT (`SYNTHESIS_SYSTEM_PROMPT`, `ROW_EXPLANATION_SYSTEM_PROMPT`, `VARIANCE_SYSTEM_PROMPT`, `EXECUTIVE_SUMMARY_SECTION_ID`, `SYNTHESIS_SECTION_RULES_PREFIX`, `SYNTHESIS_XAI_CURATION`).
...
Structured output schemas for dynamic LLM steps are governed through a decentralized Strategy and Registry architecture in the core schema registry:
...
- **Scalable Master List Browsing & Virtualization Standard**: All master list browsing views utilize virtualized rendering (`ListView.builder` with `prototypeItem` for $O(1)$ layout calculation and 60 FPS scrolling), sticky pinned top headers (`StudioMasterHeader`) integrating real-time search inputs and active item count indicators, and centered 1200px max-width containment (`Bounded Canvas Containment`), preventing unconstrained canvas stretching on high-resolution displays while maintaining responsive usability across compact and split viewports.
- **Modal Feedback & Submission Guard**: Dialogs eradicate `SnackBar` popups in favor of modal-internal error surfaces, implement in-flight atomic save debouncing (`_isSaving`), auto-scroll to invalid fields, and enforce a robust `PopScope` protocol with serialization-based model dirty checking (`jsonEncode(model.toJson()) != initialJson`) that prevents false-positive discard dialogs on models utilizing reference identity while intercepting all exit triggers with discard confirmation when uncommitted changes exist.

### 2.12. Matrix Summary Table Architecture & Evaluative Column Mechanics
The matrix summary table (`MatrixSummaryTableAdapter` / `SduiMatrixTableBlock`) presents a comparative, multi-matrix evaluation breakdown. The table format is composed of three primary columns:

1. **`Logiikkamatriisi` (Matrix Identity & Grounding)**:
   - Identifies the evaluated dimension with its localized title (e.g. *Harkintakyky (Kahnemanin Kaksoisprosessiteoria)*) and underlying academic theory citation.
   - Displays the composite star score rating for the matrix.
   - Exposes an explicit context target input scope badge (e.g. `Kaikki syötteet` for `target_input_key: "all"`, `Lopputuote` for `product_text`, or `Keskusteluhistoria` for `chat_log`), guaranteeing evidentiary transparency for the user.
...
3. **`Selitys` (Diagnostic Narrative Synthesis & Reducer Synergy)**:
   ...
   - This explanation is generated by downstream specialist synthesis steps (such as Step 7 / `synthesis_generation`) powered by `MatrixReducer`.
   - During DAG execution, `MatrixReducer` intercepts the raw atom evaluations and strips away all passing baseline atoms (e.g., the 5/5 from Level 1 and 3/5 from Level 2). It compresses the payload to contain strictly the failed atoms (`FAILED`) and quantitative data extractions.
   - The synthesis LLM ingests this concentrated exception list and formulates a diagnostic sentence explaining the precise failure modes (e.g., *"Päättely nojaa vahvasti Systeemi 1:n heuristiikkoihin: kontekstuaalinen dekonstruktio ja probabilistinen perusjoukkoajattelu puuttuvat, mikä ankkuroi analyysin pintaoletuksiin syvällisen kognitiivisen haastamisen sijaan"*).
