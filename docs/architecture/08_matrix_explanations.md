...
#### BARS Progression
- **Level 1 (Remembering):** Mechanical rote recall, verbatim reproduction, and uncontextualized cataloging of raw facts. Error radars monitor categorical dogmatism without epistemic qualification; faithful summarization of explicitly cited external research or published studies is formally distinguished from personal ungrounded dogmatism through explicit anti-pattern boundaries.
- **Level 2 (Understanding):** Paraphrasing, summarizing, and basic interpretation of concepts without deeper structural analysis.
- **Level 3 (Applying):** Direct application of established principles, rules, or procedural formulas to concrete operational contexts. Merely cataloging tactical recommendations, action lists, or organizational advice without explicitly naming and mapping an abstract governing rule or theoretical framework onto concrete mechanisms does not satisfy this level.
...
- **Level 5 (Evaluating):** Critical evaluation, appraisal of trade-offs, defense of positions based on explicit criteria, and objective validation of methodologies. Citing, cataloging, or naming regulatory standards, statutory mandates, industry benchmarks, or policy frameworks merely as background compliance context does not satisfy criteria anchoring; evaluation mandates actively measuring and auditing observed performance directly against explicit quantitative thresholds or operational benchmarks.
- **Level 6 (Creating / Synthesizing):** Novel framework synthesis, original conceptual modeling, and formulation of innovative strategic solutions resolving systemic contradictions. Thematic clustering, organizational regrouping, renaming, or hierarchical summarization of external source documents, baseline categories, or assignment background materials without deriving an explicit causal, operational, or formal mechanism does not satisfy original creation; creation mandates formulating a novel paradigm with explicit structural boundary constraints and operational interactions.
...
#### BARS Progression
- **Level 1 (Pure System 1 Heuristics):** Uncritical reliance on intuitive heuristics, cognitive biases (anchoring, availability, confirmation bias), and passive acceptance of default propositions. Quoting, citing, transcribing, or prompting with metaphors, analogies, or narrative associations authored by an external source document or conversational partner is explicitly distinguished from the evaluated speaker's organic intuition and does not satisfy user attribution.
- **Level 2 (Transitional / Mixed Scrutiny):** Awareness of intuitive shortcuts with intermittent analytical verification; partial inspection of critical claims alongside residual unexamined assumptions, marked by active cognitive pauses to deliberate on substantive assumptions. Retrospective post-hoc reflection on procedural execution, missing files, or administrative difficulties is strictly distinguished from an active cognitive deliberative pause questioning substantive domain assumptions.
...
### 8.5 Evidence Extraction Invariants & Provenance Sovereignty
1. **Anti-Ellipsis Lexical Anchoring:** Extracted quotes (`source_quote`) must strictly consist of a single, continuous, unbroken character substring directly from the source material. Splicing separate sentences with ellipses (`...` or `…`) is prohibited, preserving exact lexical matching (`str.find`) in the downstream verification engine without false-negative validation failures.
