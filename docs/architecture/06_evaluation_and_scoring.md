...
**TDA-väitteiden kohdistus ja palomuurit (Käyttäjän arvioinnin periaate):**
Cognitive Quorum -järjestelmän päätavoite on aina arvioida **käyttäjän suoritusta ja käyttäjän argumentointia** (eikä tekoälyä itseään). Tämän vuoksi semanttisten poimintasääntöjen (TDA Assertions) kohdemäärittelyt noudattavat tiukkaa arkkitehtonis-loogista periaatetta datan ristikontaminaation estämiseksi:
1. **Keskusteluhistoria (Chat Log):**
   * Keskusteluhistoria on ainoa syöteaineisto, joka jakautuu selkeästi rooleihin `user:` (käyttäjä) ja `ai:` (tekoäly).
   * Kun arvioidaan käyttäjän argumentointia ja ohjausta keskustelussa, haku on kohdistettava nimenomaan käyttäjän omiin viesteihin.
   * Tekoälyn vastaukset keskusteluhistoriassa toimivat häiriötekijöinä. Siksi keskusteluhistoriaa arvioitaessa sääntöjen tulee käyttää suodatusta `BANNED SOURCES: AI responses` (evidenssiä ei saa poimia `ai:`-etuliitteellä varustetuista vastauksista).
2. **Muut lähtösyötteet (Esim. `reflection_text`, `product_text` jne.):**
   * Kaikki muut syötteet (kuten reflektiotekstit, tuotekuvaukset tai ladatut tiedostot) käsitellään arkkitehtuurissa poikkeuksetta kokonaisuudessaan **käyttäjän omina syötteinä** (User Inputs).
   * Tämän vuoksi näille aineistoille kohdistetuissa TDA-väitteissä sääntöilmaisut kuten `BANNED SOURCES: User inputs` tai `BANNED SOURCES: Never read from user input fields` ovat **loogisia virheitä (oikosulkuja)**. Koska koko arvioitava kohdeaineisto on käyttäjän syötettä, kyseinen sääntö kieltäisi arvioimasta juuri sitä ainoaa paikkaa, jossa arvioitava argumentaatio sijaitsee.
   * Näiden syötteiden kohdalla käyttäjän kirjoittama teksti on **ensisijainen Required Target**, ja ainoat kiellettävät lähteet ovat järjestelmäohjeet (`BANNED SOURCES: System instructions/prompts`), jotta tekoäly ei poimi omia arviointiohjeitaan todisteiksi.
