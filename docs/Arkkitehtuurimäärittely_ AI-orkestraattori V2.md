...
3. **Strict Pydantic V2 & Flutter Freezed Parity:** Backendin data puretaan suoraan Rust-ytimessä (`MyModel.model_validate_json`, `extra="forbid"`). Frontendissä käytetään teollisuusstandardia (Freezed + `json_serializable` tiukalla `disallow_unrecognized_keys: true` -asetuksella). Manuaalinen JSONin purku on kielletty, ja polymorfinen reititys UI:ssa hoidetaan yksinomaan natiiveilla Dart 3 `sealed class` ja `switch` pareilla (ei `.when()` tai fallback-tyyppejä).
...
Telemetria ja raportointi noudattavat järjestelmässä arkkitehtuurista Dual-Reporting -mallia (`flutterpromptohje.md` -standardi), joka suojelee dataa ja keskittää näkyvyyden:
