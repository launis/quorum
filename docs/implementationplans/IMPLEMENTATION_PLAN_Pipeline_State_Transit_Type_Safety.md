...
1. **`TraceEvent.content` and `metadata` Naked Dictionaries**: In @[backend_v2/models/state.py#L157-L208], both fields remain annotated as `dict[str, Any]`, causing 2 FATAL AST violations under `scripts/audit_dict_eradication.py`.
2. **`StateProjector._snapshot` & Duct-Tape Fallback**:
   - In @[backend_v2/models/state.py#L480-L613], `_snapshot: dict[str, Any]` causes 1 AST violation. Directly typing as `dict[str, dict[str, StepPayloadValue]]` triggers a fatal AST Primitive Obsession violation under `_find_nested_dict_subscript`. It must be typed as `dict[str, StepOutputContentDTO]` using the authoritative SSOT DTO.
...
3. **Dynamic Reflection & Negative String Filter in `state.py`**: In @[backend_v2/models/state.py#L240-L299], `_state_localns` uses `.__dict__` reflection (`_inputs_mod.__dict__`, `_step_output_mod.__dict__`) and negative filtering (`not k.startswith("__")`), triggering 2 FATAL AST reflection violations (`QGR001`).
...
6. **`synthesis_payload_compressor.py` Naked Dicts & Dead Code**: In @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L181-L385], 3 naked `dict[str, Any]` annotations trigger 3 FATAL AST violations, and line 384 contains unreachable dead code.
...
9. **Hardcoded Language Specifications & Ad-Hoc Locale Fallbacks**:
   ...
   - In @[backend_v2/models/enums.py#L488-L493] and @[client_app_v2/lib/core/models/enums.dart#L380-L389], `SystemLocale` only defines `EN = "en"` and `FI = "fi"`, omitting `SV = "sv"` and creating a cross-domain mismatch with `auth.py`.
...
- @[backend_v2/models/enums.py#L488-L493] [MODIFY]: Add `SV = "sv"` to `SystemLocale` enum to establish unified trilingual SSOT (`EN`, `FI`, `SV`).
...
- @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L181-L385] [MODIFY]: Replace 3 naked `dict[str, Any]` annotations with `dict[str, JsonValue]` and delete unreachable dead return statement at L384.
...
2. **`_state_localns` Reflection & Negative Filter Purge**:
   - Location: @[backend_v2/models/state.py#L240-L299].
...
3. **`synthesis_payload_compressor.py` Dead Code Removal**:
   - Location: @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L181-L385].
...
<execution_protocol>
  <step id="1" name="PRE_IMPLEMENTATION_CLEANUPS">
    ...
    <action>In @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L181-L385], remove the unreachable dead code `return val` following the return comprehension.</action>
  ...
  <step id="2" name="DEFINE_TRACE_DTOS_AND_EXPAND_PAYLOAD_UNION">
    ...
    <action>In @[backend_v2/models/dtos/trace.py#L179-L201], tighten `TraceMatrixPayloadDTO.atom_quotes` to `Annotated[list[str] | None, Field(default=None, description="Optional accumulated atom quotes from matrix evaluation")] = None` using pure Python 3.14 Annotated syntax.</action>
  ...
  <step id="3" name="TRACE_EVENT_AND_STATE_PROJECTOR_TYPING">
    <action>In @[backend_v2/models/state.py#L157-L208], update `TraceEvent.content` to `Annotated[StepPayloadValue | DomainInputValue | BaseModel | dict[str, StepPayloadValue] | None, Field(default=None, description="Typed event payload")] = None` and `TraceEvent.metadata` to `Annotated[TraceEventMetadataDTO, Field(default_factory=TraceEventMetadataDTO, description="Typed event metadata")]` using pure Python 3.14 Annotated syntax (with zero duplicate `= Field(...)` trailing assignments).</action>
    <action>In @[backend_v2/models/state.py#L480-L613], update `StateProjector._snapshot` type annotation to `dict[str, StepOutputContentDTO]` to strictly eradicate Primitive Obsession nested dict violations.</action>
    <action>In @[backend_v2/models/state.py#L480-L613], eradicate `.model_construct()` fallback in `_build_dto_list()`. Log structured RFC 7807 error and raise `AppException(status_code=500, details={"error_code": ErrorCodes.VALIDATION_FAILED.value})` immediately upon `ValidationError`.</action>
    <action>In @[backend_v2/models/state.py#L480-L613], update `apply_delta()` to unwrap `ExecutionInputsDTO`, `WorkflowInputs`, `StepOutputContentDTO`, and `LightweightMatrixDTO` directly into `StepOutputContentDTO(data=...)` in `_snapshot[step_id]` without intermediate dict dumps or duct tape.</action>
    <action>In @[backend_v2/models/state.py#L591-L613], wrap GDPR tombstone marker in `apply_delta()` into `StepOutputContentDTO(data={"_redacted": True, "hash": redacted_hash})` instead of raw dictionary assignment into `_snapshot`.</action>
  ...
  <step id="6" name="SYNTHESIS_PAYLOAD_COMPRESSOR_CLEANUP">
    <action>In @[backend_v2/services/orchestrator/synthesis_payload_compressor.py#L181-L385], replace 3 naked `dict[str, Any]` annotations with `dict[str, JsonValue]` at lines 221, 293, and 374.</action>
  ...
  <step id="8" name="LOCALE_AND_INTERNATIONALIZATION_SSOT_HARMONIZATION">
    <action>In @[backend_v2/models/enums.py#L488-L493], add `SV = "sv"` to `SystemLocale` enum.</action>
