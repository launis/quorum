...
12. **[DISCOVERED DEBT RESOLVED] `logic.py` & `base.py` Native Dictionary Union Migration**: In `@[backend_v2/services/orchestrator/strategies/logic.py#L25-L203]` and `@[backend_v2/services/orchestrator/strategies/base.py#L37-L64]`, since dynamic inputs are strictly typed as scalar blackboards (`dict[str, str | int | float | bool | list[str]]`), these calls are refactored to native dictionary union operations (`state_data | delta_dict` / `new_dynamic | delta["dynamic_inputs"]`), completely removing the need for `_deep_merge` helpers or `# noqa: QGR012`.
...
14. **[DISCOVERED DEBT RESOLVED] `_deep_merge` Eradication**: Private recursive `_deep_merge()` is completely eliminated in favor of native scalar dictionary unions, producing 0 AST violations in `base.py`.
...
<execution_protocol>
  ...
  <step id="9" name="AST Guardrails Strict 4-Driver Lockdown & Rule Expansion (QGR013-QGR015)">
    ...
    <action>Add `QGR013` (TypeVar ban), `QGR014` (AsyncMock on repository interfaces ban with precise `I*Repository` AST heuristic, Severity: `FATAL`), and `QGR015` (TypeGuard ban per PEP 742 `pep742_typeis_over_typeguard`) visitor rules to `@[scripts/_ast_guardrails.py#L170-L665]`.</action>
