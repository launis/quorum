...
7. **Eradicate Reflection and Duck-Typing in `create_execution_record` Factory**:
   In @[backend_v2/services/execution.py#L136-L137], purge `# noqa: QGR012` suppression and `isinstance(resolved_metadata, dict)` reflection. Enforce strict Pydantic validation via `TypeAdapter(ExecutionMetadata).validate_python(resolved_metadata)` and enforce typed parameters, eliminating `**extra_persistence_fields` pass-throughs.
8. **Eradicate Loose Dictionary Signatures and Catch-All Handlers in Background Workers**:
   In @[backend_v2/worker.py#L139-L146] and `#L546-L554`, eliminate loose `inputs: dict[str, Any]` argument transit in favor of typed `ExecutionInputsDTO`. Replace generic multi-type exception handling with typed `AppException` mapping and RFC 7807 structured errors with explicit `ErrorCodes`.
9. **Purge Raw Prompt Block IDs from Output Profile DTOs**:
   In @[backend_v2/models/dtos/output_profile.py#L39-L478], synchronize `OutputProfileCreateDTO` and `OutputProfileUpdateDTO` with domain models by completely eradicating `variance_target_block` and `user_role_target_block` fields and their coherence validators.
