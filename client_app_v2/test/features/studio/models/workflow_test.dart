...
void main() {
  group('Epic 11 Phase B: NodeStrategy Strict Parsing', () {
    test('Successfully parses valid expectedInputs and outputSchema via Isolate', () async {
      final payload = {
        'id': 'st_valid123',
        'slug': 'test_slug',
    ...
    test('Fails-Fast when expectedInputs is the wrong type (Map instead of List)', () async {
      final payload = {
        'id': 'st_invalid123',
        'slug': 'test_slug',
    ...
    test('Logic nodes parse successfully without requiring outputSchema', () async {
      final payload = {
        'id': 'st_logic123',
        'slug': 'logic_slug',
        'name': {'default_locale': 'en'},
