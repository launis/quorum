...
class MockModelRegistryController extends AsyncNotifier<List<ModelConfig>>
  ...
  Future<ModelConfig> createSystemConfigDraft() async =>
      const ModelConfig(id: 'sys_draft12345678', slug: 'draft', type: 'model_registry');

  @override
  Future<ModelConfig> cloneConfig(String id) async =>
      const ModelConfig(id: 'sys_cloned123456', slug: 'cloned', type: 'model_registry');
...
void main() {
  group('StudioDashboardView Tab 6 Model Registry Tests', () {
    ...
    testWidgets('Renders Tab 6 with Model Registry items and tier count badge', (
      WidgetTester tester,
    ) async {
      ...
      final mockConfigs = [
        const ModelConfig(
          ...
          tierDefinitions: {
            'fast': LlmModelConfig(provider: 'google', modelName: 'gemini-3.8-flash'),
            'balanced': LlmModelConfig(provider: 'google', modelName: 'gemini-3.8-flash'),
            'deep': LlmModelConfig(provider: 'google', modelName: 'gemini-3.8-pro'),
            'reasoning': LlmModelConfig(provider: 'google', modelName: 'gemini-3.8-pro'),
          },
        ),
        const ModelConfig(
    ...
    testWidgets('Filtering search query updates active count and hides non-matching stacks', (
      WidgetTester tester,
    ) async {
      tester.view.physicalSize = const Size(1400, 900);
      tester.view.devicePixelRatio = 1.0;
