...
void main() {
  ...
  group('ExecutionClient', () {
    ...
    test('startExecution posts request and returns ExecutionRecord', () async {
      ...
      when(
        () => mockDio.post('/execution/executions/', data: any(named: 'data')),
      ).thenAnswer(
        (_) async => Response(
    ...
    test(
      'resumeExecution calls resume endpoint and returns ExecutionRecord',
      () async {
        when(
          () => mockDio.post('/execution/executions/$testExecutionId/resume'),
        ).thenAnswer(
          (_) async => Response(
            requestOptions: RequestOptions(
              path: '/execution/executions/$testExecutionId/resume',
            ),
            data: testRecordJson,
            statusCode: 200,
          ),
    ...
    test('getExecutionStatus fetches execution record by id', () async {
      when(
        ...
        (_) async => Response(
          requestOptions: RequestOptions(
            path: '/execution/executions/$testExecutionId',
          ),
          data: testRecordJson,
          statusCode: 200,
        ),
    ...
    test(
      ...
      () async {
        ...
        const payload = HumanOverrideRequestDto(
    ...
    test(
      'Negative Test 1: startExecution propagates DioException on network/server error',
      () async {
        when(
          () =>
              mockDio.post('/execution/executions/', data: any(named: 'data')),
    ...
    test(
      'Negative Test 2: overrideAtom throws when server returns malformed payload',
      () async {
        ...
        expect(
          () => client.overrideAtom(
            executionId: testExecutionId,
            atomId: 'atm_123',
            payload: const HumanOverrideRequestDto(
