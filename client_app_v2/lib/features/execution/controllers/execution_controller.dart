...
class ExecutionController extends _$ExecutionController {
  ...
  /// Rejects a specific evidence quote (Soft Delete) and optimistically updates the UI.
  Future<void> rejectEvidenceQuote(
    String executionId,
    String quoteId,
    String? reason,
  ) async {
    ...
    try {
      ...
      final currentMap = Map<String, dynamic>.from(state.value!);
      if (currentMap['report_data'] != null) {
        final reportData = currentMap['report_data'] as ReportDataDTO;

        final newEvaluativeMatrices = reportData.evaluativeMatrices.map((m) {
          if (m.forensics == null) return m;

          final newLevelQuotes = m.forensics!.levelQuotes.map((lq) {
            final newQuotes = lq.quotes.map((q) {
              if (q.id == quoteId) {
                return q.copyWith(userRejected: true, rejectionReason: reason);
              }
              return q;
            }).toList();
            return lq.copyWith(quotes: newQuotes);
          }).toList();

          final allRejected =
              newLevelQuotes.isNotEmpty &&
              newLevelQuotes.every(
                (lq) =>
                    lq.quotes.isNotEmpty &&
                    lq.quotes.every((q) => q.userRejected),
              );

          return m.copyWith(
            forensics: m.forensics!.copyWith(
              levelQuotes: newLevelQuotes,
              allEvidenceRejected: allRejected,
            ),
          );
        }).toList();

        final newInformationalMatrices = reportData.informationalMatrices.map((
          m,
        ) {
          if (m.forensics == null) return m;
        ...
        currentMap['report_data'] = reportData.copyWith(
          evaluativeMatrices: newEvaluativeMatrices,
          informationalMatrices: newInformationalMatrices,
        );
      }
      ...
      final client = ref.read(executionClientProvider);
      await client.rejectEvidenceQuote(executionId, quoteId, reason);
    } catch (e, stack) {
      ref
          .read(loggerServiceProvider)
          .error(
            'ExecutionController',
            'REJECT_EVIDENCE_FAILED: Failed to reject quote $quoteId',
            e,
            stack,
          );
      // Revert optimistic update
      state = previousState;
      // Throw AppException to be caught by UI boundary
      throw AppException.network(
        'Failed to reject evidence quote.',
      ).copyWith(extensions: const {'error_code': 'REJECT_EVIDENCE_FAILED'});
    }
  }
}
