...
- [ ] Step 2: In `@[backend_v2/services/report_service.py#L276-L288]`, update `compile_and_persist_artifact` to await `arq_pool.delete(f"arq:result:{job_key}")` before calling `arq_pool.enqueue_job`. Consolidate report compilation in `@[backend_v2/workers/synthesis_worker.py#L140-L154]`, `@[backend_v2/workers/synthesis_worker.py#L518-L528]`, and `@[backend_v2/workers/synthesis_reducers.py#L355-L365]` to delegate cleanly to `ReportService.compile_and_persist_artifact`.
