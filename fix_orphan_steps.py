...
def fix_orphan_steps():
    ...
    for wf in workflows_table.all():
        ...
        for s in steps:
            ...
            if s_id and s_id not in existing_steps:
                ...
                # Infer component
                if 'guard' in s_id:
                    step_mapping[s_id] = 'guard'
                elif 'analyst' in s_id:
                    step_mapping[s_id] = 'analyst'
                elif 'judge' in s_id:
                    step_mapping[s_id] = 'judge'
                elif 'archivist' in s_id:
                    step_mapping[s_id] = 'archivist'
                elif 'retrieve' in s_id:
                    step_mapping[s_id] = 'retrieve_context'
                elif 'report' in s_id:
                    step_mapping[s_id] = 'reporter'
                else:
                    step_mapping[s_id] = 'generic_agent'
    ...
    for s_id in missing_steps:
        ...
        prompts = []
        if comp == 'guard':
            prompts = ['TASK_SECURITY_CHECK']
        elif comp == 'analyst':
            prompts = ['TASK_ANALYST']
        elif comp == 'judge':
