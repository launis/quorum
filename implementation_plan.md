...
1. **Identity Metadata**: Mahdollistaa päivämäärän (paikallista aikaa muodossa `vvvv kk pv hh:mm`), organisaation, käyttäjänimen, pisteytysmoottorin (Scoring Engine) ja arvioinnin tiukkuuden (Strictness) dynaaminen valinta tulostusprofiilista. Otsikkokenttien tulee olla täsmälleen samat ja samassa järjestyksdessä sekä PDF-tulosteessa että UI/Flutter-puolella. Konteksti poistetaan kokonaan tai piilotetaan oletuksena.
...
5. **PDF ja UI Pariteetti**: "Zero-Math" UI:n ja PDF:n yhdenmukaistaminen uusien dynaamisten termien, sarakkeiden ja otsikkokenttien järjestyksen osalta.
