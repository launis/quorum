...
def render_dashboard(data: dict):
    ...
    if scores:
        ...
        score_items = [(k, v) for k, v in scores.items() if not k.endswith("_selitys")]
    ...
    with c_int:
        ...
        if interaction:
            ...
            st.write(
                f"**Kontrollisuhde:** {interaction.get('input_control_ratio', interaction.get('control_ratio', 0))}"
            )
    ...
    with st.expander("🔍 Syväanalyysi (Todisteet, Logiikka, Faktat)", expanded=False):
        ...
        if toulmin or schema or fidelity:
            ...
            if schema:
                st.write(f"**Tunnistettu skeema:** {schema}")
            if fidelity:
                st.write(f"**Päättelyn uskollisuus:** {fidelity}")
            if toulmin:
        ...
        if causal or align:
            ...
            if causal:
                st.write(f"**Kausaalinen päättely:** {causal}")
            if align:
                st.write(f"**Arkiston linjakkuus:** {align}")
