                ...
                if st.session_state.get(f"show_copy_{wf['id']}"):
                    with st.expander(f"Copy '{wf.get('name')}'", expanded=True):
                        new_name = st.text_input(
                            "New Name", value=f"{wf.get('name')} Copy", key=f"copy_name_{wf['id']}"
                        )
    ...
    # --- EDITOR MODE ---
        ...
        with meta_tab:
            ...
            if user and user.get("role") == "ROOT":
                ...
                chk_key = f"chk_public_{wf_data.get('id', 'new')}"
                wf_data["is_public"] = st.checkbox(
                    "Public (System Template)", value=wf_data.get("is_public", False), key=chk_key
                )
            else:
                ...
                if active_step:
                    ...
                    if not step_detail:
                        ...
                        if not is_custom:
                            ...
                            new_prompts = st.multiselect(
                                "Edit Prompt Chain", options=prompt_options, default=current_prompts
                            )
