...
<catastrophic_system_bans>
    ...
    <rule_block id="ban_heuristic_identifier_matching">
        <banned_pattern>Identifying, filtering, resolving, or targeting workflow steps, prompt blocks, matrices, execution artifacts, LLM providers, or credentials in background workers, factories, or services using hardcoded identifier tuples, name markers, keyword lists (`_MARKERS`, `_KEYWORDS`, `_NAMES`), substring searches (`marker in step.name.lower()`), string prefix checks (`model_name.startswith("vertex_ai/")`, `"gemini" in model_name`), negative string filtering (specifically and exhaustively: `not k.startswith(...)`, `not val.startswith("$")`), backward-compatibility transition shims (`allowed_keys`), heterogeneous string collections mixing entity IDs with enums (`allowed_blocks` mixing PromptBlock IDs with TargetBlockType, `step_states` mixing DAG steps and background tasks), or untyped expression strings.</banned_pattern>
        <mandatory_pattern>ALL entity targeting, provider resolution, credential binding, execution, reporting, synthesis, or metric evaluation MUST be explicitly resolved via typed DTO contracts, explicit Enum relations (`provider_type: LLMProvider`), dynamic Studio UI configuration (e.g., `OutputProfile.variance_target_block`), positive ID-set resolution (`k in valid_id_set`), and cleanly separated typed collections. Background workers and provider factories must operate as strictly deterministic executors with zero domain guessing, zero negative string exclusion, or string prefix parsing.</mandatory_pattern>
        <catastrophic_reason>Heuristic substring matching, startswith prefix checks, negative string exclusions, and hardcoded identifier markers break silently whenever workflows or model naming conventions are modernized or renamed, causing downstream synthesis, routing, and PDF generation to crash with missing metrics, misrouted credentials, or leaked context.</catastrophic_reason>
    </rule_block>
