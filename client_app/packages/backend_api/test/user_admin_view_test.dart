import 'package:test/test.dart';
import 'package:backend_api/backend_api.dart';

// tests for UserAdminView
void main() {
  final UserAdminView? instance = /* UserAdminView(...) */ null;
  // TODO add properties to the entity

  group(UserAdminView, () {
    // User email address
    // String email
    test('to test the property `email`', () async {
      // TODO
    });

    // String displayName
    test('to test the property `displayName`', () async {
      // TODO
    });

    // Assigned permission role
    // UserRole role (default value: UserRole.MEMBER)
    test('to test the property `role`', () async {
      // TODO
    });

    // String organizationId
    test('to test the property `organizationId`', () async {
      // TODO
    });

    // Is the account active?
    // bool isActive (default value: true)
    test('to test the property `isActive`', () async {
      // TODO
    });

    // Preferred UI language
    // String language (default value: 'fi')
    test('to test the property `language`', () async {
      // TODO
    });

    // Preferred Theme Mode
    // String themeMode (default value: 'system')
    test('to test the property `themeMode`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String slug
    test('to test the property `slug`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // String createdBy
    test('to test the property `createdBy`', () async {
      // TODO
    });

    // DateTime lastLoginAt
    test('to test the property `lastLoginAt`', () async {
      // TODO
    });

    // int executionCount (default value: 0)
    test('to test the property `executionCount`', () async {
      // TODO
    });
  });
}
