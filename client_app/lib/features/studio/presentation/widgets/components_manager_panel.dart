          ...
          child: Card(
            ...
            child: Column(
              children: [
                ...
                Expanded(
                  child:
                      componentsState.isLoading && !componentsState.hasValue
                          ? const Center(child: CircularProgressIndicator())
                          : componentsState.hasError
                          ? Center(
                            child: Text('Error: ${componentsState.error}'),
                          )
                    ...
                    : _ComponentEditor(
                      ...
                      initialComponent:
                          selectedId.value == 'new'
                              ? null
                              : filteredComponents.firstWhere(
                                (c) => c.id == selectedId.value,
                                orElse:
                                    () => const StudioComponentDef(
                                      id: 'err',
                                      name: 'err',
                                      type: 'err',
                                      content: {},
...
class _ComponentEditor extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final idController = useTextEditingController(
      text: initialComponent?.id ?? '',
    );
    final nameController = useTextEditingController(
      text: initialComponent?.name ?? '',
    );
    final descController = useTextEditingController(
      text: initialComponent?.description ?? '',
    );
    final citationController = useTextEditingController(
      text: initialComponent?.citation ?? '',
    );
    ...
    if (initialComponent != null) {
      if (initialComponent!.content is Map ||
          initialComponent!.content is List) {
        try {
          initialContentStr = const JsonEncoder.withIndent(
            '  ',
          ).convert(initialComponent!.content);
        } catch (_) {
      ...
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text("Component saved!")));
        }
      } catch (e) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
    ...
    Future<void> delete() async {
      ...
      if (confirm == true) {
        try {
          await ref
              .read(componentsControllerProvider.notifier)
              .delete(componentId);
          if (context.mounted) {
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text(
                isNew ? "Create New Component" : "Edit Component",
            ...
            children: [
              Expanded(
                child: DropdownButtonFormField<String>(
                  initialValue:
                      types.contains(selectedType.value)
                          ? selectedType.value
                          : types.first,
                  decoration: const InputDecoration(
                    labelText: "Type",
