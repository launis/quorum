...
class StepsManagerPanel extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final filteredSteps = useMemoized(() {
...
class _StepEditor extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final availableComponentsState = ref.watch(
      availableComponentsControllerProvider,
    );
    ...
    Future<void> save() async {
      if (idController.text.isEmpty || nameController.text.isEmpty) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(l10n.stepIdNameRequired)));
        return;
      }
      ...
      final Map<String, dynamic> newConfig = {"llm_prompts": llmPrompts.value};
      ...
      try {
        ...
        if (isMounted()) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text(l10n.stepSaveSuccess)));
          onSave();
        }
      } catch (e) {
        if (isMounted()) {
      ...
      showDialog(
        ...
        builder: (context) {
          ...
          final validComponents =
              allComponents.where((c) => validTypes.contains(c.type)).toList();
                  ...
                  validComponents
                      .where(
                        (c) =>
                            ...
                            (c.name ?? c.slug ?? '').toLowerCase().contains(
                              search.toLowerCase(),
                            ) ||
                            (c.slug?.toLowerCase().contains(
                                  search.toLowerCase(),
                                ) ??
                                false),
                      )
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Text(
                isNew ? l10n.studioCreateNew : l10n.stepEdit,
          ...
          DropdownButtonFormField<String>(
            value:
                agentOptions.contains(selectedAgent.value)
                    ? selectedAgent.value
                    : agentOptions.first,
            decoration: InputDecoration(
            ...
            if (matricesState.hasValue) ...[
              Builder(
                builder: (context) {
                  ...
                  final currentValue =
                      matrices.any((m) => m.id == selectedMatrixId.value)
          ...
          availableComponentsState.when(
            ...
            data:
                (components) => Wrap(
                  ...
                  runSpacing: 8,
                  children:
                      llmPrompts.value.map((pid) {
                        final comp = components.firstWhereOrNull(
                          (c) => c.id == pid,
                        );
                        ...
                        return InputChip(
                          label: Text(
                            comp.slug ?? comp.name ?? 'Unknown (${comp.id})',
                          ),
                          tooltip: comp.id,
                          onDeleted: () {
                            llmPrompts.value =
