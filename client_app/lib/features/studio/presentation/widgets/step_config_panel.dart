...
final componentSchemaProvider = FutureProvider.family<JsonSchema, String>((
  ref,
  type,
) async {
...
class StepConfigPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final availableComponents =
        ref.watch(availableComponentsControllerProvider).value ?? [];
    final linkedComponents =
        (step!.config['_linked_components'] as List?)?.cast<String>() ?? [];
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    StrategySelectionField(
                      currentStrategy:
                          step!.config['model_strategy'] as String?,
                      onChanged: (val) => updateConfig('model_strategy', val),
                    ),
                    const SizedBox(height: 16),
                    DynamicStepForm(
                      config:
                          step!.config, // Filters out _internal inside widget
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  component.name ?? component.slug ?? component.id,
...
class _OutputAndScoringSection extends ConsumerWidget {
  ...
  const _OutputAndScoringSection({
    required this.step,
    required this.availableComponents,
  });
      ...
      children: [
        ...
        if (workflowDef.scoringLogic.isEmpty)
          const Padding(
            ...
            child: Text(
              "No scoring bars defined. Add one to visualize metrics.",
            ...
            child: ExpansionTile(
              ...
              children: [
                _RulesTable(
                  ...
                  onUpdate:
                      (newLogic) =>
                          _updateLogic(ref, workflowDef, index, newLogic),
                ),
                Align(
  ...
  Future<void> _addLogic(WidgetRef ref, WorkflowDef workflow) async {
    ...
    final newWf = workflow.copyWith(
      scoringLogic: [...workflow.scoringLogic, newLogic],
    );
                  ...
                  cells: [
                    DataCell(
                      DropdownButton<String>(
                        ...
                        onChanged: (v) {
                          if (v != null)
                            _updateRule(idx, rule.copyWith(componentId: v));
                        },
                      ),
                    ),
                    DataCell(
                      TextFormField(
                        ...
                        onFieldSubmitted:
                            (v) =>
                                _updateRule(idx, rule.copyWith(metricKey: v)),
                      ),
                    ),
                    DataCell(
                      SizedBox(
                        ...
                        child: TextFormField(
                          ...
                          onFieldSubmitted: (v) {
                            ...
                            if (d != null)
                              _updateRule(idx, rule.copyWith(weight: d));
                          },
                        ),
                      ),
                    ),
