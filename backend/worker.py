...
async def execute_workflow_job(ctx: Any, workflow_id: str, inputs: dict, execution_id: str | None = None) -> dict:
    ...
    """
    logger.info(f"[Job] Executing workflow: {workflow_id} (Execution ID: {execution_id})")
    ...
    try:
        ...
        if execution_id:
             await repository.update_execution(execution_id, {
                 "status": "completed",
                 "results": result,
...
async def startup(ctx: Any) -> None:
    ...
    setup_logging()
    configure_logfire()
    logger.info("Arq Worker starting (V2.9 GraphEngine Mode)...")
    ...
    try:
        ...
        import backend.tasks.legacy_migration_tasks  # noqa
