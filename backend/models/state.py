...
class WorkflowState(BaseModel):
    ...
    step_logician: Annotated[
        Optional[ArgumentaatioAnalyysi], Field(description="Agent 3: Logical Structure Analysis.")
    ] = None
    step_falsifier: Annotated[
        Optional[LogiikkaAuditointi], Field(description="Agent 4: Stress Testing & Falsification.")
    ] = None
    step_overseer: Annotated[Optional[EtiikkaJaFakta], Field(description="Agent 5: Ethics & Fact Checking.")] = None
    step_causal: Annotated[
        Optional[KausaalinenAuditointi], Field(description="Agent 6: Causal & Counterfactual Analysis.")
    ] = None
    step_detector: Annotated[
        Optional[PerformatiivisuusAuditointi], Field(description="Agent 7: Performativity & Authenticity.")
    ] = None
    ...
    step_interaction: Annotated[
        Optional[InteractionAnalysis], Field(description="Agent 2.2: Interaction Dynamics.")
    ] = None
    ...
    def to_flat_dict(self) -> Dict[str, Any]:
        ...
        flat["System_Status"] = {
            ...
            "uhka_havaittu": self.step_guard.security_check.uhka_havaittu
            if (self.step_guard and self.step_guard.security_check)
            else None,
            "riski_taso": self.step_guard.security_check.riski_taso
            if (self.step_guard and self.step_guard.security_check)
            else None,
            "logiikka_validi": not (
                self.step_falsifier.paattelyketjun_uskollisuus_auditointi.onko_post_hoc_rationalisointia
            )
        ...
        if self.step_profiler:
            report["psykologinen_profiili"] = {
                ...
                "vinoumat": [b.model_dump() for b in self.step_profiler.tunnistetut_vinoumat]
                if self.step_profiler.tunnistetut_vinoumat
                else [],
        ...
        if not sources_to_process:
            if self.step_judge:
                sources_to_process.append(self.step_judge)
            if self.step_judge_cognitive:
                sources_to_process.append(self.step_judge_cognitive)
        ...
        if self.step_analyst:
            if self.step_analyst.hypoteesit:
                report["analyysi_hypoteesit"] = [h.model_dump() for h in self.step_analyst.hypoteesit]
            if self.step_analyst.rag_todisteet:
                report["analyysi_todisteet"] = [r.model_dump() for r in self.step_analyst.rag_todisteet]

        if self.step_logician:
            if self.step_logician.toulmin_analyysi:
                report["logiikka_toulmin"] = [t.model_dump() for t in self.step_logician.toulmin_analyysi]
            if self.step_logician.walton_skeema:
                report["logiikka_skeema"] = self.step_logician.walton_skeema.tunnistettu_skeema
        ...
        if self.step_panel:
            if (
                self.step_panel.performatiivisuus_auditointi
                and self.step_panel.performatiivisuus_auditointi.pre_mortem_analyysi
            ):
                # Override or set if not present (Panel is newer info often)
                report["pre_mortem_analyysi"] = (
                    self.step_panel.performatiivisuus_auditointi.pre_mortem_analyysi.model_dump()
                )

            if self.step_panel.logiikka_auditointi:
                if self.step_panel.logiikka_auditointi.toulmin_analyysi:
                    report["logiikka_toulmin"] = [
                        t.model_dump() for t in self.step_panel.logiikka_auditointi.toulmin_analyysi
                    ]
                if self.step_panel.logiikka_auditointi.walton_skeema:
                    report["logiikka_skeema"] = self.step_panel.logiikka_auditointi.walton_skeema.tunnistettu_skeema
            ...
            if (
                self.step_panel.falsifiointi_auditointi
                and self.step_panel.falsifiointi_auditointi.paattelyketjun_uskollisuus_auditointi
            ):
                report["logiikka_uskollisuus"] = (
                    self.step_panel.falsifiointi_auditointi.paattelyketjun_uskollisuus_auditointi.uskollisuus_score
                )

            if self.step_panel.etiikka_ja_fakta:
                if self.step_panel.etiikka_ja_fakta.faktantarkistus_rfi:
                    report["faktatarkistus"] = [
                        f.model_dump() for f in self.step_panel.etiikka_ja_fakta.faktantarkistus_rfi
                    ]
        ...
        raw_steps_dict = {
            "step_guard": self.step_guard.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_guard
            else None,
            "step_analyst": self.step_analyst.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_analyst
            else None,
            "step_interaction": self.step_interaction.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_interaction
            else None,
            "step_profiler": self.step_profiler.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_profiler
            else None,
            "step_logician": self.step_logician.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_logician
            else None,
            "step_falsifier": self.step_falsifier.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_falsifier
            else None,
            "step_causal": self.step_causal.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_causal
            else None,
            "step_detector": self.step_detector.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_detector
            else None,
            "step_overseer": self.step_overseer.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_overseer
            else None,
            "step_archivist": self.step_archivist.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_archivist
            else None,
            "step_judge": self.step_judge.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_judge
            else None,
            "step_judge_cognitive": self.step_judge_cognitive.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_judge_cognitive
            else None,
            "step_coach": self.step_coach.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_coach
            else None,
            "step_panel": self.step_panel.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_panel
            else None,
            "step_reporter": self.step_reporter.model_dump(exclude=noise_fields, exclude_none=True)
            if self.step_reporter
            else None,
        }
