...
class MatrixComponentResponse(AgentBaseResponse):
    """Strict model for Evaluation Matrices (Dict content)."""
...
class TextComponentResponse(AgentBaseResponse):
    ...
    def validate_known_types(cls, v: str) -> str:
        ...
        if v in ("agent", "processor"):
             raise ValueError(f"Type '{v}' cannot be a TextComponentResponse (must use AgentComponentResponse and have NO content).")
        return v

# Polymorphic Union
# Pydantic will validate against these in order or by discriminator if we added one.
# Since 'content' type varies (None vs Dict vs List vs Str), this is safe.
ComponentResponse = AgentComponentResponse | MatrixComponentResponse | ConfigComponentResponse | TextComponentResponse
...
class WorkflowConfigDefinition(BaseModel):
    ...
    steps: Annotated[list[dict[str, Any]] | list[StepDefinition], Field(description="Hydrated steps or references")] = []
...
class WorkflowConfigUpdate(BaseModel):
    ...
    default_model_mapping: Annotated[dict[str, str] | None, Field(description="Map of StepID -> ModelStrategyKey.")] = None
