...
class GraphEngine:
    ...
    async def execute_workflow(
        ...
        repository: Any | None = None,  # AbstractWorkflowRepository
            ...
            try:
                ...
                if record and record.results:
                    ...
                    if hasattr(record.results, "model_dump"):
                        persisted_dict = record.results.model_dump(mode="json")
                    else:
                    ...
                    if persisted_dict:
                        ...
                        current_dump = execution_state.model_dump(mode="json")
                ...
                if val and isinstance(val, str) and ("chat" in field or "history" in field):
                    ...
                    try:
                        ...
                        raise AppException(
                             ...
                             status_code=status.HTTP_400_BAD_REQUEST,
                             details={"error_code": ErrorCodes.INVALID_JSON_PAYLOAD.value, "field": field, "original_error": str(e)}
                         ) from e
            ...
            try:
                ...
                if repository:
                    try:
                        ...
                        msg = f"Step hydration failed for '{step.id}' (SSOT Error). Cannot continue."
                        logger.error(f"[GraphEngine] {msg}: {e}")
                        raise AppException(
                            message=msg,
                ...
                if not task_def:
                     raise AppException(
                         ...
                         details={"error_code": ErrorCodes.TASK_NOT_FOUND.value, "task_key": step.task_key}
                     )
                ...
                if repository and execution_id:
                    try:
                        ...
                        state_dict = execution_state.model_dump(mode="json")
                 ...
                 try:
                     ...
                     logger.critical(f"[GraphEngine] CATASTROPHIC FATAL: Failed to append ErrorEvent to State: {state_err}")
                     raise AppException(
                         message="Fatal State Machine Corruption: Could not append error event.",
                         status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                         details={"error_code": ErrorCodes.INTERNAL_SERVER_ERROR.value, "original_error": str(state_err)}
                     ) from state_err
                ...
                logger.error(f"[GraphEngine] {error_code.name}: Workflow failed at step '{step.id}': {e}", exc_info=True)

                # Create Error Event
                error_event = TraceEvent(
                    step_name=step.id,
                    event_type="error",
                    content={"error": str(e), "code": error_code.value},
                    metadata={"timestamp": datetime.now(timezone.utc).isoformat()},
                )
                try:
                    execution_state = execution_state.add_event(error_event)
                except Exception as state_err:
                    logger.critical(f"[GraphEngine] CATASTROPHIC FATAL: Failed to append generic ErrorEvent to State: {state_err}")
                    raise AppException(
                         message="Fatal State Machine Corruption: Could not append generic error event.",
                         status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                         details={"error_code": ErrorCodes.INTERNAL_SERVER_ERROR.value, "original_error": str(state_err)}
                     ) from state_err
                ...
                raise WorkflowExecutionError(
                    ...
                    original_error=e,
                    details={"execution_state": failed_state_dump, "error_code": error_code.value},
                ) from e
        ...
        return execution_state.model_dump(mode="json")
    ...
    async def _execute_hook(self, hook_name: str, state: WorkflowState, repository: Any) -> WorkflowState:
        ...
        try:
            ...
            raise AppException(
                ...
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                details={"error_code": ErrorCodes.HOOK_EXECUTION_FAILED.value, "original_error": str(e)}
            ) from e
                ...
                try:
                    ...
                    resolved[target_field] = value
                     ...
                     raise AppException(
                         ...
                         status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                         details={"error_code": ErrorCodes.UNKNOWN_ERROR.value, "original_error": str(e)}
                     )
            else:
                # Static value
