...
def format_matrix_component(component: dict[str, Any]) -> str:
    ...
    for crit in criteria:
        ...
        prompt_lines.append(
            f"**JSON Requirement**: You MUST use the exact ID '{c_id}' as the value for 'dimension_id' in your output."
        )
        ...
        if scale_min != anchor_min or scale_max != anchor_max:
            prompt_lines.append(
                f"  [PISTEYTYSOHJE]: Käytä Ankkuritasoja ({anchor_min}-{anchor_max}) vain LAADULLISINA KIINTOPISTEINÄ."
            )
            prompt_lines.append(f"  - Asteikko on {scale_min}-{scale_max}.")
            prompt_lines.append(
                f"  - Suoritus, joka vastaa 'Tasoa {anchor_min}', implikoi pisteitä lähellä arvoa {scale_min}."
            )
            prompt_lines.append(
                f"  - Suoritus, joka vastaa 'Tasoa {anchor_max}', implikoi pisteitä lähellä arvoa {scale_max}."
            )
            ...
            if scale_range > 0:
                prompt_lines.append(
                    f"  - Käytä KOKO ASTEIKKOA ({scale_min}-{scale_max}) kuvastaaksesi nyansseja. ÄLÄ rajoitu vain ankkuripisteisiin."
                )

            if scale_range > 10:
                prompt_lines.append(
                    f"    * Esimerkki: Pisteet {(scale_min + scale_max * 3) // 4} ovat validit, jos suoritus on Tason 3 ja Tason 4 välissä."
                )
                prompt_lines.append(
                    "    * Tarkkuusperiaate: Mitä laajempi asteikko, sitä tarkempaa arviointia vaaditaan."
                )
