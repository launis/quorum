...
def test_models_only():
    ...
    try:
        ...
        event = TraceEvent(
            step_name="step_judge", event_type="output", content={"total_score": 3.5}, timestamp=datetime.now(UTC)
        )
        print("   TraceEvent OK")

        # 2. WorkflowState
        print("2. Creating WorkflowState...")
        state = WorkflowState(execution_id=uuid4(), workflow_id="wf-test", status="completed", execution_trace=[event])
