...
def test_register_agent_instantiation_failure():
    ...
    with pytest.raises(AppException) as exc:
        TaskRegistry.register_agent(
            task_keys=["broken_task"],
            agent_cls=BrokenAgent,
...
async def test_agent_wrapper_configuration_failure():
    ...
    TaskRegistry.register_agent(
        task_keys=["config_fail_task"],
        agent_cls=MockAgent,
        output_model=MockOutput
    )
    
    wrapper = TaskRegistry.get("config_fail_task").handler
...
async def test_agent_wrapper_model_override_failure():
    ...
    TaskRegistry.register_agent(
        task_keys=["override_fail_task"],
        agent_cls=MockAgent,
        output_model=MockOutput
    )
    
    wrapper = TaskRegistry.get("override_fail_task").handler
