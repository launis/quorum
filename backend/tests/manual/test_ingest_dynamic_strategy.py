...
async def test_dynamic_strategy():
    print("Initializing dependencies for Dynamic Strategy Test...")
    ...
    try:
        ...
        print(f"Verified 'fast' config: {config_fast}")

        # Manually trigger the logic that happens inside extract_concepts_with_llm
        from backend.llm.provider import LLMFactory
        provider_fast = LLMFactory.create_provider(
             provider_type=config_fast["provider"],
             model_name=config_fast["model_name"],
             usage_service=usage
        )
        print(f"VERIFIED: 'fast' strategy resolves to provider model: {provider_fast.model_name}")
    except Exception as e:
        print(f"FAILED 'fast' test: {e}")

    # Test 2: Resolve 'deep' strategy
    print("\n--- TEST 2: Resolve 'deep' Strategy ---")
    try:
        config_deep = await registry.resolve_model_config("deep")
        print(f"Verified 'deep' config: {config_deep}")

        provider_deep = LLMFactory.create_provider(
             provider_type=config_deep["provider"],
             model_name=config_deep["model_name"],
             usage_service=usage
        )
        print(f"VERIFIED: 'deep' strategy resolves to provider model: {provider_deep.model_name}")
    except Exception as e:
        print(f"FAILED 'deep' test: {e}")
