...
from backend.models.domain.xai import XAIOutput, XAIReporterInput


# Mock the BaseAgent.execute to avoid LLM calls
class MockXAIReporterAgent(XAIReporterAgent):
    async def execute(
        self, input_data: XAIReporterInput, execution_context: dict[str, Any] | None = None, system_instruction: str | None = None, **kwargs
    ) -> XAIOutput:
...
class TestXAIFlatGeneration(unittest.TestCase):
    ...
    def test_flat_report_generation(self, mock_base_execute):
        ...
        input_data = XAIReporterInput.model_validate({"step_judge": step_judge_data})
