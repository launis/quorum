...
def test_llm_provider_config_validation():
    ...
    with pytest.raises(ValidationError):
        LLMProviderConfig(id="test/fail", provider="openai", model_name="gpt-4", tpm_limit=-1, rpm_limit=1000)

    with pytest.raises(ValidationError):
        LLMProviderConfig(id="test/fail", provider="openai", model_name="gpt-4", tpm_limit=100000, rpm_limit=-1)
