...
def apply_scoring_logic(state: WorkflowState) -> WorkflowState:
    ...
    if security_threat:
        ...
        if p_val > 0:
            ...
            penalties.append(f"Security Threat Detected (-{p_val*100:.0f}%)")
             ...
             logger.warning(f"[ScoringHook] Security Threat Detected (Logged Only - Penalty Disabled in Settings)")
    ...
    if is_post_hoc:
        ...
        if p_val > 0:
            ...
            penalties.append(f"Post-Hoc Rationalization Detected (-{p_val*100:.0f}%)")
             ...
             logger.warning(f"[ScoringHook] Post-Hoc Rationalization Detected (Logged Only - Penalty Disabled in Settings)")
    ...
    result = ScoringResult(
        ...
        calculated_average=average_score,
...
def enforce_scoring_penalties(result: Any, context: Any) -> Any:
    ...
    logger.info(f"[ScoringHook] Penalties applied: {penalties}. Factor: {penalty_factor:.2f}. Score {current_score} -> {new_score}")
    
    # 4. Return Updated Model (Immutable Update)
    updates = {"penalties": penalties}
    ...
    for judge_key in ["step_judge", "step_judge_cognitive"]:
        ...
        for dim in score_card.dimensions:
            ...
            if dim.score <= score_card.scale_min:
                 ...
                 logger.warning(f"[ScoringHook] Passive/Low Quality detected in {judge_key} dimension '{dim.dimension_id}'")
                 break
        ...
        if penalty_triggered:
            ...
            if new_score < score_card.scale_min:
                 logger.warning(f"[ScoringHook] Passivity penalty reduced score ({new_score}) below min ({score_card.scale_min}). Clamping.")
                 new_score = score_card.scale_min

            # Functional Update
            new_card = score_card.model_copy(update={"total_score": new_score, "verdict": score_card.verdict + f" [PASSIVITY PENALTY x{multiplier:.2f}]"})
