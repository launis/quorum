...
def verify_structure(state: WorkflowState) -> WorkflowState:
    ...
    if not state.context_variables:
        ...
        raise AppException(message=msg, status_code=400, details={"error_code": ErrorCodes.EMPTY_INPUT})
    ...
    try:
        ...
        result = ValidationResult(is_valid=len(warnings) == 0, errors=warnings)
    ...
    if not result.is_valid:
        ...
        raise AppException(
            message=msg, status_code=400, details={"error_code": ErrorCodes.VALIDATION_FAILED, "warnings": warnings}
        )
    else:
