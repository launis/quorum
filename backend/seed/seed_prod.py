...
def main():
    ...
    print(f"[INFO] Target: PROD DB (data/db.json)")
    print(f"[WARNING] This will WIPE and RE-POPULATE the local production database.")
    print(f"[ACTION] Running backend.seed.seeder...\n")

    # 3. Run the Module
    try:
        subprocess.run(
            [sys.executable, "-m", "backend.seed.seeder"],
            cwd=project_root,
            env=env,
            check=True
        )
