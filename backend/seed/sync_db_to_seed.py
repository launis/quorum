...
def main():
    ...
    print(f"[INFO] Source: PROD DB (data/db.json)")
    print(f"[INFO] Target: SEED FILE (backend/seed/seed_data.json)")
    print(f"[ACTION] Running backend.seed.syncer...\n")

    # 3. Run the Module
    try:
        subprocess.run(
            [sys.executable, "-m", "backend.seed.syncer"],
            cwd=project_root,
            env=env,
            check=True
        )
