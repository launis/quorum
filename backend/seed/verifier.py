...
def run_verification(seed_path: Path, db_prod_path: Path, db_mock_path: Path, firestore_creds_path: Path = None):
    ...
    SYNCED_TABLES = [
        'organizations', 'users', 'system_config', 'components',
        'steps', 'workflows', 'knowledge_base', 'dimensions',
        'banned_phrases', 'model_registry'
    ]
    ...
    print(f"LATEST UPDATES DETECTED:")
    ...
    if firestore_creds_path and firestore_diffs:
        if ts_fire > ts_seed:
             recommendations.append("[WARN] Firestore is NEWER than Seed. No auto-sync script exists. Manually update seed_data.json?")
