...
class PromptCompiler:
    ...
    def compile_markdown_rubrics(
        ...
        """
        return self._localization_compiler.compile_markdown_rubrics(criteria, target_locale, execution_persona_block)
    ...
    def build_markdown_context(
        ...
        """Build Markdown semantic blocks from raw input mappings for LLM context.
        ...
        Returns:
            A single string containing Markdown-formatted context blocks.
        """
        md_blocks = []
    ...
    def _extract_value_from_state(self, path: str, state_data: dict[str, Any]) -> str:
        ...
        if isinstance(current, dict):
            ...
            for k, v in current.items():
                formatted.append(f"### MATRIX INPUT: {str(k).upper()}")
                    ...
                    for sub_k, sub_v in target_dict.items():
                        ...
                        if isinstance(sub_v, dict):
                            ...
                            for micro_k, micro_v in sub_v.items():
                                ...
                                formatted.append(f"**{clean_key}:** {TemplateProcessor.encapsulate_payload(micro_v)}")
                        else:
                            clean_sub_k = str(sub_k).title().replace("_", " ")
                            formatted.append(f"**{clean_sub_k}:** {TemplateProcessor.encapsulate_payload(sub_v)}")
                else:
    ...
    def compile_chunk_payload_instruction(self, chunk_id: str, payload_text: str) -> str:
        """Generates an isolated context block fenced into a `## USER PAYLOAD` Markdown block.
