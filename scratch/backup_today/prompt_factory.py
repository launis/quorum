...
class PromptPayload:
    ...
    Attributes:
        ...
        user_payload: Dynamic runtime parameters and source data Markdown contexts.
...
class PromptFactory:
    ...
    def build(
        ...
        if is_grounded_step:
            ...
            anchors_md = "## MECHANICAL ANCHORS\n"
            anchors_md += "### TEXT METRICS\n"
            anchors_md += f"**Word Count:** {word_count}\n"
            anchors_md += f"**Say-Do Gap:** {say_do_gap}\n"
            anchors_md += f"**Automation Bias:** {automation_bias}\n"
            anchors_md += "### DETECTED PERFORMATIVE PHRASES\n"
            anchors_md += f"**Phrase Count:** {phrase_count}\n"
            anchors_md += f"**Items:**\n{items_md}"
        ...
        exec_params = f"## EXECUTION CONTEXT\n**Target Locale:** {target_locale}\n"
        exec_params += f"{linguistic_ctx}\n"
        if execution_time:
            exec_params += f"**Document Date:** {execution_time}\n"

        md_ctx = compiler.build_markdown_context(
