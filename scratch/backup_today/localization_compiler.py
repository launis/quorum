...
Extracts and centralizes translation resolution, Markdown rubric compilation, and
...
class LocalizationCompiler:
    ...
    def compile_markdown_rubrics(
        ...
        """Epic 12/55: Generates Thick Markdown rubrics for the System Prompt with Persona SSOT.
        ...
        Returns:
            A formatted Markdown string of rubrics for the system prompt.
        """
        md_blocks = []
        if execution_persona_block and execution_persona_block.ai_description:
            md_blocks.append(f"## EXECUTION PERSONA\n{execution_persona_block.ai_description}\n")

        md_blocks.append("## EVALUATION RUBRICS")
        for crit in criteria:
            ...
            md_blocks.append(f"### MATRIX: {label} (ID: {crit_id})")
            if desc:
                md_blocks.append(f"**Directive:** {desc}")
                ...
                for s in scales:
                    for c in s.claims:
                        for assertion in c.tda_assertions:
                            ...
                            assertion_md = []
                            assertion_md.append(f"#### Rule (ID: {assertion.tda_id})")
                            if assertion.concept_description:
                                assertion_md.append(f"**Concept Description:** {assertion.concept_description}")
                            tda_block = (
                                "**TDA Validation:**\n"
                                f"- **Anchor Target:** {assertion.anchor_target}\n"
                                f"- **Search Scope:** {assertion.bounding_box_scope}\n"
                                f"- **Validation Rule:** {assertion.extraction_rule}"
                            )
                            assertion_md.append(tda_block)

                            acs = assertion.acceptance_criteria
                            if acs:
                                ac_lines = [f"  - {ac.instruction}" for ac in acs if ac.instruction]
                                if ac_lines:
                                    assertion_md.append("**Acceptance Criteria:**\n" + "\n".join(ac_lines))

                            aps = assertion.anti_patterns
                            if aps:
                                ap_lines = [f"  - {ap.pattern}" for ap in aps if ap.pattern]
                                if ap_lines:
                                    assertion_md.append("**Anti-Patterns:**\n" + "\n".join(ap_lines))
                            ...
                            if getattr(assertion, "contrastive_example", None):
                                assertion_md.append(
                                    ...
                                    "> **WARNING:** These are HYPOTHETICAL concepts. DO NOT extract quotes from this section.\n\n"
                                    f"**Example:** {assertion.contrastive_example}"
                                )
                            ...
                            assertion_md.append(f"**Fail-Fast Mandate:** {mandate_text}")

                            claims_texts.append("\n".join(assertion_md))

                if claims_texts:
                    claims = "\n\n".join(claims_texts)
                    md_blocks.append(f"**Critical Directives:**\n{claims}")
    ...
    def compile_static_instructions(self, blocks: list[PromptBlock], target_locale: str) -> str:
        ...
        for block in blocks:
            if block.category_id != "matrix" and block.category_id != "runtime_variables":
                ...
                if label or desc:
                    compiled_lines.append(f"### STATIC INSTRUCTION: {label}\n{desc}\n")
    ...
    def compile_dynamic_instructions(
        ...
        for block in blocks:
            if block.category_id == "runtime_variables":
                ...
                if label or desc:
                    compiled_lines.append(f"### DYNAMIC INSTRUCTION: {label}\n{desc}\n")
