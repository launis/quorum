...
class PromptCompilerAdapter:
    ...
    def compile_chunk_prompt(
        ...
        target_locale: str,
        chunk_atoms_md: str | None = None,
        task_instruction: str = (
            "Analyze the provided `## SOURCE DATA` and execute the extraction strictly according to instructions."
        ),
        ...
        if content_cache_enabled:
            dynamic_parts.append(f"## SYSTEM INSTRUCTIONS\n{base_system_prompt.strip()}\n")
        ...
        local_md_rubrics = self._compiler._localization_compiler.compile_markdown_rubrics(
            chunk_criteria, target_locale, allowed_atom_ids=allowed_atom_ids
        )
        if local_md_rubrics:
            dynamic_parts.append(f"## EVALUATION CRITERIA\n{local_md_rubrics}\n")

        # 2. Chunk-specific atoms (NOT cacheable)
        if chunk_atoms_md:
            dynamic_parts.append(chunk_atoms_md)

        # 3. Task instruction
        dynamic_parts.append(f"## TASK\n{task_instruction}")
        ...
        if previous_errors:
            for err in previous_errors:
                dynamic_parts.append(f"## PREVIOUS SCHEMA ERROR\n{err}\n")
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                if in_dynamic_tail:
                    ...
                    has_exec_params = content_str and "## EXECUTION PARAMETERS" in content_str
                    has_prev_error = content_str and "## PREVIOUS SCHEMA ERROR" in content_str
                    ...
                    if not has_exec_params and not has_prev_error:
                        ...
                        exec_params_match = (
                            re.search(r"(## EXECUTION PARAMETERS.*?)(?=\n## |\Z)", content_str, re.DOTALL)
                            if content_str
                            else None
                        )
                        # Isolate previous errors block
                        prev_error_match = (
                            re.search(r"(## PREVIOUS SCHEMA ERROR.*?)(?=\n## |\Z)", content_str, re.DOTALL)
                            if content_str
                            else None
                        )
