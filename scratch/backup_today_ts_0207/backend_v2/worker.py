...
async def generate_profile_synthesis_and_pdf_task(
    ...
    try:
        ...
        # Execute Text Consolidation Hook (Now extracted from DAG output)
        await _update_render_status("Luetaan tekoälysynteesiä (tämä saattaa kestää verkosta riippuen)...")

        # 1. Map task_blueprint slugs to step IDs from the workflow definition
        ...
        if workflow_def:
            for s in workflow_def.steps:
                ...
                if step_def:
                    slug = step_def.get("slug")
                    if slug == "sp_synthesis_llm":
                        synthesis_step_id = s.id
                    elif slug == "sp_row_explanations":
                        row_exp_step_id = s.id

        # 2. Extract payloads from trace
        ...
        for evt in execution.execution_trace:
            if evt.event_type == "output":
                if evt.step_name == synthesis_step_id:
                    synthesis_payload = evt.content
                    if evt.metadata and "token_usage" in evt.metadata:
                        usage_synthesis = evt.metadata["token_usage"]
                elif evt.step_name == row_exp_step_id:
