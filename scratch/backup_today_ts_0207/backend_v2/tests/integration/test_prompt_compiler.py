...
def test_inverse_logic_injected() -> None:
    ...
    rubrics = compiler.compile_markdown_rubrics([block], target_locale="en")

    # Assert standard rule fields are present in the new Markdown format
    assert "- **Anchor Target:** The summary" in rubrics
    assert "- **Validation Rule:** Must be nice" in rubrics

    # Assert inverse logic rule ID is present in Markdown rubrics
    ...
    assert "**Rule Calibration Examples:**" in rubrics
    assert "**Example:** This is a contrastive example text." in rubrics
