...
def test_epic90_prompt_integration_holistic() -> None:
    ...
    wf = next((w for w in data.get("workflows", []) if w["id"] == "wf_9d68c573802341db"), None)
    assert wf is not None, "Workflow wf_9d68c573802341db missing"
    assert wf["default_profile_id"] == "prf_5d6e7f8091a2b3c4", "Workflow default_profile_id mismatch"

    # 2. Build Markdown Rubrics using LocalizationCompiler (inside PromptCompiler)
    compiler = PromptCompiler()

    # Note: Execution persona is passed to compile_markdown_rubrics
    rubrics_md = compiler.compile_markdown_rubrics(
        criteria=[], target_locale="fi", execution_persona_block=persona_block
    )

    # Assert persona was injected correctly
    assert "## EXECUTION PERSONA" in rubrics_md
    assert persona_block.ai_description is not None
    assert persona_block.ai_description in rubrics_md, "Persona ai_description was not injected into rubrics"
