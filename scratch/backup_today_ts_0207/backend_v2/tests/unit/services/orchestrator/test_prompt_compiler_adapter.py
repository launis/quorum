...
def test_prompt_compiler_adapter_compile_chunk_prompt() -> None:
    ...
    assert "## SOURCE DATA" in prompt.static_messages[1]["content"]
    ...
    assert "## EVALUATION CRITERIA" in prompt.dynamic_messages[0]["content"]
    assert "### STRICTNESS CALIBRATION" in prompt.dynamic_messages[0]["content"]
    assert "## PREVIOUS SCHEMA ERROR" in prompt.dynamic_messages[0]["content"]
...
def test_prompt_compiler_adapter_compile_prompt_fallback() -> None:
    ...
    assert "## SOURCE DATA\nVerbatim content\n" in static_user["content"]
    assert "## TASK\nExecute task." in static_user["content"]
    assert "## EXECUTION PARAMETERS" not in static_user["content"]
    assert "## PREVIOUS SCHEMA ERROR" not in static_user["content"]

    # 3. Dynamic blocks are isolated in dynamic
    dynamic_user = prompt.dynamic_messages[0]
    assert dynamic_user["role"] == "user"
    assert "## EXECUTION PARAMETERS" in dynamic_user["content"]
    assert "## PREVIOUS SCHEMA ERROR" in dynamic_user["content"]
    assert "## SOURCE DATA" not in dynamic_user["content"]
...
def test_content_cache_enabled_demotes_system_to_user() -> None:
    ...
    demoted from native 'system' role to '## SYSTEM INSTRUCTIONS' within a 'user' message.
    ...
    assert "## SOURCE DATA" in prompt.static_messages[0]["content"]
    # System prompt MUST NOT appear in static tier
    assert system_prompt not in prompt.static_messages[0]["content"]

    # Dynamic tier: System prompt demoted inside ## SYSTEM INSTRUCTIONS header
    dynamic_content = prompt.dynamic_messages[0]["content"]
    assert "## SYSTEM INSTRUCTIONS" in dynamic_content
...
def test_content_cache_modes_are_structurally_incompatible() -> None:
    ...
    # Content divergence: Epic 80 dynamic tier contains ## SYSTEM INSTRUCTIONS header
    epic80_dynamic = prompt_epic80.dynamic_messages[0]["content"]
    v3_dynamic = prompt_v3.dynamic_messages[0]["content"]
    assert "## SYSTEM INSTRUCTIONS" in epic80_dynamic
    assert "## SYSTEM INSTRUCTIONS" not in v3_dynamic
