...
def test_compile_markdown_rubrics_basic() -> None:
    compiler = LocalizationCompiler()
    mock_criteria = [
        {
    ...
    result = compiler.compile_markdown_rubrics(
        [PromptBlock.model_validate(c) for c in mock_criteria], target_locale="en"
    )

    assert "## EVALUATION RUBRICS" in result
...
def test_compile_dynamic_instructions() -> None:
    ...
    assert "### DYNAMIC INSTRUCTION" in result
