...
def test_build_markdown_context() -> None:
    compiler = PromptCompiler()
    state = {"a": "value_a", "b": {"c": "value_c"}}
    md = compiler.build_markdown_context({"src_1": "a", "src_2": "b.c"}, state, "en")
    assert "### MATRIX INPUT: src_1" in md
    assert "value_a" in md
    assert "### MATRIX INPUT: src_2" in md
    assert "value_c" in md
