...
def test_build_linguistic_context_default_source() -> None:
    ...
    assert "- Source Data Language: Unknown/Original" in result
    assert "- Required Output Language: fi" in result
    assert "- Internal Thought Language: English" in result
...
def test_build_linguistic_context_custom_source() -> None:
    ...
    assert "- Source Data Language: de" in result
    assert "- Required Output Language: sv" in result
    assert "- Internal Thought Language: English" in result
...
def test_build_linguistic_context_md_structure() -> None:
    """Verify proper Markdown header wrapping and mandate inclusion."""
    result = build_linguistic_context(target_locale="en", include_mandate=True)

    assert result.startswith("## LINGUISTIC CONTEXT")
