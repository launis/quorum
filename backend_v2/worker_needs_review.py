...
async def evaluate_chunk_job(
    ...
    Args:
        ...
        step_id: Workflow step ID context.
        chunk_index: The numerical index of the current chunk.
        total_chunks: Overall count of split blocks.
        file_path: Optional storage driver file reference.
        chunk_items: Primitives or payload parts.
        criteria_blocks_dump: Dict representation of PromptBlock criteria.
        base_system_prompt: AI contextual guidelines.
        has_search: Feature toggle for SERP evaluation.
        has_shuffled_atoms: Evaluator integrity toggle.
        atom_to_block_ids: Matrix projection maps.
        effective_mcp_tools: Available tool names.
        target_locale: Language preference string.
        synthesis_instructions: Profile driven AI instructions.
        strictness_level: Output rigidness.
        step_metadata: Execution node metrics.

    Raises:
        AppException: If job execution is routed to DLQ or completely fails.
    """
    ...
    if isinstance(c_final, dict) and c_final.get("_dlq_status") == "FAILED/DLQ":
        reason = str(c_final["reason"]) if "reason" in c_final else "Unknown DLQ Failure"
        logger.error(
...
async def execute_workflow_job(
    ...
    with logfire.span("execute_workflow_job", tags={"execution_id": execution_id or "unknown"}):
        ...
        if organization_id:
            inputs.setdefault("organization_id", organization_id)
        ...
        try:
            ...
            if exec_id:
                ...
                if redis:
                    ...
                    await redis.enqueue_job("render_profile_job", exec_id, None, profile_id)
            ...
            logger.warning(f"[Job] Workflow {workflow_id} CANCELLED (Timeout/Shutdown). Execution ID: {exec_id}")
            if exec_id:
                try:
                    await repository.update_execution(
                        exec_id,
...
async def generate_pdf_job(
    ...
    Args:
        ctx: Task state map.
        execution_id: Traceable ID context.
        accept_language: User selected localized text mode.
        profile_id: Configuration pointer.

    Returns:
        Message indicating success state.
    """
...
async def generate_pdf_task(
    ...
    Args:
        execution_id: Targeted trace sequence.
        accept_language: Fallback locale parsing code.
        profile_id: Presentation structural definition.

    Raises:
        Exception: Propagates internal errors wrapped dynamically.
    """
    ...
    try:
        ...
        transformer = BlueprintTransformer(exec_repo=repo, workflow_repo=repo, comp_repo=repo, identity_repo=repo)
...
async def render_profile_job(
    ...
    Returns:
        Confirmation completion status message.
    """
...
async def generate_profile_synthesis_and_pdf_task(
    ...
    Args:
        execution_id: Active tracing flow pointer.
        accept_language: Explicit locale to map.
        profile_id: Visual blueprint model index.
        redis: Context instance of Redis pool connection.

    Raises:
        Exception: Propagates error up if compilation fails completely.
    """
...
async def startup(ctx: Any) -> None:
    ...
    Args:
        ctx: Contextual Arq global worker map.
    """
    try:
        setup_logging()
        configure_logfire()
    except TypeError:
        # Bypass for unit tests where settings might be a PydanticDescriptorProxy
        pass
...
async def shutdown(ctx: Any) -> None:
    ...
    Args:
        ctx: Context pointer during unregister stage.
    """
...
async def health_check(ctx: Any) -> str:
    ...
    Args:
        ctx: Worker mapping variable pool.

    Returns:
        Constant string marking system ok.
    """
    return "OK"
...
class WorkerSettings:
    ...
    Attributes:
        functions: List of exposed worker async callables.
        on_startup: Lifecyle startup hook implementation.
        on_shutdown: Lifecycle teardown hook logic.
        redis_settings: Configuration object pointing queue state engine.
        job_timeout: Threshold length logic limits.
        max_jobs: Core execution boundary for thread scaling.
    """
