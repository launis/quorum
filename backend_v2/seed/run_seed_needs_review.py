...
def _fail_fast(msg: str, error: Exception) -> None:
    """Log critical errors and raise specialized AppException compliant with RFC 7807."""
    logger.error(
        "[Seeder] %s: [CRITICAL FAIL FAST] %s",
...
async def _seed_tinydb(db_path: str, seed_data: dict[str, Any], target_env: str) -> None:
    """Seeding operations for TinyDB database instance."""
    try:
...
def batch_upsert(db: Any, collection_name: str, items: list[dict[str, Any]], id_field: str = "id") -> None:
    """Upsert documents into Firestore in batches."""
    batch = db.batch()
    count = 0
    total = 0
    for item in items:
...
def _delete_collection(coll_ref: Any, batch_size: int = 50) -> None:
    """Prune target collection strictly."""
...
async def _seed_firestore(seed_data: dict[str, Any], target_env: str) -> None:
    """Seeding operations for Google Firestore backend infrastructure."""
    ...
    for col_key, config in STANDARD_REGISTRY.items():
        ...
        batch_upsert(db, col_key, valid_items, id_field=id_field)


async def seed_database(target: str) -> None:
    """Process seeding for designated environment targets (local/firestore)."""
    ...
    if not os.path.exists(SEED_PATH):
        ...
        raise AppException(
            ...
            details={"error_code": ErrorCodes.FILE_NOT_FOUND.name},
        )
...
def main() -> None:
    """Command-line entry point for database seeder tool execution."""
