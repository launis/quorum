...
class SynthesisOutputDTO(BaseModel):
    ...
    cited_sources: list[str] = Field(
        default_factory=list, description="List of references or citations found."
    )
...
async def text_consolidation_hook(state: HookState, deps: HookDependencies) -> HookResult:
    ...
    for k, v in inputs.items():
        if omit_empty and (v is None or v == "" or v == [] or str(v).strip() == "" or str(v).strip().lower() in ["null", "none", "n/a", "ei saatavilla"]):
    ...
    sys_prompt += "You are a professional report synthesizer. Merge, deduplicate, and seamlessly synthesize the provided information into cohesive markdown.\n"
    if length_constraint:
         sys_prompt += f"LENGTH CONSTRAINT: The output should be approximately {length_constraint} characters.\n"
    if preamble:
         sys_prompt += f"PREAMBLE INTRODUCTION: Start your synthesis intuitively using the following preamble tone/context: '{preamble}'\n"
    sys_prompt += "Omit internal system identifiers or raw JSON keys. Cite your sources using inline tags [1], [2] if data originates from external tools."
    ...
    try:
         ...
         with logfire.span("text_consolidation_hook") as span:
             ...
             step_metadata = state.step_metadata or {}
