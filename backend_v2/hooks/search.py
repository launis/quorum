...
async def execute_google_search_hook(state: HookState, deps: HookDependencies) -> HookResult:
    ...
    # V2 Isolation: If running as a post-hook, 'inputs' is already the local step dictionary.
    analyst_output = state.global_context_vars.get("step_analyst", state.inputs)
    ...
    try:
        ...
        return HookResult(success=True, state_delta={"search_result": {"results": [i for i in domain_items]}})
