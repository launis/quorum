...
def _map_llm_extensions_to_domain(raw_data: Any) -> Any:
    ...
    and the strict LightweightMatrixOutput domain schema, delegating the actual Enum hydration
    to Pydantic V2 native aliases.
    """
...
async def enforce_passivity_penalty_hook(state: HookState, deps: HookDependencies) -> HookResult:
    ...
    for judge_key, judge_model, is_post_hook in judges_to_check:
        ...
        for k in matrix_blocks_meta.keys():
            if k in judge_model:
                try:
                    matrix_dto = LightweightMatrixOutput.model_validate(_map_llm_extensions_to_domain(judge_model[k]))
        ...
        for k, matrix_dto in matrix_keys:
            ...
            if matrix_dto.raw_score <= math_min:
    ...
    try:
        ...
        for pb_id, stats in block_scale_stats.items():
            ...
            if global_total == 0:
                ...
                if not execution_data.scoring_strategy:
                    msg = "Strict Fail-Fast Enforced: ExecutionRecord missing 'scoring_strategy'."
                    logger.error("[ScoringHook] %s: %s", ErrorCodes.VALIDATION_FAILED.name, msg)
                    raise AppException(
                        message=msg, status_code=500, details={"error_code": ErrorCodes.VALIDATION_FAILED.value}
                    )
                raw_strategy = execution_data.scoring_strategy.value

                try:
                    engine = get_scoring_engine(raw_strategy)
                    dampening_score, calculation_log, level_breakdown = engine.calculate(
                        stats, math_min, math_max, strictness_level=execution_data.strictness_level
                    )
            ...
            if pb_id in new_payload:
                ...
                if isinstance(raw_data, dict) and "raw_score" in raw_data and "normalized_score" in raw_data:
                    try:
                        mapped_data = _map_llm_extensions_to_domain(raw_data)
                    ...
                    justification = existing_matrix.justification if existing_matrix.justification else calculation_log
                    ...
                    raw_data["raw_score"] = float(dampening_score)
                    norm_val = (dampening_score / 5.0) * 100.0 if dampening_score <= 5.0 else dampening_score
                    raw_data["normalized_score"] = float(norm_val)

                    try:
                        temp_parsed = LightweightMatrixOutput.model_validate(_map_llm_extensions_to_domain(raw_data))
                        extensions = temp_parsed.extensions
                        justification = calculation_log
                        if temp_parsed.justification:
                            justification += "\n\n" + temp_parsed.justification
                    except ValidationError as e:
            ...
            normalized_score = (
                float((dampening_score / 5.0) * 100.0) if dampening_score <= 5.0 else float(dampening_score)
            )
...
async def normalize_matrix_scores_hook(state: HookState, deps: HookDependencies) -> HookResult:
    ...
    try:
        ...
        for pb_id in prompt_block_ids:
            ...
            try:
                parsed_payload = LightweightMatrixOutput.model_validate(_map_llm_extensions_to_domain(raw_input_val))
            except Exception as e:
