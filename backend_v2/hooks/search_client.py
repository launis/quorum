        ...
        for i, query in enumerate(queries[:limit]):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    error_msg = str(e)
                    is_quota_error = (
                        "429" in error_msg or "Quota exceeded" in error_msg or "RESOURCE_EXHAUSTED" in error_msg
                    )
                    ...
                    logger.error(
                        f"[VertexAISearchTool] {error_code}: Grounding failed for '{query}': {e}", exc_info=True
                    )
