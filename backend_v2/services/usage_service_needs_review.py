...
class UsageService:
    ...
    def __init__(self, identity_repo: IIdentityRepository, audit_repo: IAuditRepository) -> None:
        """Initialize the UsageService.

        Args:
            identity_repo: Repository for identity lookups.
            audit_repo: Repository for usage/audit persistence.
        """
    ...
    async def track_usage(
        ...
        Args:
            ...
            cost_usd: The cost calculated by LiteLLM.
            cached_tokens: Number of tokens read from the cache.
            cache_creation_input_tokens: Number of tokens written to the cache.
            estimated_savings_usd: The estimated amount saved due to cache hits.
            reasoning_tokens: Dedicated reasoning token count (o1/o3/DeepSeek).
            latency_ms: Call latency in milliseconds.
            finish_reason: Terminating condition string.
            system_fingerprint: Unique generation backend environment fingerprinted hash.
            provider_name: Canonical model orchestrating engine provider.
            model_pricing_config: Model configuration prices metadata.
        ...
        """
        try:
            # Deferred import for heavy AI module
            ...
            # Utilizing secrets module for ID generation standard (Rule 26)
                ...
                if hasattr(self.audit_repo, "get_usage_records"):
                    ...
                    if len(records) == 5:
                        ...
                        total_all_tokens = sum(
                            (r.input_tokens + r.output_tokens + (r.cached_tokens or 0)) for r in records
                        )
    ...
    async def check_quota(self, org_id: str) -> bool:
        ...
        Args:
            org_id: The target unique identifier for organization entity.

        Returns:
            True if SAFE (under limit), False if EXCEEDED.
    ...
    async def get_usage_report(self, scope: str, entity_id: str | None = None, since: str | None = None) -> UsageReport:
        """Generate a token consumption aggregate report across the desired tracking boundary.

        Args:
            scope: Aggregation category (e.g., 'org' or 'user').
            entity_id: Identifier of target tracking entity.
            since: Optional ISO 8601 start date bound.

        Returns:
            UsageReport: Instantiated reporting metrics DTO.
        """
        period = "all-time"
        if since:
            now = datetime.now(UTC)
