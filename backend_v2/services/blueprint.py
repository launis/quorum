...
class BlueprintTransformer:
    ...
    async def build_report_dto(
        ...
        execution_dict = execution.model_dump() if hasattr(execution, "model_dump") else {}
        section_syntheses = {}
        if isinstance(execution_dict, dict):
            section_syntheses = execution_dict.get("section_syntheses") or {}
        elif hasattr(execution_dict, "section_syntheses"):
            section_syntheses = execution_dict.section_syntheses or {}
        ...
        try:
            for layout_def in layout_defs:
                ...
                if axes or preset_view == "text_only":
                    ...
                    # Hash string dict instead of I18nText object directly
                    title_str = str(layout_title.model_dump()) if hasattr(layout_title, "model_dump") else str(layout_title)
