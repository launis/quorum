...
class ChatParserService:
    ...
    async def parse_pasted_chat(raw_paste: str, repository: Any) -> ChatHistoryDTO:
        ...
        RULES:
        1. Separate human ('user') and AI ('ai') messages from the text. The 'role' MUST be either 'user' or 'ai'.
        2. CRITICAL - PDF PRINTOUT RECONSTRUCTION: The user input might be a browser printout (Print to PDF) of a ChatGPT conversation that PyMuPDF has shredded into a single long string of text, destroying all visual boundaries.
           - Look for repeating identifiers like a person's name ("You", full name, initials) vs "ChatGPT", "AI".
           - Even without these, YOU MUST INFER the turn changes from context: human messages are typically questions or prompts (e.g., "Do this next...", "What do you mean by..."), followed by structured informational text generated by the machine.
           - Merge the fragmented text seamlessly back together under each proper role ('user' or 'ai'), restoring the original question-answer rhythm. Prefer overly long blocks over overly fragmented ones.
        3. Completely ignore all UI garbage text (e.g., "Regenerate", "Copy code", timestamps, "Was this response better or worse?", side menus, profile names).
        4. Assumption: messages alternate. If the text starts with a human question, the first 'role' is 'user'.
        5. Return the data EXACTLY in the requested Pydantic JSON format.
        6. WARNING (FAIL-SAFE): If the raw text does not look like a conversation in any way (e.g., it is just a messy learning diary or essay without dialogue), NEVER return an empty list. In this emergency, place all the text as one single long message under the 'user' role.
