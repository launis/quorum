"""Service logic module for managing and processing LLM execution chunks.

Isolates the physical LLM interaction, MCP tool loops, and caching logic.
"""
...
class AtomIdentifier(BaseModel):
    """Strict model for identifying an execution atom."""
...
class ExtractionPayload(BaseModel):
    """Payload container for extraction evaluations."""
...
def evaluate_extraction(extraction: BaseModel, source_text: str, is_negative_rule: bool) -> str:
    ...
    Args:
        extraction: The extraction payload model.
        source_text: Raw context text to validate against.
        is_negative_rule: Flag indicating if the rule represents a negative condition.

    Returns:
        String indicating validation status (PASS, FAIL, or DLQ).
    """
        ...
        async with sem:
            ...
            try:
                if effective_mcp_tools:
                    ...
                    est_tokens = step_metadata.get("estimated_token_count", 0) if step_metadata else 0
                    ...
                    if chunk is not None:
                        for item in chunk.items:
                            aid = (
                                ...
                                else getattr(item, "atom_id", None)
                            )
                        ...
                        if is_shuffled and "evaluations" in merged:
                            ...
                            for idx in range(num_evals):
                                ...
                                if votes:
                                    ...
                                    quotes = [v.get("exact_quote", None) for v in votes]
