...
class AtomizationSchema(BaseModel):
    ...
    micro_atoms: list[str] = Field(
        ...
        min_length=15,
        max_length=15,
        description="Exactly 15 distinct micro-atoms extracted from the claim. Domain-specific vocabulary MUST be obfuscated. Use Scaffolded exception mechanisms for special terms."
    )
    rubric_cot: str = Field(
        ...,
        description="Rubric-CoT reasoning to explain how these atoms preserve the original meaning to prevent Context Drift."
    )
...
class PromptAtomizer:
    ...
    @classmethod
    async def atomize_prompt_block(cls, block: PromptBlock, repository: Any = None, is_test: bool = False) -> PromptBlock:
        ...
        system_prompt = (
            ...
            "4. Provide a 'Rubric-CoT' reasoning to explain how these atoms preserve the original meaning to prevent Context Drift."
        )
