    ...
    def compress_synthesis_payload(
        ...
        def _prune_and_stratify_evaluations(evals: list[dict[str, Any]], limit: int) -> list[dict[str, Any]]:
            ...
            for item in evals:
                ...
                is_unmet = extensions.get("is_unmet", False) if isinstance(extensions, dict) else False
                ext_status = extensions.get("status") if isinstance(extensions, dict) else None

                if (
                    status in ("FAILED", "UNMET", "NON_COMPLIANT")
                    or ext_status in ("FAILED", "UNMET", "NON_COMPLIANT")
                    or is_unmet is True
                ):
        ...
        def _normalize_result_item(item: dict[str, Any]) -> dict[str, Any]:
            if "exact_quotes" in item:
                try:
                    ...
                    logger.warning(
                        "[SynthesisPayloadCompressor] %s: Normalization fallback for result item: %s",
                        ErrorCodes.VALIDATION_FAILED.name,
                        str(e),
            ...
            if isinstance(obj, dict):
                ...
                if "evaluations" in obj:
                    ...
                    if not isinstance(evals, list):
                        logger.error(
                            "[SynthesisPayloadCompressor] %s: 'evaluations' must be a list.",
                            ErrorCodes.VALIDATION_FAILED.name,
                        )
                        raise AppException(
                    ...
                    if not lite_evals:
                        logger.error(
                            "[SynthesisPayloadCompressor] %s: Evaluations list cannot be empty after compression.",
                            ErrorCodes.VALIDATION_FAILED.name,
                        )
                        raise AppException(
                    ...
                    obj["evaluations"] = _prune_and_stratify_evaluations(lite_evals, settings.max_synthesis_evaluations)
                elif "results" in obj:
                    results_list = obj["results"]
                    if isinstance(results_list, list):
                        obj["results"] = [
                            _normalize_result_item(item) if isinstance(item, dict) else item for item in results_list
                        ]
