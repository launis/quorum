...
class EvaluationCriterion(V2CoreBase):
    ...
    def validate_weight(cls, v: float) -> float:
        ...
        Raises:
            AppException: If weight is < 0.0.
        """
        if v < 0.0:
            msg = "weight must be >= 0.0"
...
class EvaluationMatrixConfig(V2CoreBase):
    ...
    criteria: Annotated[
        ...
        Field(min_length=1, json_schema_extra={"x-ui-group": "Evaluation Criteria"}),
    ]
...
class EvaluationResult(ReasoningTrace):
    ...
    matrix_id: Annotated[str, Field(min_length=1)]
    timestamp: datetime
    total_score: Annotated[float, Field(description="Total score.")]
    final_verdict: Annotated[str, Field(min_length=1, description="Final verdict.")]
    dimensions: Annotated[list[DimensionResultItem], Field(min_length=1, description="Evaluation dimensions.")]

    # Scale Metadata (Added for XAI/BFF Compatibility)
    scale_min: Annotated[float, Field(description="Minimum possible score.")]
    scale_max: Annotated[float, Field(description="Maximum possible score.")]

    # Container for aggregated results (if applicable)
    score_cards: Annotated[
        list[JudgeScoreCard] | None,
        Field(description="List of score cards if this results aggregates multiple."),
    ] = None

    # Citation Support (Restored per User Request)
    citation_snippets: Annotated[
        list[str],
        Field(
...
class ValidationResult(V2CoreBase):
    ...
    errors: Annotated[
        list[str],
        Field(default_factory=list, description="Validation errors.", json_schema_extra={"x-ui-label": "Errors"}),
    ]
