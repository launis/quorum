...
MATRIX_SENSOR_SYSTEM_PROMPT: str = (
    ...
    "- SUBSTANTIVE HEDGING CRITERIA: A negative claim is refuted (is_true = false) ONLY if the author provides substantive, context-specific delimitation (specifically: explicit boundary conditions, empirical counter-arguments, or defined scope limitations).\n"
    "- BANNED META-HEDGING: Generic disclaimers, boilerplate statements of uncertainty, superficial hedging words (specifically: merely adding 'maybe' or 'possibly' to an ungrounded absolute claim), or global declarations of epistemic humility do NOT negate an active logical fallacy or methodological error.\n"
    ...
    "- SUBSTANTIATED EVIDENCE VS. SUBJECTIVE UNCERTAINTY: When evaluating whether a required capability, analytical structure, decision, or methodology is present in the context (positive claim), the physical presence of that substantive work constitutes definitive positive evidence. An author's retrospective expressions of uncertainty, hesitation, process difficulty, or modesty elsewhere in the text do NOT negate the physically demonstrated presence of the required work, UNLESS the author explicitly repudiates the work through disclaiming authorship, stating the content is unvetted, or contradicting the outcome.\n"
    "- SELF-EVALUATION SCOPE: If and only if a claim's concept description explicitly evaluates an author's internal self-awareness, personal feelings, or bias disclosure, expressions of subjective uncertainty or self-critique constitute the direct target of evaluation.\n"
    "- DOCUMENT DIRECTIVE HIERARCHY: When inputs are demarcated with <ai_context_directive>, respect the designated role of each document (specifically: source deliverable vs. process dialogue vs. retrospective reflection) and do not conflate meta-cognitive commentary with primary operational output.\n"
    "</epistemic_hierarchy_protocol>\n"
