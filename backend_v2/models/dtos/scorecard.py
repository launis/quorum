...
class MatrixScorecardRowDTO(BaseModel):
    ...
    missing_context: str = Field(..., description="Any missing context from evaluation.")
...
class ScorecardResponseDTO(BaseModel):
    ...
    global_average: float | None = Field(
        default=None, description="The mathematical average extracted from normalized evaluative matrices."
    )

    evaluative_matrices: list[MatrixScorecardRowDTO] = Field(..., description="Matrices that impact the final grade.")
