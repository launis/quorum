...
class FirestoreWorkflowRepository(AbstractWorkflowRepository):
    ...
    def _serialize_for_firestore(self, data: dict[str, Any] | list[Any] | Any) -> Any:
    ...
    async def get_execution_status(self, execution_id: str) -> str | None:
        ...
        if doc.exists:
            status_val = doc.to_dict().get("status")
            return str(status_val) if status_val else None
        return None
    ...
    async def get_all_workflows(
        ...
        if role != "ROOT":
            # Basic RBAC: filter by org if provided, otherwise assume user
            # only sees own org unless specified handled by caller
            if organization_id:
    ...
    async def update_workflow(self, workflow_id: str, updates: dict[str, Any]) -> str:
        """V2 Append-Only Update strategy.
        1. Fetch old document.
        2. Set old doc is_latest to False.
        3. Create new document with new ID (_vX) and is_latest True.
        """
        doc_ref = self.db.collection("workflows").document(workflow_id)
        doc = await doc_ref.get()
        if not doc.exists:
        ...
        # Calculate new version number based on ID slug
        base_id = workflow_id.split("_v")[0]
        version = 1
        if "_v" in workflow_id:
            try:
                version = int(workflow_id.split("_v")[1]) + 1
            except ValueError:
                version = 2
        else:
            version = 2
        ...
        new_data["slug"] = base_id

        safe_new = self._serialize_for_firestore(new_data)
        safe_old_update = self._serialize_for_firestore({"is_latest": False})

        try:
            # Batch update in Firestore
            batch = self.db.batch()
            batch.update(doc_ref, safe_old_update)

            new_doc_ref = self.db.collection("workflows").document(new_id)
            batch.set(new_doc_ref, safe_new)
    ...
    async def update_agent(self, agent_id: str, updates: dict[str, Any]) -> bool:
        """V2 Append-Only update strategy for agents."""
        ...
        if "_v" in agent_id:
            try:
                version = int(agent_id.split("_v")[1]) + 1
            except ValueError:
                version = 2
        else:
            version = 2
        ...
        try:
            ...
            new_doc_ref = self.db.collection("agents").document(new_id)
            batch.set(new_doc_ref, safe_new)
    ...
    async def update_matrix(self, matrix_id: str, updates: dict[str, Any]) -> bool:
        ...
        try:
            ...
            new_doc_ref = self.db.collection("matrices").document(new_id)
            batch.set(new_doc_ref, safe_new)
    ...
    async def update_component_metadata(self, component_id: str, module: str, component_class: str) -> bool:
        """Update legacy_prompt_block metadata in Firestore."""
        try:
            await (
            ...
            logger.error(f"Failed to update legacy_prompt_block metadata {component_id}: {e}")
            return False

    async def register_component(self, component_data: dict[str, Any]) -> str:
        """Register a new legacy_prompt_block."""
    ...
    async def delete_component(self, component_id: str) -> bool:
        """Delete a legacy_prompt_block."""
        try:
            await self.db.collection("components").document(component_id).delete()
            return True
        except Exception as e:
            logger.error(f"Failed to delete legacy_prompt_block {component_id}: {e}")
            return False
    ...
    async def get_model_registry(self) -> dict[str, Any]:
        ...
        logger.error(
            "[FirestoreRepository] SYSTEM_CONFIG_NOT_FOUND: 'model_registry' "
            "document is missing from database."
        )
    ...
    async def get_components_using_dimension(self, dimension_id: str) -> list[str]:
        """Return list of legacy_prompt_block IDs that reference this dimension."""
