...
@pytest.mark.asyncio
async def test_get_active_model_registry_success(handler, mock_repo):
...
@pytest.mark.asyncio
async def test_get_active_model_registry_corrupt(handler, mock_repo):
    mock_repo.get_model_registry.return_value = {"invalid": "data"}
    with pytest.raises(ConfigurationError):
        await handler.get_active_model_registry()

@pytest.mark.asyncio
async def test_get_model_config(handler):
...
async def test_call_llm_success(mock_create_provider, mock_get_settings, handler):
...
@patch("backend_v2.llm.handler.get_settings")
def test_fetch_all_available_models_mock(mock_get_settings, handler):
    mock_settings = MagicMock()
    mock_settings.vertex_location = "us-central1"
...
@pytest.mark.asyncio
@patch("backend_v2.llm.handler.get_settings")
async def test_call_llm_disabled_model(mock_get_settings, handler):
    mock_settings = MagicMock()
    mock_settings.vertex_location = "us-central1"
    mock_get_settings.return_value = mock_settings
