...
def mock_repos() -> dict[str, Any]:
    ...
    workflow_repo.get_step_by_id = AsyncMock(
        side_effect=lambda bp_id: {
            ...
            "slug": f"slug_{bp_id}",
            "name": {"default_locale": "en", "translations": {"en": f"Step {bp_id}"}},
...
async def test_dag_executor_mcp_concurrency_deduplication(mock_repos: dict[str, Any]) -> None:
    ...
    workflow = Workflow(
        ...
        name=I18nText(default_locale="en", translations={"en": "Concurrency Test"}),
        description=I18nText(default_locale="en", translations={"en": "Concurrency Test"}),
        version=1,
        status="active",
        default_profile_id="prof_1111111111111111",
        allowed_exports=["pdf"],
...
async def test_dag_executor_generated_schemas_accumulation(mock_repos: dict[str, Any]) -> None:
    ...
    workflow = Workflow(
        ...
        name=I18nText(default_locale="en", translations={"en": "Schema Test"}),
        description=I18nText(default_locale="en", translations={"en": "Schema Test"}),
        version=1,
        status="active",
        default_profile_id="prof_1111111111111111",
        allowed_exports=["pdf"],
