...
from backend_v2.models.v2_core import Workflow, WorkflowInputs, StepRule, I18nText, ExecutionStatus, SystemConfigModelRegistry, ModelProfile
...
def mock_compiler() -> MagicMock:
    compiler = MagicMock()
    compiler.compile_llm_prompt = AsyncMock(return_value="This is a sufficiently long mocked prompt to pass the fail-fast check")
    compiler.compile_prompt_block = AsyncMock(return_value="This is a sufficiently long mocked prompt to pass the fail-fast check")
    return compiler
...
async def test_dag_executor_atom_ceiling(mock_repo: MagicMock, mock_compiler: MagicMock) -> None:
    ...
    workflow = Workflow(
        id="wf_1234567890abcdef", slug="test", status="draft", version=1,
        default_profile_id="prof_1234567890abcde", name=I18nText(default_locale="en", translations={"en": "test"}),
        description=I18nText(default_locale="en", translations={"en": "test"}),
        steps=[
            StepRule(id="stp_1234567890abcdef", task_blueprint="blp_1234567890abcdef", input_mappings={}, depends_on=[], engine_override=EngineOverrideStrategy.PRE_HYDRATED_SYNTHESIS)
        ],
    )
    ...
    model_registry_data = SystemConfigModelRegistry(
        ...
        type="model_registry",
        slug="models",
        models={"test_strategy": ModelProfile(provider="openai", model_name="gpt-4o", tpm_limit=40000, rpm_limit=100, temperature=0.0, max_tokens=4000)}
