...
async def test_execute_with_expected_inputs_and_source_document_packer(
    ...
    step.input_mappings = {"chat_log": "$inputs.chat_log", "product_text": "$inputs.product_text"}
    step.allowed_mcp_tools = []
...
@pytest.mark.asyncio
async def test_execute_with_step_scoped_inputs_filtering(llm_strategy: LLMNodeStrategy, mock_repo: MagicMock) -> None:
    """Verify LLMNodeStrategy.execute scopes global_source_text strictly based on step.input_mappings."""
    from unittest.mock import AsyncMock, patch
    ...
    step_a = MagicMock()
    step_a.id = "step_a"
    step_a.task_blueprint = "bp_scoped"
    step_a.input_mappings = {"product_text": "$inputs.product_text"}
    step_a.allowed_mcp_tools = []
    ...
    with (
        ...
        mock_pre.return_value = (mock_hook_state, [])
        mock_post_hook_state = MagicMock()
        mock_post_hook_state.inputs = ExecutionInputsDTO(dynamic_inputs={"output": "Scoped execution"})
        mock_post.return_value = (mock_post_hook_state, [])
        ...
        assert "chat_log" not in source_text_a
        assert "Strategic question." not in source_text_a

    # Case B: step maps only $steps (e.g. synthesis node) -> global_source_text is empty string
    step_b = MagicMock()
    step_b.id = "step_b"
    step_b.task_blueprint = "bp_scoped"
    step_b.input_mappings = {"prior_analysis": "$steps"}
    step_b.allowed_mcp_tools = []
    ...
    step_c = MagicMock()
    step_c.id = "step_c"
    step_c.task_blueprint = "bp_scoped"
    step_c.input_mappings = {}
    step_c.allowed_mcp_tools = []
