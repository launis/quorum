...
async def test_process_raw_inputs_text_decoding() -> None:
    ...
    raw_inputs = {"file_1": {"filename": "notes.txt", "content_base64": b64_content}}

    await service.process_raw_inputs(raw_inputs)

    # The base64 blob should be destroyed and replaced with decoded text
    assert raw_inputs["file_1"] == original_text  # type: ignore
...
async def test_process_raw_inputs_pdf_extraction() -> None:
    ...
    raw_inputs = {"file_1": {"filename": "document.pdf", "content_base64": b64_content}}
    ...
    assert raw_inputs["file_1"] == "# Extracted PDF Content"  # type: ignore
