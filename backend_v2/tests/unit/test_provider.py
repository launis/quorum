...
from backend_v2.exceptions import ConfigurationError, AppException
from backend_v2.llm.provider import LLMFactory, MockProvider, LiteLLMProvider
...
def test_llm_factory_create_mock() -> None:
    ...
    provider = LLMFactory.create_provider(
        provider_type="mock",
        model_name="mock",
        limits={"tpm": 100, "rpm": 100}
    )
    assert isinstance(provider, MockProvider)


def test_litellm_provider_init_missing_limits():
...
def test_litellm_provider_init_missing_tpm():
...
def test_litellm_provider_init_success():
...
@pytest.mark.asyncio
async def test_litellm_generate_missing_config():
...
async def test_litellm_generate_success(mock_getenv):
    ...
    provider = LiteLLMProvider(model_name="gemini/gemini-1.5-pro", limits={"tpm": 100, "rpm": 100}, settings=mock_settings)
...
def test_llm_factory_create_litellm():
...
def test_llm_factory_missing_model_name():
...
def test_llm_factory_with_config():
...
def test_llm_factory_grounding_unsupported():
