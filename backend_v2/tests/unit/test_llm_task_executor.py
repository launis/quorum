...
@pytest.mark.asyncio
async def test_execute_structured_task_retry_on_schema_error(mock_prompt_compiler: MagicMock, mock_client: AsyncMock) -> None:
    executor = LLMTaskExecutor(prompt_compiler=mock_prompt_compiler)
...
@pytest.mark.asyncio
async def test_execute_structured_task_max_schema_retries_exceeded(mock_prompt_compiler: MagicMock, mock_client: AsyncMock) -> None:
    executor = LLMTaskExecutor(prompt_compiler=mock_prompt_compiler)
...
@pytest.mark.asyncio
async def test_execute_structured_task_stuck_loop_detection(mock_prompt_compiler: MagicMock, mock_client: AsyncMock) -> None:
    executor = LLMTaskExecutor(prompt_compiler=mock_prompt_compiler)
    
    # Identical payloads will trigger stuck loop immediately on the second attempt
    error1 = LLMSchemaValidationError(raw_llm_payload="identical bad json", validation_error_msg="syntax error", is_eof=False)
    error2 = LLMSchemaValidationError(raw_llm_payload="identical bad json", validation_error_msg="syntax error", is_eof=False)
...
@pytest.mark.asyncio
async def test_execute_structured_task_logical_error_retry(mock_prompt_compiler: MagicMock, mock_client: AsyncMock) -> None:
    executor = LLMTaskExecutor(prompt_compiler=mock_prompt_compiler)
    expected_model = MockResponseSchema(value="fixed logic")
    
    # The client returns valid model, but validator_hook fails
    async def mock_validator(model: BaseModel) -> None:
