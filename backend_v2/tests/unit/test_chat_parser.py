...
@pytest.mark.asyncio
async def test_chat_parser_empty_input_fails_fast(mock_repository):
...
async def test_chat_parser_role_segregation_and_success(mock_from_strategy, mock_repository):
