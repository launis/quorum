...
def valid_workflow_data_for_synthesis() -> dict[str, Any]:
    return {
        ...
        "expected_inputs": [],
        "steps": [],
        "system_audit_trail": True,  # Enables system audit trail logic
    }
...
async def test_synthesis_audit_trail_block_type(
    ...
    class DummyToolRes:
        ...
        # Setup one trace item to trigger audit trail collection
        audit_traces = [
            MCPAuditTrace(
                tool_id="test_tool",
                step_name="synthesis",
    ...
    # The synthesis hook should NO LONGER append an alert_box for the system audit trail.
    # The UI layer (Flutter/PDF) will render the audit trail natively from the execution data.
    blocks = result.state_delta["content_blocks"]
    assert len(blocks) == 1
    assert blocks[0]["block_type"] == "paragraph"
    assert "Happy summary" in blocks[0]["text"]
