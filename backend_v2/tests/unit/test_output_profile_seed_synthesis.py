    ...
    for profile in profiles:
        ...
        if synthesis is not None:
            ...
            if not synthesis_block_id and not row_block_id:
                missing_references.append(f"Profile '{profile_id}' ({slug})")

    assert not missing_references, (
        "Fail-Fast: The following OutputProfiles are missing 'synthesis_block_id' and 'row_explanations_block_id': "
