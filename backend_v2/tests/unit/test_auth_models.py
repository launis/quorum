...
def test_organization_fail_fast_on_corrupt_datetime() -> None:
    ...
    assert "Invalid datetime 'invalid_date_string'" in str(exc_info.value)
...
def test_organization_fail_fast_on_corrupt_subscription() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        Organization.model_validate(data)
    assert "Invalid subscription status 'LIFETIME_HACKER'" in str(exc_info.value)
...
def test_user_fail_fast_on_corrupt_datetime() -> None:
    ...
    assert "Invalid datetime '30.02.2025'" in str(exc_info.value)
...
def test_user_fail_fast_on_corrupt_role() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        User.model_validate(data)
    assert "Invalid role 'SUPER_ADMIN_GOD'" in str(exc_info.value)
