...
async def test_atomize_prompt_block_success(mock_llm_client_class, sample_block):
    # Setup mock LLM Client
    mock_client = AsyncMock()

    # Needs exactly 15 micro_atoms to satisfy Pydantic Schema
    mock_res = AtomizationSchema(
        micro_atoms=[f"atom_{str(i).zfill(10)}" for i in range(15)],
        rubric_cot="Test Rubric"
    )
...
@pytest.mark.asyncio
@patch("backend_v2.services.orchestrator.atomizer.LLMClient")
async def test_atomize_prompt_block_missing_en_translation(mock_llm_client_class, sample_block):
