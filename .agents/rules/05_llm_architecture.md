...
<architectural_invariants>
    ...
    <rule_block id="atom_batching_vs_api_call_cardinality">
        <mandate>NEVER assume that evaluating N matrix atoms requires N discrete LLM API requests, or diagnose workflow execution latency as a pacing delay bottleneck. Phase 1 DAG execution groups atoms into batched/chunked payloads (Enriched Atom Graph / `sensor_batch_size: 15` / sliding windows), resulting in only 5–15 discrete LLM calls per full workflow execution. Static provider pacing delays (1–4s) contribute under 5% to total run duration (<50s out of a 15–30 min run). Over 95% of execution latency is driven by model inference latency (reasoning/thinking token generation in models like GPT-5.4) and multi-turn interactive MCP tool loops (20–70 web search roundtrips). NEVER propose speculative token-bucket throttlers or rate-limit overhauls based on the false premise of per-atom network calls.</mandate>
    </rule_block>
</architectural_invariants>
