...
<architectural_invariants>
    ...
    <rule_block id="ban_anonymous_state_tuples">
        <mandate>NEVER use anonymous multi-value tuples (3+ elements or 2+ elements of the same primitive type, e.g. `tuple[ExecutionStatus, str | None, str | None, dict[str, str]]`), positional index access (`item[0]`, `res[1]`), or positional tuple destructuring (`a, b, c = ...`) for domain state transit, callback returns, or service layer data exchange ("Tuple Hell"). ALWAYS encapsulate multi-field return states and intermediate pipeline payloads into strictly typed, immutable Pydantic V2 DTOs (`ConfigDict(extra="forbid", strict=True, frozen=True)`). 2-tuples are permitted EXCLUSIVELY for standard (Payload, TokenUsage) or (Key, Value) pairs at low-level utility boundaries.</mandate>
    </rule_block>
