...
> [!CAUTION]
> **VERY IMPORTANT RULE:** The live development database (TinyDB) must NEVER be modified directly or on-the-fly (`db_v2.json` or `.db`). Modifying `backend_v2/seed/seed_data.json` autonomously is STRICTLY BLOCKED without a safety net. Bypassing these instructions corrupts the system IDs permanently.
...
4. **SCRIPTING RULES (Mandatory):** Create a dedicated Python script file (e.g., `modify_seed.py`) to systematically update the JSON. 
   - 🚫 **NEVER** use inline terminal commands (like `python -c` or `sed`) because PowerShell/Bash will silently expand variables like `$c1f...` and destroy the Stripe UUIDs.
   - 🚫 **NEVER** use raw string replacement or regex on the JSON file directly.
   - ✅ **ALWAYS** use proper parsing: use `json.load()` to parse the dict, mutate the Python dictionary intelligently in-memory, and use `json.dump()` to save it.
   - 🚫 **NEVER** add undocumented "extra keys" or hallucinated data structures. Strictly observe Pydantic domain schemas.
   - 🚫 **All new IDs** MUST strictly follow the Opaque ID (Stripe Pattern) rule (see `Arkkitehtuuristandardi_Tietokannan_Tunnisteet.md`). Do not invent human-readable words in IDs.
