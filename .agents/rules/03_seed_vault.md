...
<vault_mutation_protocol>
    ...
    <step id="2_backup">BACKUP: Store a precise timestamped backup copy inside `backend_v2/seed/backups/` using `run_command` with PowerShell (`New-Item -ItemType Directory -Force -Path backend_v2/seed/backups; Copy-Item backend_v2/seed/seed_data.json -Destination backend_v2/seed/backups/seed_data_<timestamp>.json`) BEFORE making any modifications.</step>
    <step id="3_modify">MODIFY: Commit structural edits safely into `backend_v2/seed/seed_data.json` using `multi_replace_file_content` with exact bounded line ranges. NEVER attempt to rewrite the entire file at once (causes truncation), and DO NOT create standalone python scripts for modifying.</step>
    <step id="3.5_syntax_check">JSON INTEGRITY CHECK: Immediately after executing `multi_replace_file_content`, verify JSON syntax via a dry-run read. CIRCUIT BREAKER: If JSON syntax is broken, DO NOT attempt iterative patching; immediately restore the backup from step `2_backup` and STOP.</step>
    <step id="4_verify">VERIFY (Critical Gate): Trigger native backend audit script via `run_command`: `uv run python scripts/backend_audit_loop.py backend_v2 --test`. If tests fail, YOU MUST revert.</step>
    <step id="5_report">REPORT: Describe specifically what paths aligned with expectations contextually.</step>
    <step id="6_reseed">RE-SEED: Once verified, autonomously execute the final seeding command via `run_command`: `uv run python backend_v2/seed/run_seed.py local`.</step>
</vault_mutation_protocol>