...
<ide_orchestration_protocol>
    ...
    <rule_block id="anti_ambiguity_mandate">
        <mandate>Implementation plans, epics, research analysis, and bug hunting artifacts MUST be strictly programmatic and deterministic. NEVER use "e.g.", "such as", "like", "etc.", or open-ended "specifically:" lists in planning, tracking, and architectural documentation. Use either an explicit closed list ("specifically and exhaustively: A, B, C") OR a programmatic reference to the Single Source of Truth (specifically ALL models inheriting from AnySduiBlock in models/view/sdui.py). When executing programmatic references, your absolute FIRST action MUST be to use `grep_search` to physically query and explicitly list all matching concrete entities. NEVER use generic model definitions, generic paths, visual string examples ("A" -> "B"), or ambiguous UI tree placements. PROMPT & SCHEMA EXEMPTION: This documentation ban DOES NOT apply to LLM prompts, system instructions, or JSON schema field descriptions. In LLM instructions, "e.g." is permitted and expected whenever providing illustrative, open-ended examples (such as alias examples `(e.g., 'a0', 'a1')`, sample absence rules, or context modalities). You may remove "e.g." or replace it with "specifically:" ONLY IF you are 100% mathematically certain that the referenced list is permanently and exhaustively fixed across the system lifecycle (an immutable closed set); otherwise, "e.g." MUST be retained.</mandate>
    </rule_block>
...
<catastrophic_system_bans>
    ...
    <rule_block id="ban_heuristic_identifier_matching">
        <mandate>NEVER identify, filter, target, or resolve workflow steps, prompt blocks, matrices, execution artifacts, LLM providers, or credentials in background workers, factories, or services using hardcoded identifier tuples, name markers, keyword lists (`_MARKERS`, `_KEYWORDS`, `_NAMES`), substring searches (`marker in step.name.lower()`), string prefix checks (`model_name.startswith("vertex_ai/")`, `"gemini" in model_name`), negative string filtering (specifically and exhaustively: `not k.startswith(...)`, `not val.startswith("$")`), backward-compatibility transition shims (`allowed_keys`), heterogeneous string collections mixing entity IDs with enums (`allowed_blocks` mixing PromptBlock IDs with TargetBlockType, `step_states` mixing DAG steps and background tasks), or untyped expression strings. ALL entity targeting, provider resolution, credential binding, execution, reporting, synthesis, or metric evaluation MUST be explicitly resolved via typed DTO contracts, explicit Enum relations (`provider_type: LLMProvider`), dynamic Studio UI configuration (e.g., `OutputProfile.variance_target_block`), positive ID-set resolution (`k in valid_id_set`), and cleanly separated typed collections. Hardcoding heuristic string lookups or string prefix parsing as a substitute for explicit typed contracts is STRICTLY PROHIBITED.</mandate>
    </rule_block>
    <rule_block id="positive_id_set_filtering_mandate">
        <mandate>NEVER use negative string exclusion patterns (specifically and exhaustively: `not k.startswith("sys_")`, `not val.startswith("$")`, `not b.startswith("glb_")`) to filter entity IDs, determine document targets, or separate system tasks from domain workflow steps. ALL entity selection, step verification, and context targeting MUST be executed via strictly positive set membership against authoritative entity collections (e.g. `workflow_step_ids.issubset(record.step_states.keys())`, `target_wf.get_allowed_prompt_block_targets(all_steps)`, `targets.allowed_input_keys`). If an entity ID is missing from an authoritative set, Fail-Fast or treat as unmapped.</mandate>
    </rule_block>
