...
<catastrophic_system_bans>
    ...
    <rule_block id="the_zero_compromise_pledge">
        <mandate>Enforce strict Pydantic V2/Freezed schemas and 100% typed domain transit. If an expected key is missing, raise an explicit `AppException` and CRASH. Zero tolerance for silent bypasses. NEVER implement backwards compatibility, fallback chains ("if A is missing, try B"), language-level defaults (`v.get('field', '')`), `hasattr()`, `isinstance(data, dict)`, `match/case dict`, `cast(Any, ...)`, naked dictionaries (`dict[str, Any]`, `list[dict]`, `TypedDict`), `# noqa: QGR` suppressions, or recursive dictionary loops.</mandate>
    </rule_block>
