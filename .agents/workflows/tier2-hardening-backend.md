...
8. **AUDIT LOOP MANDATE & CIRCUIT BREAKER:** After code changes, you MUST run the Universal Quality Gate YOURSELF as defined in `AGENTS.md`. DIRTY STATE ROLLBACK: If the Quality Gate fails 3 times on your refactor (Circuit Breaker trips), you MUST STOP attempting to duct-tape the code. You MUST explicitly instruct the user to run `git restore .` to wipe the corrupted workspace state before re-evaluating your approach or requesting a handover.
