  ...
  <execution_protocol level="1">
    ...
    <step id="11">STANDARDIZED TRACKER GENERATION (Synthesized Headings from EPIC 106/107/108 Trackers): After generating the implementation plans, you MUST create a master tracker file at absolute path `c:\src\quorum\docs\epic\EPIC_[num]_[name]_tracker.md` using `write_to_file`. The tracker MUST adhere to the following standard header structure:
      ...
      - **`### Post-Implementation Gates`**:
        ...
        - `[ ] **[NOK] Documentation & Knowledge Item Update**`: Update `docs/architecture/`, `.agents/rules/04_directory_reference.md`, and create Knowledge Item in `<appDataDir>\knowledge\`.
        - `[ ] **[NOK] MANDATORY Final E2E REST API Verification Gate**: `$env:RUN_LIVE_E2E="true"; uv run pytest backend_v2/tests/integration/test_integration_real_llm.py`.
      - **`## Instructions for the Execution Agent`**: Atomic commit mandates, seeding environment commands (`uv run python backend_v2/seed/run_seed.py local`), `@-reference` syntax rule.
      ...
      - **`# Session Handover Context`**: Exhaustive summaries for `## Achieved`, `## Learned`, and `## Remaining`.
      - **`**Resume Command for Next Session:**`: Provide explicit `/tier5-resume` commands:
        - For Plan Validation: `/tier5-resume --workflow=/tier0-research-plan --target="@[c:\src\quorum\docs\epic\tasks_EPIC_[num]_[name]\implementation_plan_phase_1.md]"`
        - For Direct Execution: `/tier5-resume --workflow=/tier2-execute --target="@[c:\src\quorum\docs\epic\EPIC_[num]_[name]_tracker.md]"`
    </step>
    <step id="12">PAUSE & EMBEDDED HANDOVER: Every generated sub-plan MUST end with an explicit "Session Handover" markdown block. Provide the exact copy-paste command for the user to start the continuous loop (`/tier5-resume`). Once files and tracker are written, present them to the user. Inform the user that they MUST switch to a fresh context window. Do NOT implement any domain code yourself in this session.</step>
