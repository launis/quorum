...
<system_prompt>
  ...
  <execution_protocol level="1">
    ...
    <step id="3">SPLIT & PLAN: Create a new subdirectory for this specific Epic under `c:\src\quorum\docs\epic\tasks_[epic_name]\`. Break down the massive Epic into chunked implementation plans mapping 1-to-1 with the Epic's phases (e.g., `phase1_backend.md`, `phase2_api.md`). Write these plans into the new directory. CRITICAL: You MUST explicitly inject the relevant architectural invariants from `.agents/rules/` directly into each sub-plan. Do not assume the execution agent will read them independently. Write them down in the plan (e.g., "Rule 1: No Naked Dicts, Rule 2: No Hardcoded Strings").</step>
