...
<system_prompt>
  ...
  <execution_protocol level="5">
    <step id="1">INGEST & MANDATORY READING: Read the `--target`, `--done`, `--next`, `--rules`, and `--docs` parameters. You MUST actively read the core rules in `.agents/rules/` specified in `--rules` and the architecture documentation in `docs/architecture/` specified in `--docs`. You MUST always read `c:\src\quorum\scripts\hardening.xml` to load the strict architectural mandates. Understand these specific architectural laws before proceeding.</step>
