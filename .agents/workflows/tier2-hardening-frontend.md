...
* If a Master Tracker was provided in the command or context (e.g. `@[docs\epic\EPIC_XXX_tracker.md]`), you MUST read the tracker's `### Post-Implementation Gates` section and extract the indented `- [ ] @[relative_path.dart]` sub-items under `Tier 2 Hardening (Frontend)`. Use this extracted file list as the DEFINITIVE target list for this hardening session, skipping any sub-items already marked `- [x]`.
...
3. **EVIDENCE-BASED INSPECTION & MANDATED MATRIX**:
   ...
   * For EVERY audited file, you MUST physically read the file line-by-line using `view_file` and produce SUBSTANTIVE CODE EVIDENCE for Frontend Invariants:
     1. **ZERO COMPROMISE & FALLBACK BAN:** Grep/inspect for null-coalescing (`?? 'default'`), `.maybeWhen` defaults hiding structural errors, or empty `SizedBox.shrink()` masking missing data. Prove that missing data triggers Fail-Fast `AppErrorBoundary` or crashes loudly.
     2. **FREEZED & ENUM PARITY:** Verify models use `@freezed` with `@JsonSerializable(disallowUnrecognizedKeys: true)` and camelCase `@JsonKey(name: '...')` annotations perfectly 1:1 with Python backend.
     3. **DUAL-AXIS LOCALIZATION (ICU/ARB):** Prove all static labels use `context.l10n.<key>` or `AppLocalizations.of(context)`; ban raw hardcoded UI strings.
     4. **SDUI POLYMORPHIC DESERIALIZATION:** Verify SDUI blocks parse through sealed union `SduiBlockDTO` without raw `Map<String, dynamic>` casting in presentation widgets.
     5. **DATABASE SOVEREIGNTY:** Verify no hardcoded table IDs, column slugs, or UUIDs exist in UI components; all metadata must derive from SSOT Enums or API envelopes.
     6. **ISTQB WIDGET/UNIT TEST PROOF:** Locate the corresponding test file in `client_app_v2/test/`, verify at least 2 negative/boundary test cases exist (missing fields triggering error boundary, malformed JSON), and verify pass via `flutter_audit_loop.py`.
   * **Matrix Generation & Verification:** Generate a fresh matrix for the specific target: `uv run python scripts/audit_matrix_manager.py generate --type frontend --target <target_file>`. Populate file-specific substantive justifications with concrete line references for applicable rules and set orchestration rules to NA. Then, run `uv run python scripts/audit_matrix_manager.py verify --file tmp/audit_matrix.json --target <target_file>`. Reusing a stale matrix JSON without regeneration for each target file is a catastrophic quality gate violation. You are strictly forbidden from proceeding until this script exits with success.
