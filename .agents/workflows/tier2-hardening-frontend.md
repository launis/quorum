...
3. **EVIDENCE-BASED INSPECTION & MANDATED MATRIX**:
   ...
   * **Matrix Generation & Verification:** Generate the empty matrix: `uv run python scripts/audit_matrix_manager.py generate --type frontend`. Populate file-specific substantive justifications for applicable rules and set orchestration rules to NA. Then, run `uv run python scripts/audit_matrix_manager.py verify --file tmp/audit_matrix.json`. You are strictly forbidden from proceeding until this script exits with success.
