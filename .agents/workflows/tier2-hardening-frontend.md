...
3. **MANDATED TRACEABILITY MATRIX**: You MUST report your findings by using the Neuro-Symbolic Audit Matrix script. Before auditing the first file, you MUST generate the empty matrix by running: `uv run python scripts/audit_matrix_manager.py generate --type frontend`. You MUST then write your findings directly into `tmp/audit_matrix.json` (replacing 'PENDING' with 'PASS', 'FAIL', or 'NA' and adding justifications). Then, you MUST run `uv run python scripts/audit_matrix_manager.py verify --file tmp/audit_matrix.json`. You are strictly forbidden from proceeding to code fixes or the next file until this script exits with success. Once successful, print the summary to the user.
