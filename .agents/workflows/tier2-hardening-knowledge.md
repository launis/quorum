...
<system_prompt>
  ...
  <context_rules>
    ...
    <rule_block id="context_amnesia_prevention">
      <mandatory_pattern>Whenever you generate a handover command, tracker file, or output a list of files, you MUST explicitly wrap all target file paths in `@-reference` syntax (e.g., `@[knowledge\target.md]`).</mandatory_pattern>
      ...
      <rule_block id="xml_refactoring_mandate">
        ...
        <mandatory_pattern>You MUST completely rewrite KI files into strict XML format using `<domain_boundary>`, `<architectural_invariants>`, `<rule_block>`, `<banned_pattern>`, `<mandatory_pattern>`, and `<catastrophic_reason>`.</mandatory_pattern>
...
4. **KI CODE TEMPLATE COMPILATION GATE**: If the KI artifact contains Python code templates (inside fenced code blocks), you MUST validate them by running `uv run python -m py_compile <temp_file>` on the extracted snippet (written to a scratch directory file). If the KI artifact contains Dart code templates, validate basic syntax by checking for matching braces and import resolution. If compilation fails, the KI file MUST be flagged as FAIL and the code template corrected before marking the KI as DONE.
...
6. **AUTONOMOUS FIX & NEXT:** Use `write_to_file` to overwrite the KI file with the new XML structure. Print a brief summary of the changes and immediately proceed to the next undone file on the list. Do NOT wait for a "FIX" or "NEXT" command. 
7. **STATE PERSISTENCE:** When the file is completely refactored and saved, update or create `tmp\hardening_ki_state.json` and mark the KI file as "DONE".
8. **SESSION LIMIT & HANDOVER:** Keep a tally of the total number of KI files you have audited in this session. If you have processed 3 files, STOP immediately once the file is complete. Do not move on to the next. You MUST physically append a `# Session Handover Context` block to the bottom of a Tracker file (e.g., create or update `tmp\hardening_ki_tracker.md`) detailing `achieved` and `remaining`, because `/tier5-resume` requires reading from a file, NOT from the chat history. Then, print to the user exactly: "Session limit reached. Continue by issuing the command: `/tier5-resume --target="@[absolute_path_to_tracker.md] @[target_directory_being_audited]" --workflow=/tier2-hardening-knowledge --rules="@[.agents\rules\00-antigravity-core.md]"`".
    </phase>
  </phases>
</system_prompt>
```
