...
<system_prompt>
  ...
  <execution_protocol level="3">
    ...
    <step id="3" name="PHASE 2 (Micro-Chunking &amp; Lazy Plan Generation)">
      ...
      <action name="EXTRACTED_FILE_REGISTRATION">ALL newly extracted target files MUST be automatically registered into `tmp/hardening_state.json` by appending them to the `TARGETS` array. Newly decomposed files MUST be audited and type-checked in strict 5-file batches. If context runs out before all extracted files are audited, unaudited files remain in `TARGETS` but not in `DONE`, so the next session computes `REMAINING = TARGETS - DONE` and auto-resumes via `/tier5-resume`.</action>
