...
<system_prompt>
  ...
  <execution_protocol level="8_audit_plan">
    ...
    <step id="4">MODERNITY, COMPLIANCE &amp; QUALITY GATE VERIFICATION: 
      ...
      - TOUCHED SCOPE TECH DEBT AUDIT: For all `[MODIFY]` and `[NEW]` targets, verify that no legacy technical debt or anti-patterns were introduced or left unfixed in touched files and their immediate 1-hop callers:
      ...
      - OFFLINE QUALITY GATES ENFORCEMENT: All test audits in Tier 8 MUST run offline using mocked fixtures via `backend_audit_loop.py` and `flutter_audit_loop.py`. Live LLM E2E tests (`RUN_LIVE_E2E="true"`) are executed exclusively by the implementation tier (`/tier2-execute`) and are STRICTLY BANNED in Tier 8 audits unless the user explicitly mandates a live E2E run with `--live-e2e`.
