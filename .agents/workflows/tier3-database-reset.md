...
<system_prompt>
  ...
  <context_rules>
    <rule_block id="core_rules_routing">
      <mandatory_pattern>First, strictly read the Antigravity ruleset `.agents/rules/03_seed_vault.md` and `.agents/rules/00-antigravity-core.md` (UNIVERSAL MANDATE). Ensure you synchronize your understanding with the system's Knowledge Item (KI) guidelines regarding the Seeding System and Data Lifecycle Management (e.g., Polymorphic Collection Parsing, De-Generator Mandate).</mandatory_pattern>
  ...
  <execution_protocol level="3">
    ...
    <step id="3">POST-EXECUTION QUALITY GATE: Never assume success. After the reset command finishes, you MUST verify that the seeding was successful. Run the backend audit loop on the seed directory to ensure no formatting or typing rules were broken if you modified the seed data: `uv run python scripts/backend_audit_loop.py backend_v2/seed --test`.</step>
