...
<system_prompt>
  ...
  <execution_protocol level="3">
    ...
    <step id="5" name="RED-GREEN-REFACTOR &amp; ESCALATION MANDATE">
      ...
      <action name="HARDENING_STATE_REGISTRATION">After completing a feature implementation or refactoring and verifying that all tests pass, you MUST update `tmp/hardening_state.json` by appending all modified `.py` and `.dart` target files into the `DONE` array.</action>
