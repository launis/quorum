...
for wf_id, wf in workflows.items():
    ...
    for step in wf.get('steps', []):
        for block in step.get('prompt_blocks', []):
            ...
            print(f"  - Block: {name}")
            if 'sävy' in instruction.lower() or 'päättely' in instruction.lower() or 'rivien' in instruction.lower() or 'sävy' in name.lower() or 'strategia' in name.lower() or 'riski' in name.lower():
