...
async def test_delete_step_integrity_violation(client: AsyncClient, admin_token_headers):
    ...
    # 1. Create a Step
    step_payload = {"id": "step_critical", "name": "Critical Step", "component": "Judge", "execution_config": {}}
...
async def test_delete_workflow_integrity_violation(client: AsyncClient, admin_token_headers):
    ...
    # 1. Create Workflow
    wf_payload = {"name": "Historical Workflow", "description": "Has history", "steps": []}
    res = await client.post("/builder/workflows", json=wf_payload, headers=admin_token_headers)
...
async def test_delete_organization_active_jobs(client: AsyncClient, admin_token_headers):
    ...
    org_res = await client.post(
        "/organizations/", json={"name": "Busy Corp", "tier": "standard"}, headers=admin_token_headers
    )
    assert org_res.status_code == 201 or org_res.status_code == 200
    org_id = org_res.json()["id"]

    # 2. Create User in Org
    user_payload = {"email": "worker@busy.com", "display_name": "Worker", "role": "MEMBER", "password": "password123"}
    ...
    wf_payload = {"name": "Org Workflow", "steps": [], "organization_id": org_id}
    ...
    if wf_res.status_code == 200:
        ...
        await client.delete(f"/organizations/{org_id}", headers=admin_token_headers)
        # Verify 409
