...
from backend.models.domain import ActionGroup, ActionItem, CoachingPlan
from backend.models.state import WorkflowInputs, WorkflowState
...
def test_coach_hook_logic():
    ...
    try:
        ...
        if len(res) > 0 and "Aristotle" in res[0]:
            ...
            logger.warning(
                f"PARTIAL: Code ran but didn't find citation. Resurssit: {res}. (This might be regex/text match issue, but no crash is the main goal)"
            )
