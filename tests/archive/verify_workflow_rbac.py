...
class MockUser(User):
    def __init__(self, uid, role, org_id):
        super().__init__(uid=uid, email=f"{uid}@test.com", role=role, organization_id=org_id, created_at="2024-01-01")
...
def run_test():
    ...
    if res.status_code == 200:
        ...
        if sys_wf["organization_id"] == "system" and sys_wf["is_public"]:
