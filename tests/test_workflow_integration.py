    ...
    for i in range(max_retries):
        try:
            ...
            if status in ["completed", "COMPLETED"]:
                ...
                if sec_check:
                    assert "uhka_havaittu" in sec_check or "anonymisointi_tehty" in sec_check, (
                        "GuardAgent security_check incomplete"
                    )
                else:
                    assert (
                        "tainted" in guard or "is_clean" in guard or "analyysi" in guard or "security_check" in guard
                    ), "GuardAgent output missing critical keys"
                ...
                assert evidence_count > 0 or analyst.get("hypoteesit"), (
                    "AnalystAgent failed to extract specific evidence or return valid structure."
                )
                ...
                has_sub_audit = (
                    "logiikka_auditointi" in panel
                    or "falsifiointi_auditointi" in panel
                    or "kausaalinen_auditointi" in panel
                )
                ...
                report_content = (
                    result.get("xai_report") or result.get("report_content") or result.get("xai_report_content")
                )
