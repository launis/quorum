...
import backend.api.tools_router
...
async def test_ssrf_blocking_localhost(client: AsyncClient, admin_token_headers):
    ...
    assert "Access to local network resources is forbidden" in res.json()["detail"] or "Access to private IP" in res.json()["detail"]
...
async def test_quota_enforcement(client: AsyncClient, admin_token_headers):
    ...
    org_res = await client.post("/organizations/", json={"name": "Poor Corp", "quota_limit": 0.01}, headers=admin_token_headers)
