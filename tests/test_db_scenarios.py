...
def test_missing_step_definition(engine):
    ...
    engine.repository.workflows.insert({"id": "wf_missing_step", "name": "BrokenWorkflow", "steps": ["STEP_MISSING"]})
...
def test_empty_prompt_content(engine):
    ...
    engine.repository.steps.insert(
        {"id": "STEP_TEST", "component": "TestAgent", "execution_config": {"llm_prompts": ["PROMPT_EMPTY"]}}
    )

    # 2. Define Prompt Component (Empty)
    engine.repository.components.insert({"id": "PROMPT_EMPTY", "type": "prompt", "content": ""})
    ...
    engine.repository.steps.insert(
        {
            ...
            "component": "TestAgent",
            "execution_config": {"llm_prompts": ["PROMPT_NONEXISTENT"]},
        }
    )
