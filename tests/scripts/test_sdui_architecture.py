...
def audit_file(filepath: str) -> list[str]:
    ...
    if not build_method:
        ...
        # Check staticmethod
        is_static = any(isinstance(d, ast.Name) and d.id == "staticmethod" for d in build_method.decorator_list)
        if not is_static:
        ...
        if len(args) != 1 or not (
            args[0].arg == "context" and getattr(args[0].annotation, "id", "") == "AdapterContext"
        ):
    ...
    class RuleVisitor(ast.NodeVisitor):
        ...
        def visit_Call(self, node):
            ...
            if isinstance(node.func, ast.Attribute) and node.func.attr == "get":
                if isinstance(node.func.value, ast.Name) and node.func.value.id.endswith("_RULES"):
                    self.violations.append(
                        f"[KI_SDUI_ADAPTER_PATTERN] Fail-fast violation: Used .get() on {node.func.value.id} at line {node.lineno}."
                    )
            ...
            if func_id in ("min", "max", "sum", "round"):
                self.violations.append(
                    f"[KI_TRIPARTITE_PIPELINE_ARCHITECTURE] God Method leak: SDUI Adapters must be dumb painters and must not perform mathematical calculations like `{func_id}()` at line {node.lineno}."
                )
            ...
            if isinstance(node.func, ast.Attribute):
                if isinstance(node.func.value, ast.Name):
                    if node.func.value.id in ("db", "session", "repository", "llm"):
                        self.violations.append(
                            f"[KI_TRIPARTITE_PIPELINE_ARCHITECTURE] God Method leak: Direct IO or DB lookup `{node.func.value.id}.{node.func.attr}` at line {node.lineno}."
                        )
        ...
        def visit_BinOp(self, node):
            if isinstance(node.op, (ast.Mult, ast.Div, ast.Sub)):
                self.violations.append(
                    f"[KI_TRIPARTITE_PIPELINE_ARCHITECTURE] Business Logic leak: SDUI Adapters must not perform mathematical operations (*, /, -) at line {node.lineno}."
                )
            self.generic_visit(node)

        def visit_ImportFrom(self, node):
            if node.module and any(
                forbidden in node.module for forbidden in ("utils.scoring", "orchestrator", "llm", "repository", "api")
            ):
                self.violations.append(
                    f"[KI_TRIPARTITE_PIPELINE_ARCHITECTURE] Business Logic leak: SDUI Adapters must not import from Domain Engines or IO layers (`{node.module}`) at line {node.lineno}."
                )
            self.generic_visit(node)
        ...
        def visit_Attribute(self, node):
            if node.attr in ("computed_min", "computed_max"):
                self.violations.append(
                    f"[KI_TRIPARTITE_PIPELINE_ARCHITECTURE] Banned pattern: SDUI Adapters must be dumb painters and must not perform domain math bounds calculation using `{node.attr}` at line {node.lineno}."
                )
            self.generic_visit(node)
        ...
        def visit_Name(self, node):
            if node.id == "blocks_by_id":
                self.violations.append(
                    f"[KI_TRIPARTITE_PIPELINE_ARCHITECTURE] Banned pattern: SDUI Adapters must not perform raw database/ontology lookups using `{node.id}` at line {node.lineno}."
                )
            self.generic_visit(node)
            ...
            if len(node.ops) > 0 and isinstance(node.ops[0], ast.In):
                for comp in node.comparators:
                    if isinstance(comp, ast.Name) and comp.id.endswith("_RULES"):
                        self.violations.append(
                            f"[KI_SDUI_ADAPTER_PATTERN] Fail-fast violation: Used `in` operator on {comp.id} at line {node.lineno}."
                        )
            self.generic_visit(node)
                ...
                for body_node in node.body:
                    if isinstance(body_node, ast.Expr) and isinstance(body_node.value, ast.Call):
                        ...
                        if (
                            ...
                            and getattr(call.func.value, "id", "") == "logger"
                    ...
                    if isinstance(body_node, ast.Raise):
                        if (
                            isinstance(body_node.exc, ast.Call)
                            and getattr(body_node.exc.func, "id", "") == "AppException"
                        ):
                            has_raise_app_exception = True

                if not (has_logger_error and has_raise_app_exception):
                    self.violations.append(
                        f"[KI_SDUI_ADAPTER_PATTERN] Dual Logging violation: KeyError handler at line {node.lineno} does not `logger.error(..., exc_info=True)` followed by `raise AppException(...)`."
                    )
...
def get_adapter_files():
    ...
    return [str(p) for p in base_dir.glob("*.py") if p.name not in ("__init__.py", "base_adapter.py")]
