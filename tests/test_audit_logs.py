...
async def test_audit_access_control(client: AsyncClient):
    ...
    # 4. ROOT Access -> 200
    res = await client.get(f"/audit/logs", headers=root_header)
    with open("test_progress.log", "a") as f:
...
async def test_audit_lifecycle_root(client: AsyncClient, admin_token_headers):
    ...
    res_logs = await client.get(f"/audit/logs?organization_id={org_id}&action=USER_CREATED", headers=admin_token_headers)
    assert res_logs.status_code == 200
    logs = res_logs.json()
    ...
    res_logs = await client.get(f"/audit/logs?organization_id={org_id}&action=USER_DELETED", headers=admin_token_headers)
    with open("test_progress.log", "a") as f:
