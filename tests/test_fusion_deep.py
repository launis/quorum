...
MOCK_PANEL_OUTPUT = {
    "logiikka_auditointi": {
        "metadata": {
            "luontiaika": "2024-12-25T12:00:00",
            "agentti": "Panel",
            "vaihe": 5.0,
            "versio": "2.0",
    ...
    "kausaalinen_auditointi": {
        ...
        "kontrafaktuaalinen_testi": {
            "skenaario_A_toteutunut": "A",
            "skenaario_B_simulaatio": "B",
            "uskottavuus_arvio": "Valid",
        },
...
def test_compile_fusion_flow(shared_engine):
    ...
    try:
        ...
        copy_res = client.post(
            f"/builder/workflows/{target['id']}/copy", json={"new_name": "Fusion Test WF"}, headers=headers
        )
...
async def test_panel_agent_fan_out():
    ...
    state = WorkflowState(
        execution_id="test_exec", inputs=InputData(history_text="H", product_text="P", reflection_text="R")
    )
