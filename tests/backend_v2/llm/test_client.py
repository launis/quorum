...
async def test_semantic_self_healing_retry(mock_create_provider: MagicMock) -> None:
    ...
    result_model, total_usage = await client.run_structured_task(
        messages=messages,
        response_model=DummyStrictModel,
        max_retries=2  # Essential for Self-Healing!
    )
    ...
    assert "STRICT LOGICAL COMPLIANCE REQUIRED" in socratic_system
