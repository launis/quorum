...
async def main():
    ...
    for evt in execution.execution_trace:
        if evt.step_name == 'sr_2fa56dc36614469a':
            ...
            content = evt.content
            print(f'Tuotettu asiasisalto (avaimet): {list(content.keys()) if isinstance(content, dict) else "Ei Dict"}')
            if isinstance(content, dict):
                for k, v in content.items():
                    val_str = str(v)
                    print(f'  -> Sisa-avain "{k}": pituus {len(val_str)} merkkia')
                    if len(val_str) > 100:
                        print(f'     Esikatselu: {val_str[:150].replace(chr(10), " ")}...')
    if not found:
        print('Ei loytynyt tapahtumia talla steppi-id:lla!')
