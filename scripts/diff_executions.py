...
if __name__ == "__main__":
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for idx, (run_name, exe_path) in enumerate(zip(loaded_runs, loaded_paths, strict=False)):
            ...
            if model_used == "Ei tallennettu DB":
                if "gemini-2.5-pro" in raw_data:
                    model_used = "gemini-2.5-pro (Jälki-loki)"
                elif "gemini-2.5-flash" in raw_data:
                    model_used = "gemini-2.5-flash (Jälki-loki)"
                elif "gpt-4" in raw_data:
                    model_used = "gpt-4 (Jälki-loki)"
        ...
        f.write(
            f"- **Erimielisyyttä näiden välillä:** {len([a for a in common_atoms if get_state(evals_1[a]) != get_state(evals_2[a])])} kpl\n"
        )
        f.write(
            f"- **Contextual Override -lähtöiset erimielisyydet koko setissä:** {contextual_override_mismatches} / {len(mismatching_atoms)}\n"
        )
    ...
    print(
        f"PASSED->FAILED: {summary_2way['PASSED->FAILED']}, FAILED->PASSED: {summary_2way['FAILED->PASSED']}, Other: {summary_2way['Other']}"
    )
