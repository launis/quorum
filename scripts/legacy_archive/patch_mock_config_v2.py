...
# Updated path to match backend logs
db_path = r'c:\Users\risto\OneDrive\quorum\backend\database\db_mock.json'

if not os.path.exists(db_path):
    print(f"DB not found at {db_path}")
    # Fallback to verify logic
    alt_path = r'c:\Users\risto\OneDrive\quorum\data\db_mock.json'
    if os.path.exists(alt_path):
        print(f"Found at alt path {alt_path}, patching that instead.")
        db_path = alt_path
    else:
        print("Cannot find db_mock.json anywhere.")
        exit(1)
